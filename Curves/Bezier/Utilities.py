...
from Utilities.ObjectOps.ObjectOps import ObjectOps as oop
from Utilities.CurveOps.CurveOps import CurveOps as cop
from bpy.props import BoolProperty, IntProperty, EnumProperty, FloatProperty, StringProperty, CollectionProperty, FloatVectorProperty, PointerProperty
...
from math import e, pi, log, sin, cos, tan, radians, degrees, sqrt, asin, acos, atan, floor, ceil, pow, exp
...
def copyBezierPt(src, target, freeHandles = None, srcMw = Matrix(), invDestMW = Matrix()): #TODO combine with copyObjAttr
    ...
    if(freeHandles == None or freeHandles[0] == False): target.handle_left_type = src.handle_left_type
    if(freeHandles == None or freeHandles[1] == False): target.handle_right_type =  src.handle_right_type
...
def createSpline(curveData, srcSpline, excludePtIdxs = {}):
    ...
    for i in range(0, len(srcSpline.bezier_points)):
        if(i not in excludePtIdxs):
            copyBezierPt(srcSpline.bezier_points[i], spline.bezier_points[ptIdx], freeHandles = None)
            ptIdx += 1
    return spline
...
def createSkeletalCurve(obj, collections):
    ...
    for coll in collections: coll.objects.link(objCopy)
    return objCopy
...
def createObjFromPts(curvePts, dimensions = '3D', collection = None, \
    ...
    obj = bpy.data.objects.new('BezierCurve', data) # collection = context.collection
    if(collection == None): collection = bpy.context.scene.collection
    ...
    for i, pt in enumerate(curvePts):
        ...
        if(not calcHdlTypes and len(pt) > 3):
            ...
            and pt[0] == pt[1] and currPt.co != prevPt.co): # straight line
                if(prevPt.handle_left_type != 'VECTOR'): prevPt.handle_left_type = 'FREE'
            ...
            ldiffV = (pt[1] - pt[0])
            rdiffV = (pt[2] - pt[1])
            if(vectCmpWithMargin(ldiffV, rdiffV) and not floatCmpWithMargin(ldiffV.length, 0)):
                currPt.handle_left_type = 'ALIGNED'
                currPt.handle_right_type = 'ALIGNED'
        prevPt = currPt
    bpts = spline.bezier_points
    if(spline.use_cyclic_u and vectCmpWithMargin(bpts[-1].handle_right, bpts[-1].co) and vectCmpWithMargin(bpts[0].handle_left, bpts[0].co)):
            if(bpts[-1].handle_left_type != 'VECTOR'): bpts[-1].handle_left_type = 'FREE'
            bpts[-1].handle_right_type = 'VECTOR'
            if(bpts[0].handle_right_type != 'VECTOR'): bpts[0].handle_right_type = 'FREE'
            bpts[0].handle_left_type = 'VECTOR'
    return obj

def removeShapeKeys(obj):
    if(obj.data.shape_keys == None): return
    keyblocks = reversed(obj.data.shape_keys.key_blocks)
    for sk in keyblocks: obj.shape_key_remove(sk)
...
def getShapeKeyInfo(obj):
    ...
    if(obj.data.shape_keys != None):
        ...
        for key in keyblocks:
            keyData.append([[d.handle_left.copy(), d.co.copy(), d.handle_right.copy()] for d in key.data])
            keyNames.append(key.name)
    return keyNames, keyData

def updateShapeKeyData(obj, keyData, keyNames, startIdx, cnt = None, add = False):
    if(obj.data.shape_keys == None and not add): return
...
def copyObjAttr(src, dest, invDestMW = Matrix(), mw = Matrix()): #TODO: Fix this hack if possible
    for att in dir(src):
        try:
            if(att not in ['co', 'handle_left', 'handle_right', 'handle_left_type', 'handle_right_type']): setattr(dest, att, getattr(src, att))
...
def moveSplineStart(obj, splineIdx, idx):
    ...
    cnt = len(pts)
    ptCopy = [[p.co.copy(), p.handle_right.copy(), p.handle_left.copy(), p.handle_right_type, p.handle_left_type] for p in pts]
...
def joinCurves(curves):
    ...
    for curve in curves[1:]:
        ...
        for spline in curve.data.splines:
            ...
            for i, pt in enumerate(spline.bezier_points): copyObjAttr(pt, newSpline.bezier_points[i], invDestMW = invMW, mw = mw)
        oop.safe_remove(curve)
    return obj

def getObjBBoxCenter(obj):
    bbox = obj.bound_box
    return obj.matrix_world @ Vector(((bbox[0][0] + bbox[4][0]) / 2, (bbox[0][1] + bbox[3][1]) / 2, (bbox[0][2] + bbox[1][2]) / 2))

def shiftOrigin(obj, origin): # Only mesh and Bezier curve
    oLoc = obj.location.copy()
    mw = obj.matrix_world
    invMw = mw.inverted_safe()
    if(obj.type == 'MESH'):
        for vert in obj.data.vertices: vert.co += invMw @ oLoc - invMw @ origin
    elif(obj.type == 'CURVE'):
...
def shiftMatrixWorld(obj, mw): # Only mesh and Bezier curve; depsgraph not updated
    invMw = mw.inverted_safe()
    omw = obj.matrix_world
    if(obj.type == 'MESH'):
        for vert in obj.data.vertices: vert.co = invMw @ (omw @ vert.co)
    elif(obj.type == 'CURVE'):
        for s in obj.data.splines:
            bpts = s.bezier_points
            for bpt in bpts:
...
def alignToNormal(curve): # Also shifts origin; depsgraph not updated
    ...
    cnt = len(normals)
    if(cnt > 0):
        normal =  Vector([sum(normals[i][j] for i in range(cnt)) for j in range(3)]) / cnt
        quatMat = normal.to_track_quat('Z', 'X').to_matrix().to_4x4()
        shiftMatrixWorld(curve, quatMat)

def copyProperties(srcObj, destCurve):
    if(srcObj == None or destCurve == None): return
    destData = destCurve.data
    srcData = srcObj.data
    if(cop.is_bezier(srcObj)): # If object is bezier curve copy curve properties and material
        destData.dimensions = srcData.dimensions # Copying just a few attributes
    ...
    if(hasattr(srcData, 'materials') and len(srcData.materials) > 0):
        ...
        if(len(destData.materials) == 0 or mat.name not in destData.materials):
            ...
            activeIdx = -1 #Last
        else: activeIdx = destData.materials.find(mat.name)
...
def insertSpline(obj, srcSplineIdx, insertIdx, removePtIdxs): # Insert spline at location insertIdx, duplicated from existing spline at location srcSplineIdx and remove points with indices in removePtIdxs from new spline
    srcSpline = obj.data.splines[srcSplineIdx]
    createSpline(obj.data, srcSpline, removePtIdxs) # Appended at end
...
def removeBezierPts(obj, splineIdx, removePtIdxs):
    ...
    if(min(removePtIdxs) >= len(bpts)): return
    if(len(set(range(len(bpts))) - set(removePtIdxs)) == 0) :
        obj.data.splines.remove(oldSpline)
        if(len(obj.data.splines) == 0): oop.safe_remove(obj)
        return
    insertSpline(obj, splineIdx, splineIdx, removePtIdxs)
    obj.data.splines.remove(obj.data.splines[splineIdx + 1])

def removeBezierSeg(obj, splineIdx, segIdx): # Returns a tuple with first value indicating change in spline index (-1, 0, 1) and second indicating shift in seg index (negative) due to removal
    ...
    if(ptCnt <= 2):
        ...
        splineIdxIncr = -1 # Spline removed by above call
    else:
        ...
        if(spline.use_cyclic_u):
            ...
            if(segIdx == lastSegIdx): removeBezierPts(obj, splineIdx, {lastSegIdx + 1})
            elif(segIdx == 0):
...
def toHexStr(rgba): # https://devtalk.blender.org/t/get-hex-gamma-corrected-color/2422/2
    ch = []
    for c in rgba[:3]:
        if c < 0.0031308: cc = 0.0 if c < 0.0 else c * 12.92
        else: cc = 1.055 * pow(c, 1.0 / 2.4) - 0.055
        ch.append(hex(max(min(int(cc * 255 + 0.5), 255), 0))[2:])
    return ''.join(ch), str(rgba[-1])

def changeMW(obj, newMW): # Change position of bezier points according to new matrix_world
...
def updateCurveEndPtMap(endPtMap, addObjNames = None, removeObjNames = None): # Return map in the form of objName->[splineIdx, [startPt, endPt]] . Remove the invalid keys (if any) from it.
    invalOs = set()
    if(addObjNames == None):
        addObjNames = [o.name for o in bpy.context.scene.objects]
        invalOs = endPtMap.keys() - set(addObjNames) # In case of redo
    if(removeObjNames != None): invalOs.union(set(removeObjNames))
    for o in invalOs: del endPtMap[o]
    for objName in addObjNames:
        obj = bpy.context.scene.objects.get(objName)
        if(obj != None and cop.is_bezier(obj) and obj.visible_get()):
            endPtMap[objName] = []
            mw = obj.matrix_world
            for i, s in enumerate(obj.data.splines):
                pts = [mw @ pt.co for pt in s.bezier_points]
                endPtMap[objName].append([i, pts])
        elif(endPtMap.get(objName) != None): del endPtMap[objName]
    return endPtMap

def roundedVect(space3d, vect, rounding, axes): #Round to logarithmic scale .1, 0, 10, 100 etc. (47.538, -1) -> 47.5; (47.538, 0) -> 48.0; (47.538, 1) -> 50.0; (47.538, 2) -> 0, # TODO: Rework after grid subdiv is enabled (in a version later than 2.8)
    rounding += 1
    subdiv = getGridSubdiv(space3d)
    if(subdiv == 1): subdiv = 10 # TODO: Separate logic for 1
    fact = ((subdiv ** rounding) / subdiv) / getUnitScale()
    retVect = vect.copy()
    for i in axes: retVect[i] = round(vect[i] / fact) * fact # ~ Vector([round(vect[i] / fact) * fact for i in axes])
    return retVect
...
def  getViewDistRounding(space3d, rv3d): # TODO: Rework after grid subdiv is enabled (in a version later than 2.8)
    viewDist = rv3d.view_distance * getUnitScale()
    gridDiv = getGridSubdiv(space3d)
    subFact = 1
    if(gridDiv == 1): gridDiv = 10 # TODO: Separate logic for 1
    elif(gridDiv == 2): subFact = 5
    elif(viewDist < 0.5): subFact = 2
    return int(log(viewDist, gridDiv)) - subFact

def getClosestPlaneToView(rv3d): # Return axis-indices (x:0, y:1, z:2) of plane with closest orientation to view
    ...
    for idx, normal in enumerate(normals):
        ...
        if(rotDiff > pi / 2): rotDiff = pi - rotDiff
        if(rotDiff < minAngle):
            minIdx = idx
            minAngle = rotDiff
...
def getCoordFromLoc(region, rv3d, loc):
    ...
    return coord if(coord != None) else Vector((9000, 9000)) # return a unlocatable pt if None to avoid errors

def getCurrAreaRegion(context): # To be called only from 3d view
    a, r = [(a, r) for a in bpy.context.screen.areas if a.type == 'VIEW_3D' for r in a.regions if(r == context.region)][0]
    return a, r
...
def isOutside(context, event, exclInRgns = True):
    ...
    if(x < 0 or x > region.width or y < 0 or y > region.height): return True
    elif(not exclInRgns): return False
    area, r = getCurrAreaRegion(context)
    for r in area.regions:
        if(r == region): continue
        xR = r.x - region.x
        yR = r.y - region.y
        if(x >= xR and y >= yR and x <= (xR + r.width) and y <= (yR + r.height)): return True
    return False

def getPtProjOnPlane(region, rv3d, xy, p1, p2, p3, p4 = None):
    vec = region_2d_to_vector_3d(region, rv3d, xy)
    orig = region_2d_to_origin_3d(region, rv3d, xy)
    pt = geometry.intersect_ray_tri(p1, p2, p3, vec, orig, False)#p4 != None) # if(not pt and p4):  pt = geometry.intersect_ray_tri(p2, p4, p3, vec, orig, True)
    return pt

def getPtProjOnLine(region, rv3d, xy, p1, p2): # find the location on 3d line p1-p2 if xy is already on 2d projection (in rv3d) of p1-p2
    pd1 = p2 - p1 # Just find a non-linear point (TODO: simpler way)
    ...
    pd[maxIdx1] = pd2[0]
    p3 = p2 + pd
    return getPtProjOnPlane(region, rv3d, xy[:2], p1, p2, p3) # Raycast from 2d point onto the plane
    ...
    for i, a in enumerate(areas):
        ...
        for j, r in enumerate(regions):
            if(x > r.x and x < r.x + r.width and y > r.y and y < r.y + r.height):
                if(r.type == 'WINDOW'):
                    if(not exclInRgns): return [i, j]
                    idxs = [i, j]
                elif(exclInRgns): return None
    return idxs
...
def getAllAreaRegions():
    ...
    i = 0
    areas = [a for a in bpy.context.screen.areas if(a.type == 'VIEW_3D')]
    while(len(areas) == 0 and i < len(bpy.data.screens)): # bpy.context.screen doesn't work in case of Add-on Config window
        areas = [a for a in bpy.data.screens[i].areas if(a.type == 'VIEW_3D')]
        i += 1
    for a in areas:
        regions = [r for r in a.regions if r.type == 'WINDOW']
        if(len(a.spaces[0].region_quadviews) > 0):
            for i, r in enumerate(a.spaces[0].region_quadviews): info.append([a, regions[i], r])
        else:
            r = a.spaces[0].region_3d
...
def getFaceUnderMouse(obj, region, rv3d, xy, maxFaceCnt): # From python template
    if(obj == None or obj.type != 'MESH' or len(obj.data.polygons) > maxFaceCnt): return None, None, None
    ...
    if(success): return mw @ location, normal, faceIdx
    else: return None, None, None

def getSnappableObjs(region, rv3d, xy):
    objs = bpy.context.selected_objects
    if(bpy.context.object != None): objs.append(bpy.context.object)
    return [o for o in objs if(o.type == 'MESH' and len(o.modifiers) == 0 and isPtIn2dBBox(o, region, rv3d, xy))]

def get2dBBox(obj, region, rv3d, precise = False): # precise can be pretty expensive with large vert count
    mw = obj.matrix_world
    if(precise): co2ds = [getCoordFromLoc(region, rv3d, mw @ Vector(v.co)) for v in obj.data.vertices]
    else:        co2ds = [getCoordFromLoc(region, rv3d, mw @ Vector(b)) for b in obj.bound_box]
...
def isPtIn2dBBox(obj, region, rv3d, xy, extendBy = 0, precise = False):
    ...
    if(xy[0] > (minX - extendBy) and xy[0] < (maxX + extendBy) and xy[1] > (minY - extendBy) and xy[1] < (maxY + extendBy)): return True
    else: return False
...
def getClosestEdgeLoc2d(obj, region, rv3d, xy, faceIdx = None):
    ...
    vertPairs = obj.data.polygons[faceIdx].edge_keys if faceIdx != None else [e.vertices for e in obj.data.edges]
    for i, vertPair in enumerate(vertPairs):
        ...
        if(dist < minDist):
            ...
            edgeWSCos = [co0, co1]
            edgeIdx = i
    if(edgeWSCos != None): closestLoc = getPtProjOnLine(region, rv3d, closestIntersect, edgeWSCos[0], edgeWSCos[1])
    return edgeIdx, edgeWSCos, closestLoc, minDist

def getSelFaceLoc(region, rv3d, xy, maxFaceCnt, objs = None, checkEdge = False): # TODO: Fix the signature
    ...
    for obj in objs:
        ...
        if(loc != None):
            if(checkEdge):
                edgeIdx, edgeWSCos, closestLoc, minDist = getClosestEdgeLoc2d(obj, region, rv3d, xy, faceIdx)
                if(closestLoc != None and minDist < FTProps.snapDist): return obj, closestLoc, normal, faceIdx
            return obj, loc, normal, faceIdx
    return None, None, None, None

# def getSelEdgeLoc(region, rv3d, xy, maxFaceCnt, objs = None): # TODO: Fix the signature
    ...
    # if(bpy.context.object != None): objs.append(bpy.context.object)
    # for obj in objs:
        # edgeIdx, edgeWSCos, loc, dist = getClosestEdgeLoc2d(obj, region, rv3d, xy)
        # if(dist < minDist):
            # minDist = dist
            # closestLoc = loc
...
def splitCurveSelPts(selPtMap, newColl = True): # TODO: Update shapekey (not working due to moving of start pt in cyclic)
    changeCnt = 0
    newObjs = []
    if(len(selPtMap) == 0): return newObjs, changeCnt
    for obj in selPtMap.keys():
        splinePtMap = selPtMap.get(obj)
        if((len(obj.data.splines) == 1 and len(obj.data.splines[0].bezier_points) <= 2 and not obj.data.splines[0].use_cyclic_u) or len(splinePtMap) == 0): continue
        keyNames, keyData = getShapeKeyInfo(obj)
        collections = obj.users_collection
        if(newColl):
            objGrp = bpy.data.collections.new(obj.name)
            parentColls = [objGrp]
        else: parentColls = collections
        splineCnt = len(obj.data.splines)
        endSplineIdx = splineCnt- 1
        if(endSplineIdx not in splinePtMap.keys()): splinePtMap[endSplineIdx] = [len(obj.data.splines[endSplineIdx].bezier_points) - 1]
        ...
        for i in splineIdxs:
            for j in range(lastSplineIdx + 1, i):
                ...
                createSpline(objCopy.data, srcSpline) # updateShapeKeyData(objCopy, keyData, keyNames, skStart, ptCnt)
            srcSpline = obj.data.splines[i]
            selPtIdxs = sorted(splinePtMap[i])
            if(len(selPtIdxs) == 0): createSpline(objCopy.data, srcSpline)
            else:
                ...
                if(len(selPtIdxs) > 0 and selPtIdxs[0] == 0): selPtIdxs.pop(0)
                if(len(selPtIdxs) > 0 and selPtIdxs[-1] == len(bpts) - 1): selPtIdxs.pop(-1)
                bpts = srcSpline.bezier_points
                if(len(selPtIdxs) == 0):
                    ...
                    for j in selPtIdxs:
                        ...
                        createSplineForSeg(objCopy.data, segBpts) # updateShapeKeyData(objCopy, keyData, keyNames, len(newObjs), 2)
                        objCopy = createSkeletalCurve(obj, parentColls)
                        newObjs.append(objCopy)
                        lastSegIdx = j
        ...
        if(len(objCopy.data.splines) == 0):
            ...
            oop.safe_remove(objCopy)
        if(newColl):
            for collection in collections: collection.children.link(objGrp)
        oop.safe_remove(obj)
        changeCnt += 1
    for obj in newObjs: obj.data.splines.active = obj.data.splines[0]
    return newObjs, changeCnt

def splitCurve(selObjs, split, newColl = True): #split value is one of {'spline', 'seg', 'point'} (TODO: Enum)
    changeCnt = 0
    newObjs = []
    if(len(selObjs) == 0): return newObjs, changeCnt
    for obj in selObjs:
        if(not cop.is_bezier(obj) or len(obj.data.splines) == 0): continue
        ...
        for i, spline in enumerate(obj.data.splines):
            if(split == 'seg' or split == 'point'):
                ...
                if(split == 'seg'): ptLen -= 1
                for j in range(0, ptLen):
                    objCopy = createSkeletalCurve(obj, parentColls)
                    if(split == 'seg'):
                        createSplineForSeg(objCopy.data, spline.bezier_points[j:j+2])
                        ...
                        copyObjAttr(spline.bezier_points[j], newSpline.bezier_points[0], newWM.inverted_safe(), mw)
                        removeShapeKeys(objCopy) # No point having shapekeys (pun intended :)
                    newObjs.append(objCopy)
                if(split == 'seg' and spline.use_cyclic_u):
                    objCopy = createSkeletalCurve(obj, parentColls)
                    createSplineForSeg(objCopy.data, [spline.bezier_points[-1], spline.bezier_points[0]])
...
def getCurvesArrangedByDist(curves):
    ...
    while(len(remainingCurves) > 0): #Arrange in order
        ...
        if(len(orderedCurves) == 1): #Check the start also for the first curve
...
def joinSegs(curves, optimized, straight, srcCurve = None, margin = DEF_ERR_MARGIN):
    ...
    if(len(curves) == 1): return curves[0]
    if(optimized): curves = getCurvesArrangedByDist(curves)
    firstCurve = curves[0]
    if(srcCurve == None): srcCurve = firstCurve
    ...
    for curve in curves[1:]:
        if(curve == srcCurve): curveData = srcCurveData
        else: curveData = curve.data
        mw = curve.matrix_world
        ...
        if(vectCmpWithMargin(srcMW @ currBezierPt.co, mw @ nextBezierPt.co, margin)): #Don't add new point if the last one and the current one are the 'same'
            currBezierPt.handle_right_type = nextBezierPt.handle_right_type
            if(currBezierPt.handle_right_type != 'VECTOR'): currBezierPt.handle_right_type = 'FREE'
            currBezierPt.handle_right = invSrcMW @ (mw @ nextBezierPt.handle_right)
            ptIdx = 1
        else: ptIdx = 0
        if(straight and ptIdx == 0):
            currBezierPt.handle_left_type = 'FREE'
            currBezierPt.handle_right_type = 'VECTOR' # currBezierPt.handle_right = currBezierPt.co
        for i in range(ptIdx, len(nextSpline.bezier_points)):
            if((i == len(nextSpline.bezier_points) - 1) and
                vectCmpWithMargin(mw @ nextSpline.bezier_points[i].co, srcMW @ currSpline.bezier_points[0].co, margin)):
            ...
            if(straight and i ==  0):
                ...
                currBezierPt.handle_left_type = 'VECTOR' # currBezierPt.handle_left = currBezierPt.co
        for spline in curveData.splines[1:]: #Simply add the remaining splines
            newSpline = newCurveData.splines.new('BEZIER')
            copyObjAttr(spline, newSpline)
            for i, pt in enumerate(spline.bezier_points):
                if(i > 0): newSpline.bezier_points.add(1)
                copyObjAttr(pt, newSpline.bezier_points[-1], invSrcMW, mw)
        if(curve != srcCurve): oop.safe_remove(curve)
    if(firstCurve != srcCurve): oop.safe_remove(firstCurve)
    return srcCurve
    ...
    for spline in curve.data.splines:
        ...
        for pt in cmpPts:
            if(prevPt != None and vectCmpWithMargin(prevPt.co, pt.co, margin)):
                ...
                copyObjAttr(pt, currSpline.bezier_points[-1])
            prevPt = pt
    if(dupliFound): curve.data = newCurveData
    else: bpy.data.curves.remove(newCurveData)
...
def convertToFace(curve, remeshRes, perSeg, fillType, optimized):
    ...
    for splineIdx, spline in enumerate(curve.data.splines):
        ...
        if(not perSeg and remeshRes > 0):
            ...
            if(numSegs <= 2): vertCos = [bpts[0].co, bpts[-1].co]
            else:
                pts = getInterpBezierPts(segPts, subdivPerUnit = 100, segLens = None)
                vertCos = getInterpolatedVertsCo(pts, numSegs)
            for co in vertCos: verts.append(bm.verts.new(co))
        else:
            if(remeshRes > 0):
                segPtPairs = [getSegPtsInSpline(bptData, splineIdx, ptIdx, addLastVert) for ptIdx in range(len(bpts))]
                if(not addLastVert): segPtPairs.pop()
                segs = [[segPts[0][1], segPts[0][2], segPts[1][0], segPts[1][1]] for segPts in segPtPairs]
                ...
                for i, segPts in enumerate(segPtPairs):
                    if(optimized and isStraightSeg(segPts)):
                        ...
                        pts = getInterpBezierPts(segPts, subdivPerUnit = 100, segLens = [segLens[i]])
                ...
                for ptIdx, bpt in enumerate(bpts): verts.append(bm.verts.new(bpts[ptIdx].co))
                if(addLastVert): verts.append(bm.verts.new(bpts[0].co))
        if(len(verts) < 2): pass
        elif(len(verts) == 2): bm.edges.new(verts)
        else:
            if(spline.use_cyclic_u):
                bm.verts.remove(verts[-1])
                verts.pop()
            vertCos = [v.co for v in verts]
            center =  Vector([sum(vertCos[i][j] for i in range(len(vertCos))) for j in range(3)]) / len(vertCos)
            normal = geometry.normal(vertCos)
            centers.append(center)
            normals.append(normal)
            if(fillType == 'NGON'): bm.faces.new(verts)
            elif(fillType == 'NONE'):
                for i in range(1, len(verts)): bm.edges.new([verts[i-1], verts[i]])
                if(spline.use_cyclic_u): bm.edges.new([verts[-1], verts[0]])
            elif(fillType == 'FAN'):
                centerVert = bm.verts.new(center)
                for i in range(1, len(verts)): bm.faces.new([centerVert, verts[i-1], verts[i]])
                if(spline.use_cyclic_u): bm.faces.new([centerVert, verts[-1], verts[0]])
    cnt = len(centers)
    if(cnt > 0):
        center =  Vector([sum(centers[i][j] for i in range(cnt)) for j in range(3)]) / cnt
        normal =  Vector([sum(normals[i][j] for i in range(cnt)) for j in range(3)]) / cnt
    else:
        center =  None
        normal =  None
    ...
    for c in collections: c.objects.link(meshObj)
...
def applyMeshModifiers(meshObj, remeshDepth):
    ...
    if(vectCmpWithMargin(normal, Vector())): return meshObj
    ...
    for i, v in enumerate(bm.verts):
        ...
        if (abs(geometry.distance_point_to_plane(co, planeVert, normal)) > DEF_ERR_MARGIN): toRemove.append(v)
    for v in toRemove: bm.verts.remove(v)
...
def intersectCurves(curves, action, firstActive, margin, rounding):
    allIntersectCos, intersectMap = getCurveIntersectPts(curves, firstActive, margin, rounding)
    if(action in {'MARK_EMPTY', 'MARK_POINT'}):
        ...
        for co in allIntersectCos:
            if(action  == 'MARK_EMPTY'): obj = bpy.data.objects.new(objName, None)
            ...
            newObjs.append(obj)
            collection.objects.link(obj)
        for obj in newObjs: obj.select_set(True)
    if(action in {'INSERT_PT', 'CUT'}):
        ...
        for i, key in enumerate(mapKeys):
            ...
            if(prevCurveIdx == curveIdx and prevSplineIdx == splineIdx): segIdx += prevCnt
            else: prevCnt = 0
            curve = curves[curveIdx]
            if(firstActive and curve == curves[0]): continue
            mw = curve.matrix_world
            pts = curve.data.splines[splineIdx].bezier_points
            nextIdx = getAdjIdx(curve, splineIdx, segIdx)
            seg = [mw @ pts[segIdx].co, mw @ pts[segIdx].handle_right, mw @ pts[nextIdx].handle_left, mw @ pts[nextIdx].co]
            ...
            for j, co in enumerate(insertCos): insertCos[j] = mw.inverted_safe() @ co
            ...
            if(action == 'CUT'):
                if(selPtMap.get(curve) == None): selPtMap[curve] = {}
                if(selPtMap[curve].get(splineIdx) == None): selPtMap[curve][splineIdx] = []
                selPtMap[curve][splineIdx] += list(range(segIdx + startIdxIncr, segIdx + len(sortedCos) + endIdxIncr))
        if(action == 'CUT'):
            for curve in list(selPtMap.keys()):
                splineIdxs = sorted(selPtMap[curve].keys())
                if(len(curve.data.splines) > 1):
                    newObjs, changeCnt = splitCurve([curve], 'spline', curve.users_collection)
                    ...
                    for idx in splineIdxs:
                        ...
                        if(len(selPtMap[curve]) == 0): selPtMap.pop(curve)
...
def getPathD(path):
    ...
    for i, part in enumerate(path):
        ...
        for j, segment in enumerate(part):
            if(j == 0): comps.append('M {},{} C'.format(segment[0].real, segment[0].imag))
            args = (segment[1].real, segment[1].imag, segment[2].real, segment[2].imag, segment[3].real, segment[3].imag)
...
def getPathBBox(path):
    ...
    for part in path:
        for seg in part:
            ...
            if(minX == None or leftBotFront[0] < minX): minX = leftBotFront[0]
            if(minY == None or leftBotFront[1] < minY): minY = leftBotFront[1]
            if(maxX == None or rgtTopBack[0] > maxX):   maxX = rgtTopBack[0]
            if(maxY == None or rgtTopBack[1] > maxY):   maxY = rgtTopBack[1]
    return minX, minY, maxX, maxY
...
def getSVGPathElem(doc, docW, docH, path, idx, lineWidth, lineCol, lineAlpha, fillCol, fillAlpha, clipView, clipElemId):
    idPrefix = 'id'
    style= {'opacity':'1', 'stroke':'#000000', 'stroke-width':'1',  'fill':'none', 'stroke-linecap':'round', 'stroke-linejoin':'miter',  'stroke-miterlimit':'4'}
    clipped = False
    if(clipView):
        minX, minY, maxX, maxY = getPathBBox(path)
        if(maxX < 0 or maxY < 0 or minX > docW or minY > docH): return None
        if(minX < 0 or minY < 0 or maxX > docW or maxY > docH): clipped = True
    ...
    if(clipped): elem.setAttribute('clip-path', 'url(#' + clipElemId + ')')
    return elem
...
def exportSVG(context, filepath, exportView, clipView, lineWidth, lineColorOpts, \
    ...
    if(lineColorOpts == 'PICK'): lineCol, lineAlpha = toHexStr(lineColor)
    if(fillColorOpts == 'PICK'): fillCol, fillAlpha = toHexStr(fillColor)
    if exportView == 'ACTIVE_VIEW':
        area = context.area
        if(area.type != 'VIEW_3D'): area = [a for a in bpy.context.screen.areas if  a.type == 'VIEW_3D'][0]
        region = [r for r in area.regions if r.type == 'WINDOW'][0]
        space3d = area.spaces[0]
        if(len(space3d.region_quadviews) > 0): rv3d = space3d.region_quadviews[3]
        else: rv3d = space3d.region_3d
        camera = None
        docW = region.width
        docH = region.height
    else:
    ...
    if(clipView): createClipElem(doc, svgElem, docW, docH, clipElemId)
    idx = 0
    for o in bpy.context.scene.objects:
        mw = o.matrix_world
        if(cop.is_bezier(o) and o.visible_get()):
            path = []
            filledPath = []
            for spline in o.data.splines:
                ...
                if(spline.use_cyclic_u):
                    ...
                    part.append([getSVGPt(mw @ co, docW, docH, camera, region, rv3d) for co in seg])
                if(len(part) > 0):
                    if (spline.use_cyclic_u and o.data.dimensions == '2D' and o.data.fill_mode != 'NONE'): filledPath.append(part)
                    else: path.append(part)
            for p in [path, filledPath]:
                ...
                if(p == path): fc, fa = None, None
                elif(fillColorOpts == 'RANDOM'):
                    fillColor = [random.random() for i in range(3)] + [1]
                    fc, fa = toHexStr(fillColor)
                else: fc, fa = fillCol, fillAlpha
                svgPathElem = getSVGPathElem(doc, docW, docH, p, idx, lineWidth, lineCol, lineAlpha, fc, fa, clipView, clipElemId)
                if(svgPathElem != None):
...
class SeparateSplinesObjsOp(Operator):
    ...
    def execute(self, context):
        ...
        if(changeCnt > 0): self.report({'INFO'}, "Separated "+ str(changeCnt) + " curve object" + ("s" if(changeCnt > 1) else "") + " into " +str(len(newObjs)) + " new ones")
        return {'FINISHED'}
...
class SplitBezierObjsOp(Operator):
    ...
    def execute(self, context):
        selObjs = [o for o in bpy.context.selected_objects if(cop.is_bezier(o))]
        ...
        else: newObjs, changeCnt = splitCurve(selObjs, split = 'seg')
        if(changeCnt > 0):
            bpy.context.view_layer.objects.active = newObjs[-1]
            self.report({'INFO'}, "Split "+ str(changeCnt) + " curve object" + ("s" if(changeCnt > 1) else "") + " into " + str(len(newObjs)) + " new objects")
        return {'FINISHED'}
...
class JoinBezierSegsOp(Operator):
    ...
    def execute(self, context):
        curves = [o for o in bpy.data.objects if o in bpy.context.selected_objects and cop.is_bezier(o)]
        ...
        newCurve = joinSegs(curves, optimized = optimized, straight = straight, margin = mergeDist) # removeShapeKeys(newCurve)
    ...
    def execute(self, context):
        if(bpy.context.active_object != None):
            ...
            for collection in collections:
                for o in collection.objects: o.select_set(not o.select_get())
        return {'FINISHED'}
...
class OpenSplinesOp(Operator):
    ...
    def execute(self, context):
        curves = [o for o in bpy.data.objects if o in bpy.context.selected_objects and cop.is_bezier(o)]
        for curve in curves:
            for spline in curve.data.splines: spline.use_cyclic_u = False
        return {'FINISHED'}
...
class RemoveDupliVertCurveOp(Operator):
    ...
    def execute(self, context):
        curves = [o for o in bpy.data.objects if o in bpy.context.selected_objects and cop.is_bezier(o)]
        for curve in curves: removeDupliVert(curve, bpy.context.window_manager.bezierToolkitParams.dupliVertMargin)
        return {'FINISHED'}
...
class convertToMeshOp(Operator):
    ...
    def execute(self, context):
        ...
        for curve in curves:
            ...
            if(fillType == 'QUAD'):
                for spline in curve.data.splines: spline.use_cyclic_u = True
                curve.data.dimensions = '2D'
                curve.data.fill_mode = 'BOTH'
                meshObj = convertToMesh(curve)
                applyMeshModifiers(meshObj, remeshDepth)
                if(unsubdivide): unsubdivideObj(meshObj)
            else: meshObj, center, normal = convertToFace(curve, remeshRes, perSeg, fillType, optimized)
...
class AlignToFaceOp(Operator):
    ...
    bl_description = "Align all points of selected curves with nearest face of selected meshes"
    bl_options = {'REGISTER', 'UNDO'}
    def execute(self, context):
        ...
        curves = [o for o in bpy.data.objects if o in bpy.context.selected_objects and cop.is_bezier(o) and o.visible_get()]
        if(len(curves) == 0): return {'FINISHED'}
        mesheObjs = [o for o in bpy.data.objects if o in bpy.context.selected_objects and  o.type == 'MESH' and o.visible_get()]
        if(len(mesheObjs) == 0): return {'FINISHED'}
        depsgraph = bpy.context.evaluated_depsgraph_get()
        medianLists = []
        for o in mesheObjs: medianLists.append([o.matrix_world @ f.center for f in o.data.polygons])
...
class SetCurveColorOp(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        for curve in curves: curve.data['curveColor'] = bpy.context.window_manager.bezierToolkitParams.curveColorPick
        return {'FINISHED'}
...
class RemoveCurveColorOp(bpy.types.Operator):
    ...
    def execute(self, context):
        curves = bpy.context.selected_objects
        for curve in curves:
            if(curve.data.get('curveColor')): del curve.data['curveColor']
        return {'FINISHED'}
...
class PasteLengthOp(Operator):
    ...
    def execute(self, context):
        ...
        if(src != None and cop.is_bezier(src)):
            dests = [o for o in bpy.context.selected_objects if(cop.is_bezier(o) and o != src)]
            if(len(dests) > 0): pasteLength(src, dests)
        return {'FINISHED'}
...
class IntersectCurvesOp(Operator):
    ...
    def execute(self, context):
        ...
        if(params.intersectNonactive):
            if(not cop.is_bezier(actCurve)):
                self.report({'WARNING'}, "Active Object Not A Curve")
                return {'FINISHED'}
        curves = [o for o in bpy.data.objects if o in bpy.context.selected_objects and cop.is_bezier(o) and o != actCurve]
        if(actCurve != None and cop.is_bezier(actCurve)): curves = [actCurve] + curves
        if(len(curves) < 2): self.report({'INFO'}, "Please select at least two Bezier curve objects")
        else: intersectCurves(curves, params.intersectOp, params.intersectNonactive, params.intersectMargin, rounding)
        return {'FINISHED'}
...
class ExportSVGOp(Operator):
    ...
    def getExportViewList(scene = None, context = None):
        ...
        for c in cameras: vlist.append((c.name, c.name, 'Export view from ' + c.name))
        return vlist
    filepath : StringProperty(subtype='FILE_PATH')
    clipView : BoolProperty(name="Clip View", description = "Clip objects to view boundary", default = True) #User input
    exportView: EnumProperty(name = 'Export View', items = getExportViewList, description='View to export')
    lineWidth: FloatProperty(name="Line Width", description='Line width in exported SVG', default = 3, min = 0)
    lineColorOpts: EnumProperty(name = 'Line Color', items = (('RANDOM', 'Random', 'Use random color for curves'), ('PICK', 'Pick', 'Pick color'), ), description='Color to draw curve lines')
    lineColor: bpy.props.FloatVectorProperty( name="Line Color", subtype="COLOR", size=4, min=0.0, max=1.0, default=(0.5, 0.5, 0.5, 1.0))
    fillColorOpts: EnumProperty(name = 'Fill Color', items = (('RANDOM', 'Random', 'Use random fill color'), ('PICK', 'Pick', 'Pick color'), ), description='Color to fill solid curves')
    fillColor: bpy.props.FloatVectorProperty( name="Fill Color", subtype="COLOR", size=4, min=0.0, max=1.0, default=(0, 0.3, 0.5, 1.0))

    def execute(self, context):
        exportSVG(context, self.filepath, self.exportView, self.clipView, self.lineWidth, self.lineColorOpts, self.lineColor, self.fillColorOpts, self.fillColor)
        return {'FINISHED'}
...
def markVertHandler(self, context):
    if(self.markVertex): bpy.ops.wm.bb_mark_vertex()
...
class MarkerController:
    ...
    def createSMMap(self, context):
        ...
        smMap = {}
        for curve in objs:
            if(not cop.is_bezier(curve)): continue
            smMap[curve.name] = {}
            mw = curve.matrix_world
            for splineIdx, spline in enumerate(curve.data.splines):
                if(not spline.use_cyclic_u): continue
                smMap[curve.name][splineIdx] = [mw @ curve.data.splines[splineIdx].bezier_points[0].co, 0] #initialize to the curr start vert co and idx
                for pt in spline.bezier_points: pt.select_control_point = False
            if(len(smMap[curve.name]) == 0): del smMap[curve.name]
        return smMap
    ...
    def createBatch(self, context):
        ...
        self.batch = batch_for_shader(self.shader, "POINTS", {"pos": positions, "color": colors})
    ...
    def removeMarkers(self, context):
        if(MarkerController.drawHandlerRef != None):
            ...
            if(context.area and hasattr(context.space_data, 'region_3d')): context.area.tag_redraw()
            MarkerController.drawHandlerRef = None
        self.deselectAll()

    def __init__(self, context):
        self.smMap = self.createSMMap(context)
        self.shader = gpu.shader.from_builtin('3D_FLAT_COLOR') # self.shader.bind()
        try: MarkerController.defPointSize = context.preferences.addons[__name__].preferences.markerSize
        except Exception as e: MarkerController.defPointSize = 6 # print("BezierUtils: Fetching marker size", e)
        MarkerController.drawHandlerRef = bpy.types.SpaceView3D.draw_handler_add(self.drawHandler, (), "WINDOW", "POST_VIEW")
    ...
    def updateSMMap(self):
        for curveName in self.smMap.keys():
            ...
            for splineIdx in spMap.keys():
                ...
                loc, idx = markerInfo[0], markerInfo[1]
                pts = curve.data.splines[splineIdx].bezier_points
                selIdxs = [x for x in range(0, len(pts)) if pts[x].select_control_point == True]
    ...
    def resetShowHandleState(context, handleStates):
        ...
        for i, s in enumerate(spaces):
            if(hasattr(s.overlay, 'show_curve_handles')): s.overlay.show_curve_handles = handleStates[i]
            elif(hasattr(s.overlay, 'display_handle')):   s.overlay.display_handle = handleStates[i] # 2.90
...
class ModalMarkSegStartOp(bpy.types.Operator):
    ...
    def modal (self, context, event):
        if(context.mode  == 'OBJECT' or event.type == "ESC" or not bpy.context.window_manager.bezierToolkitParams.markVertex):
    ...
    def execute(self, context):
        self._timer = context.window_manager.event_timer_add(time_step = 0.0001, window = context.window) #TODO: Why such small step?
        context.window_manager.modal_handler_add(self)
        self.markerState = MarkerController(context)
        self.handleStates = MarkerController.hideHandles(context) #Hide so that users don't accidentally select handles instead of points
        return {"RUNNING_MODAL"}
...
class BezierUtilsPanel(Panel):
    ...
    def draw(self, context):
        ...
        if(context.mode == 'OBJECT'):
            row = layout.row()
            row.prop(params, "intersectExpanded", icon="TRIA_DOWN" if params.intersectExpanded else "TRIA_RIGHT", icon_only=True, emboss=False )
            ...
            row = layout.row()
            row.prop(params, "splitExpanded", icon="TRIA_DOWN" if params.splitExpanded else "TRIA_RIGHT", icon_only=True, emboss=False)
            ...
            row = layout.row()
            row.prop(params, "joinExpanded", icon="TRIA_DOWN" if params.joinExpanded else "TRIA_RIGHT", icon_only=True, emboss=False )
            ...
            row = layout.row()
            row.prop(params, "alignToFaceExpanded", icon="TRIA_DOWN" if params.alignToFaceExpanded else "TRIA_RIGHT", icon_only=True, emboss=False )
            ...
            row = layout.row()
            row.prop(params, "selectExpanded", icon="TRIA_DOWN" if params.selectExpanded else "TRIA_RIGHT", icon_only=True, emboss=False )
            ...
            row = layout.row()
            row.prop(params, "convertExpanded", icon="TRIA_DOWN" if params.convertExpanded else "TRIA_RIGHT", icon_only=True, emboss=False )
            ...
            row = layout.row()
            row.prop(params, "handleTypesExpanded", icon="TRIA_DOWN" if params.handleTypesExpanded else "TRIA_RIGHT", icon_only=True, emboss=False )
            ...
            row = layout.row()
            row.prop(params, "removeDupliExpanded", icon="TRIA_DOWN" if params.removeDupliExpanded else "TRIA_RIGHT", icon_only=True, emboss=False )
            ...
            ######## Curve Color #########
            row = layout.row()
            row.prop(params, "curveColorExpanded", icon="TRIA_DOWN" if params.curveColorExpanded else "TRIA_RIGHT", icon_only=True, emboss=False )
            ...
            ######## Other Tools #########
            row = layout.row()
            row.prop(params, "otherExpanded", icon="TRIA_DOWN" if params.otherExpanded else "TRIA_RIGHT", icon_only=True, emboss=False)
            ...
            if(tool.idname == FlexiDrawBezierTool.bl_idname and params.drawObjType == 'MATH'):
                row = layout.row()
                row.prop(params, "mathExtraExpanded", icon="TRIA_DOWN" if params.mathExtraExpanded else "TRIA_RIGHT", icon_only=True, emboss=False )
                row.label(text='Flexi Draw Math Function', icon='GRAPH')
                if params.mathExtraExpanded:
                    box = layout.box() # Equation params are duplicated in the toolbar...  any changes here should also reflect there
                    col = box.column().split()
                    ...
                    for i in range(Primitive2DDraw.getParamCnt()):
                        ...
                        row = col.row()
                        row.label(text = char)
                        row.prop(params, MathFnDraw.startPrefix + str(i), text = '') # Value
                        row.prop(params, MathFnDraw.incrPrefix + str(i), text = '') # Step
                    col = box.column().split()
                    row = col.row()
    ...
    def colorCurves(scene = None, add = False, remove = False):
        def ccDrawHandler():
            if(bpy.context.window_manager.bezierToolkitParams.applyCurveColor):
                ...
                if(BezierUtilsPanel.lineBatch != None): BezierUtilsPanel.lineBatch.draw(BezierUtilsPanel.shader)
        if(add and BezierUtilsPanel.drawHandlerRef == None):
            try: BezierUtilsPanel.lineWidth = bpy.context.preferences.addons[__name__].preferences.lineWidth
            except Exception as e:
                print("BezierUtils: Error fetching line width in ColorCurves: ", e)
                BezierUtilsPanel.lineWidth = 1.5
            BezierUtilsPanel.drawHandlerRef = bpy.types.SpaceView3D.draw_handler_add(ccDrawHandler, (), "WINDOW", "POST_VIEW")
            BezierUtilsPanel.shader = gpu.shader.from_builtin('3D_FLAT_COLOR')
            return
        elif(remove):
            if(BezierUtilsPanel.drawHandlerRef != None):
                bpy.types.SpaceView3D.draw_handler_remove(BezierUtilsPanel.drawHandlerRef, "WINDOW")
                BezierUtilsPanel.drawHandlerRef = None
                return
        if(bpy.context.screen == None): return
        if(bpy.context.window_manager.bezierToolkitParams.applyCurveColor):
            objs = [o for o in bpy.context.scene.objects if(cop.is_bezier(o) and o.visible_get() and len(o.modifiers) == 0 and not o.select_get())]
            lineCos = []
            lineColors = []
            for o in objs:
                ...
                if(colorVal != None):
                    for i, spline in enumerate(o.data.splines):
                        for j in range(0, len(spline.bezier_points)):
                            segPts = getBezierDataForSeg(o, i, j, withShapeKey = True, shapeKeyIdx = None, fromMix = True)
                            if(segPts == None): continue
                            pts = getPtsAlongBezier2D(segPts, getAllAreaRegions(), FTProps.dispCurveRes, maxRes = MAX_NONSEL_CURVE_RES)
                            linePts = getLinesFromPts(pts)
                            lineCos += linePts
                            lineColors += [colorVal for i in range(0, len(linePts))]
            BezierUtilsPanel.lineBatch = batch_for_shader(BezierUtilsPanel.shader, "LINES", {"pos": lineCos, "color": lineColors})
        # else: BezierUtilsPanel.lineBatch = batch_for_shader(BezierUtilsPanel.shader, "LINES", {"pos": [], "color": []})
            areas = [a for a in bpy.context.screen.areas if a.type == 'VIEW_3D']
            for a in areas: a.tag_redraw()

################### Common Bezier Functions & Classes ###################
def getPtFromT(p0, p1, p2, p3, t):
    c = (1 - t)
    pt = (c ** 3) * p0 + 3 * (c ** 2) * t * p1 + 3 * c * (t ** 2) * p2 + (t ** 3) * p3
    return pt

def getTangentAtT(p0, p1, p2, p3, t):
    c = (1 - t)
    tangent = -3 * (c * c) * p0 + 3 * c * c * p1 - 6 * t * c * p1 - 3 * t * t * p2 + 6 * t * c * p2 + 3 * t * t * p3
    return tangent

def getTsForPt(p0, p1, p2, p3, co, coIdx, tolerance = 0.000001, maxItr = 1000): # iterative brute force, not optimized, some iterations maybe redundant
    ts = set()
    for T in [1., 0.]: # check t from start to end and end to start
        for dirn in [1, -1]: # check clockwise as well as anticlockwise
            t = T
            t2 = 1
            ...
            while(abs(error) > tolerance and i < maxItr):
                t2 /= 2
                if(dirn * error < 0): t += t2
                else: t -= t2
                rhs = getPtFromT(p0, p1, p2, p3, t)[coIdx]
                error = rhs - co
                i += 1
            if(i < maxItr and t >= 0 and t <= 1): ts.add(round(t, 3))
    return ts

def getTForPt(curve, testPt, tolerance = .000001): #TODO: There may be a more efficient approach, but this seems foolproof
    minLen = LARGE_NO
    retT = None
    for coIdx in range(0, 3):
        ts = getTsForPt(curve[0], curve[1], curve[2], curve[3], testPt[coIdx], coIdx, tolerance)
        for t in ts:
...
def getCosSortedByT(seg, cos, margin):
    ...
    for co in cos:
        t = getTForPt(seg, co, margin)
        if(t >= 1): coInfo.add(((seg[3]).freeze(), 1)) # if(all(abs(co[i] - seg[3][i]) < margin for i in range(3)) or t >= 1):
        elif(t <= 0): coInfo.add(((seg[0]).freeze(), 0)) # elif(all(abs(co[i] - seg[0][i]) < margin for i in range(3)) or t <= 0):
        else: coInfo.add((co.freeze(), t))
    return [inf[0] for inf in sorted(coInfo, key = lambda x: x[1])]

def removeDupliCos(sortedCos, margin): # Currently inserting points more than once if intersect is invoked repeatedly # TODO: Check for initial and end points of the segment (here or in getCosSortedByT)
    prevCo = sortedCos[0]
    newCos = [prevCo]
    for i in range(1, len(sortedCos)):
        co = sortedCos[i]
        if(not vectCmpWithMargin(co, prevCo, margin)): newCos.append(co)
        prevCo = co
    return newCos

def getBBox(seg): #(3 D - 9 C + 9 B - 3 A) t^2 + (6 A - 12 B + 6 C) t + 3 (B - A) # https://stackoverflow.com/questions/24809978/calculating-the-bounding-box-of-cubic-bezier-curve
    A = seg[0]
    B = seg[1]
    C = seg[2]
    D = seg[3]
    ...
    for i in range(0, 3):
        solns = []
        if(a[i] == 0):
            if(b[i] == 0): solns.append(0)#Independent of t so lets take the starting pt
            else: solns.append(c[i] / b[i])
        else:
            rootFact = b[i] * b[i] - 4 * a[i] * c[i]
            if(rootFact >=0 ): #Two solutions with + and - sqrt
...
def getBBoxOverlapInfo(seg0, seg1):
    ...
    bbox1 = getBBox(seg1)
    overlap = True
    if(any(all(bbox1[i][j] < min0[j] for i in range(2)) for j in range(3)) or any(all(bbox1[i][j] > max0[j] for i in range(2)) for j in range(3))): overlap = False
...
def getIntersectPts(seg0, seg1, soln, solnRounded, recurs, margin, rounding, \
    ...
    if(overlap):
        ...
        if(all(abs(bbox0[0][i] - bbox0[1][i]) < margin for i in range(3))):
            ...
            r0 = getIntersectPts(seg01, seg11, soln, solnRounded, recurs + 1, margin, rounding)
            r1 = getIntersectPts(seg01, seg12, soln, solnRounded, recurs + 1, margin, rounding)
            r2 = getIntersectPts(seg02, seg11, soln, solnRounded, recurs + 1, margin, rounding)
            r3 = getIntersectPts(seg02, seg12, soln, solnRounded, recurs + 1, margin, rounding)
            return any((r0, r1, r2, r3))
    return False

def getSplineIntersectPts(curves, splineInfos, firstActive, margin, rounding): # splineInfos: [(curveIdx0, splineIdx0), (curveIdx0, splineIdx1),...] # First active means intersections only with the first curve (otherwise all combinations)
    segPairMap = {}
    areas = [a for a in bpy.context.screen.areas if  a.type == 'VIEW_3D']
    for i, c0Info in enumerate(splineInfos):
        idxCurve0, idxSpline0 = c0Info
        if(firstActive and idxCurve0 != splineInfos[0][0]): break
        c0 = curves[idxCurve0]
        ...
        for j in range(i + 1, len(splineInfos)):
            ...
            segPairMap[((idxCurve0, idxSpline0), (idxCurve1, idxSpline1))] = [((idxSeg0, s0), (idxSeg1, s1)) for idxSeg0, s0 in enumerate(getRoundedSplineSegs(c0.matrix_world, spline0)) for idxSeg1, s1 in enumerate(getRoundedSplineSegs(c1.matrix_world, spline1))]
    intersectMap = {}
    allIntersectCos = []
    for key in segPairMap:
        ...
        for info in segPairInfo:
            ...
            ret = getIntersectPts(seg0, seg1, soln, solnRounded, recurs = 0, margin = margin, rounding = rounding)
            if(ret):
                extKey = (idxCurve0, idxSpline0, idxSeg0)
                if(intersectMap.get(extKey) == None): intersectMap[extKey] = []
...
def getCurveIntersectPts(curves, firstActive, margin, rounding):
    splineInfos = [(x, y) for x in range(len(curves)) for y in range(len(curves[x].data.splines))]
...
def getSegLenTmpObj(tmpSpline, bpts, mw = Matrix()): # Because there is some discrepancy between this and getSegLen This seems to be more accurate
...
def getSegLen(pts, error = DEF_ERR_MARGIN, start = None, end = None, t1 = 0, t2 = 1):
    ...
    if (l2 - l > error): return (getSegLen(pts, error, start, mid, t1, t1_5) + getSegLen(pts, error, mid, end, t1_5, t2))
    return l2

def hasAlignedHandles(pt):
    if(len(pt) == 5 and 'ALIGNED' in {pt[3], pt[4]} and 'FREE' not in {pt[3], pt[4]}): return True
    diffV1 = pt[1] - pt[0]
    diffV2 = pt[2] - pt[1]
    if(vectCmpWithMargin(diffV1.normalized(), diffV2.normalized())): return True
    return False

def isStraightSeg(segPts):
    if(len(segPts) != 2): return False
    if((len(segPts[0]) == 5 or len(segPts[1]) == 5) and segPts[0][4] == 'VECTOR' and segPts[1][3] == 'VECTOR'): return True
    if(vectCmpWithMargin((segPts[0][2]-segPts[0][1]).normalized(), (segPts[1][1] - segPts[1][0]).normalized())): return True
    return False

def getInterpBezierPts(segPts, subdivPerUnit, segLens = None, maxRes = None): # Get pt coords along curve defined by the four control pts (segPts). subdivPerUnit: No of subdivisions per unit length (which is the same as no of pts excluding the end pts)
    if(len(segPts) < 2): return []
    curvePts = []
    for i in range(1, len(segPts)):
        seg = [segPts[i-1][1], segPts[i-1][2], segPts[i][0], segPts[i][1]]
        if(segLens != None and len(segLens) > (i-1)): res = int(segLens[i-1] * subdivPerUnit)
        else: res = int(getSegLen(seg) * subdivPerUnit)
        if(res < 2): res = 2
        if(maxRes != None and res > maxRes): res = maxRes
        curvePts += geometry.interpolate_bezier(*seg, res)
    return curvePts

def getPtsAlongBezier3D(segPts, rv3d, curveRes, minRes = 200): # Used in functions where actual locs of pts on curve matter (like subdiv Bezier) (... kind of expensive)
    viewDist = rv3d.view_distance
    curveRes = curveRes / viewDist # The smaller the view dist (higher zoom level), the higher the num of subdivisions
    if(curveRes < minRes): curveRes = minRes
    return getInterpBezierPts(segPts, subdivPerUnit = curveRes)

def getPtsAlongBezier2D(segPts, areaRegionInfo, curveRes, maxRes = None): # Used in functions where only visual resolution of curve matters (like draw Bezier) # (... not so expensive) # TODO: Calculate maxRes dynamically
    segLens = []
    for i in range(1, len(segPts)):
        seg = [segPts[i-1][1], segPts[i-1][2], segPts[i][0], segPts[i][1]]
        segLen = 0 #TODO: A more optimized solution... (Called very frequently)
        for info in areaRegionInfo:
            seg2D = [getCoordFromLoc(info[1], info[2], loc) for loc in seg]
            sl = getSegLen(seg2D)
            if(sl > segLen): segLen = sl
        segLens.append(segLen)
    return getInterpBezierPts(segPts, subdivPerUnit = curveRes, segLens = segLens, maxRes = maxRes)

def getLinesFromPts(pts):
    positions = []
    for i, pt in enumerate(pts):
        positions.append(pt)
        if(i > 0 and i < (len(pts)-1)): positions.append(pt)
    return positions

def getPartialSeg(seg, t0, t1): #see https://stackoverflow.com/questions/878862/drawing-part-of-a-b%c3%a9zier-curve-by-reusing-a-basic-b%c3%a9zier-curve-function/879213#879213
...
def getInterpolatedVertsCo(curvePts, numDivs):
    curveLength = sum((curvePts[i] - curvePts[i-1]).length for i in range(1, len(curvePts))) # Can be calculated only once
    if(floatCmpWithMargin(curveLength, 0)): return [curvePts[0]] * numDivs
    ...
    for i in range(1, numDivs):
        ...
        if(floatCmpWithMargin(actualLen, targetLen)): co = curvePts[vertIdx]
        else:   #interpolate
            diff = actualLen - targetLen
            co = (nextVertCo - (nextVertCo - vertCo) * (diff/(nextVertCo - vertCo).length))
            vertIdx -= 1 #Revert to last pt
            actualLen -= (nextVertCo - vertCo).length
        vertCos.append(co)
    vertCos.append(curvePts[-1]) # if(not vectCmpWithMargin(curvePts[0], curvePts[-1])):
    return vertCos

# The following section is a Python conversion of the javascript a2c function at: https://github.com/fontello/svgpath
# (Copyright (C) 2013-2015 by Vitaly Puzrin). Note: Most of the comments are retained
######################## a2c start #######################
TAU = pi * 2
# eslint-disable space-infix-ops
def unit_vector_angle(ux, uy, vx, vy): # Calculate an angle between two unit vectors. # Since we measure angle between radii of circular arcs, we can use simplified math (without length normalization)
    if(ux * vy - uy * vx < 0): sign = -1
    else: sign = 1
    dot  = ux * vx + uy * vy
    # dot /= sqrt(ux * ux + uy * uy) * sqrt(vx * vx + vy * vy) # Add this to work with arbitrary vectors:
    if (round(dot, 3) >=  1.0): dot =  1.0 # rounding errors, e.g. -1.0000000000000002 can screw up this
    if (round(dot, 3) <= -1.0): dot = -1.0
    return sign * acos(dot)

def get_arc_center(x1, y1, x2, y2, fa, fs, rx, ry, sin_phi, cos_phi): # Convert from endpoint to center parameterization, # see http:#www.w3.org/TR/SVG11/implnote.html#ArcImplementationNotes # Return [cx, cy, theta1, delta_theta]
    x1p =  cos_phi*(x1-x2)/2 + sin_phi*(y1-y2)/2 # Step 1.# Moving an ellipse so origin will be the middlepoint between our two points. After that, rotate it to line up ellipse axes with coordinate axes.
    y1p = -sin_phi*(x1-x2)/2 + cos_phi*(y1-y2)/2
    rx_sq  =  rx * rx
    ry_sq  =  ry * ry
    x1p_sq = x1p * x1p
    y1p_sq = y1p * y1p    
    radicant = (rx_sq * ry_sq) - (rx_sq * y1p_sq) - (ry_sq * x1p_sq) # Step 2.# Compute coordinates of the centre of this ellipse (cx', cy') in the new coordinate system.
    if (radicant < 0): radicant = 0 # due to rounding errors it might be e.g. -1.3877787807814457e-17
    radicant /=   (rx_sq * y1p_sq) + (ry_sq * x1p_sq)
    factor = 1
    if(fa == fs): factor = -1 # Migration Note: note ===
    radicant = sqrt(radicant) * factor #(fa === fs ? -1 : 1)
    cxp = radicant *  rx/ry * y1p
    cyp = radicant * -ry/rx * x1p
    cx = cos_phi*cxp - sin_phi*cyp + (x1+x2)/2 # Step 3. # Transform back to get centre coordinates (cx, cy) in the original coordinate system.
    cy = sin_phi*cxp + cos_phi*cyp + (y1+y2)/2
    v1x =  (x1p - cxp) / rx # Step 4.# Compute angles (theta1, delta_theta).
    v1y =  (y1p - cyp) / ry
    v2x = (-x1p - cxp) / rx
    v2y = (-y1p - cyp) / ry
    theta1 = unit_vector_angle(1, 0, v1x, v1y)
    delta_theta = unit_vector_angle(v1x, v1y, v2x, v2y)
    if (fs == 0 and delta_theta > 0): delta_theta -= TAU #Migration Note: note ===
    if (fs == 1 and delta_theta < 0): delta_theta += TAU #Migration Note: note ===
    return [ cx, cy, theta1, delta_theta ]

def approximate_unit_arc(theta1, delta_theta): # Approximate one unit arc segment with bezier curves, # see http:#math.stackexchange.com/questions/873224
...
def a2c(x1, y1, x2, y2, fa, fs, rx, ry, phi, noSegs):
    ...
    x1p =  cos_phi*(x1-x2)/2 + sin_phi*(y1-y2)/2 # Make sure radii are valid
    y1p = -sin_phi*(x1-x2)/2 + cos_phi*(y1-y2)/2
    if (x1p == 0 and y1p == 0): return [] # we're asked to draw line to itself # Migration Note: note ===
    if (rx == 0 or ry == 0): return [] # one of the radii is zero # Migration Note: note ===
    rx = abs(rx) # Compensate out-of-range radii
    ry = abs(ry)
    lmbd = (x1p * x1p) / (rx * rx) + (y1p * y1p) / (ry * ry)
    if (lmbd > 1):
        rx *= sqrt(lmbd)
        ry *= sqrt(lmbd)
    cc = get_arc_center(x1, y1, x2, y2, fa, fs, rx, ry, sin_phi, cos_phi) # Get center parameters (cx, cy, theta1, delta_theta)
    result = []
    theta1 = cc[2]
    delta_theta = cc[3]
    segments = noSegs # int(max(ceil(abs(delta_theta) / (TAU / 4)), 1)) # Split an arc to multiple segments, so each segment will be less than 90
    delta_theta /= segments
    for i in range(0, segments):
        result.append(approximate_unit_arc(theta1, delta_theta))
        theta1 += delta_theta    
    return getMappedList(result, rx, ry, sin_phi, cos_phi, cc) # We have a bezier approximation of a unit circle, now need to transform back to the original ellipse
...
def getMappedList(result, rx, ry, sin_phi, cos_phi, cc):
    ...
    for elem in result:
        ...
        for i in range(0, len(elem), 2):
            ...
            x *= rx # scale
            y *= ry
            xp = cos_phi*x - sin_phi*y # rotate
            yp = sin_phi*x + cos_phi*y
            elem[i + 0] = xp + cc[0] # translate
            elem[i + 1] = yp + cc[1]
...
def getSegsForArc(start, radius, sweep, end, noSegs, axisIdxs, z):
    ...
    newSegs = []
    for curvePts in curvesPts: newSegs.append([get3DVector(curvePts[0], axisIdxs, z), get3DVector(curvePts[1], axisIdxs, z), get3DVector(curvePts[2], axisIdxs, z), get3DVector(curvePts[3], axisIdxs, z)])
    return newSegs
...
SEARCH_CURVE_RES = .5 # Per pixel seg divisions (.5 is one div per 2 pixel units) # Some global constants
DBL_CLK_DURN = 0.25
SNGL_CLK_DURN = 0.3
...
class BptDisplayInfo:
    def __init__(self, pt, tipColors, handleNos = None): # handleNos: 0: seg1-left, 1: seg1-right # tipColors: leftHdl, pt, rightHdl # Caller to make sure there are no tips without handle
        self.pt = pt
        if(len(tipColors) == 1): self.tipColors = [None, tipColors[0], None]
        elif(len(tipColors) == 3): self.tipColors = tipColors
...
class RegionMouseXYInfo:
    ...
    def __eq__(self, other):
        ...
        return self.area == other.area and self.region == other.region and self.rv3d == other.rv3d
...
class BGLDrawInfoLine(BGLDrawInfo):
    def __init__(self, size, color, pts, gradientStart = None, gradientEnd = None, mid = True):
...
class BGLDrawMgr:
    ...
    def addLineInfo(self, infoId, size, color, pts, \
        ...
        self.lineInfoMap[infoId] = BGLDrawInfoLine(size, color, pts, gradientStart, gradientEnd, mid)
    ...
    def redraw(self):
        ...
        for i, info in enumerate(lineInfos):
            ...
            if(info.gradientEnd != None and info.gradientStart != None):
                if(len(info.pts) != 2 and len(info.color) != 1): raise ValueError('Exactly two coordinates and one color for gradient line')
                linePos, lineCols = getLineShades(info.pts, info.color[0], info.gradientStart, info.gradientEnd, info.mid)
            else:
                ...
                if(diff >= 0):
                    for j in range(diff): lineCols.append(info.color[-1])
                else:
                    for j in range(-diff): lineCols.pop()
            pos += linePos
            col += lineCols
        if(len(pos) > 0):
        ...
        for i, info in enumerate(ptInfos):
            ...
            if(diff >= 0):
                for j in range(diff): ptCols.append(info.color[-1])
            else:
                for j in range(-diff): ptCols.pop()
            pos += info.pts[:]
            col += ptCols
    ...
    def reset(self):
        for key in list(self.ptInfoMap.keys()): self.ptInfoMap[key].pts = []
        for key in list(self.lineInfoMap.keys()): self.lineInfoMap[key].pts = []

def updateBezierBatches(bglDrawMgr, segDispInfos, bptDispInfos, areaRegionInfo, defHdlType = 'ALIGNED'): # Return line batch for bezier line segments and handles and point batch for handle tips
    ...
    for i, info in enumerate(segDispInfos):
        ...
        if(isStraightSeg(segPts)):
            ...
            pts = getPtsAlongBezier2D(segPts, areaRegionInfo, FTProps.dispCurveRes, maxRes = MAX_NONSEL_CURVE_RES)
    ...
    for i, info in enumerate(bptDispInfos):
        ...
        for hn in info.handleNos:
            ...
            lineColors += [ModalBaseFlexiOp.hdlColMap[htype], ModalBaseFlexiOp.hdlColMap[htype]]
        tc = info.tipColors # Re-arrange tips so handles are on top of Bezier point
        tc = [tc[1], tc[0], tc[2]]
        pt = [pt[1], pt[0], pt[2]]
...
def updateMetaBtns(caller, event, keymap = None):
    if(keymap == None): keymap = {'LEFT_SHIFT': 'shift', 'RIGHT_SHIFT': 'shift', 'LEFT_CTRL':'ctrl', 'RIGHT_CTRL':'ctrl', 'LEFT_ALT': 'alt', 'RIGHT_ALT': 'alt'}
...
class FTHotKeyData:
    def __init__(self, id, key, label, description, isExclusive = False, \
        ...
        if(isExclusive and '+' in key): raise ValueError('Exclusive keys cannot be combined with meta keys')
        self.id = id
        self.key = key
        self.label = label
        self.description = description
        self.default = key
        # isExclusive means the hot key function will be invoked even if there are meta keys (that are not part of the hot key combination) are held down together with this key. This way user can have both
        # meta key related functionality (e.g. snapping) amd hot key functionality (e. g. tweak position) simultaneously (Tweak from a snapped point)
...
class FTHotKeys: ##################### Key IDs #####################
    hkGrabRepos = 'hkGrabRepos' # Draw
    hkUndoLastSeg = 'hkUndoLastSeg'
    hkDissociateHdl = 'hkDissociateHdl'
    hkResetLastHdl = 'hkResetLastHdl'
    drawHotkeys = []
    drawHotkeys.append(FTHotKeyData(hkGrabRepos, 'G', 'Grab Bezier Point', 'Grab Bezier point while drawing', inclTools = TOOL_TYPES_FLEXI_DRAW_COMMON, isExclusive = True))
    drawHotkeys.append(FTHotKeyData(hkDissociateHdl, 'V', 'Dissociate Draw Hendle', 'Dissociate right handle from left while drawing', inclTools = TOOL_TYPES_FLEXI_DRAW_COMMON, isExclusive = True))
    drawHotkeys.append(FTHotKeyData(hkResetLastHdl, 'Shift+R', 'Reset Last Handle', 'Reset last handle so that new segment starts as straight line', inclTools = TOOL_TYPES_FLEXI_DRAW_COMMON))
    drawHotkeys.append(FTHotKeyData(hkUndoLastSeg, 'BACK_SPACE', 'Undo Last Segment', 'Undo drawing of last segment', inclTools = TOOL_TYPES_FLEXI_DRAW_COMMON))
    hkUniSubdiv = 'hkUniSubdiv' # Edit
    hkBevelPt = 'hkBevelPt'
    ...
    editHotkeys = []
    editHotkeys.append(FTHotKeyData(hkUniSubdiv, 'W', 'Segment Uniform Subdivide', 'Hotkey to initiate Segment Uniform Subdiv op', inclTools = {TOOL_TYPE_FLEXI_EDIT}))
    editHotkeys.append(FTHotKeyData(hkBevelPt, 'Ctrl+B', 'Bevel Selected Points', 'Hotkey to initiate Bevel op', inclTools = {TOOL_TYPE_FLEXI_EDIT}))
    editHotkeys.append(FTHotKeyData(hkAlignHdl, 'K', 'Align Handle', 'Hotkey to align one handle with the other', inclTools = {TOOL_TYPE_FLEXI_EDIT}))
    editHotkeys.append(FTHotKeyData(hkDelPtSeg, 'DEL', 'Delete Point / Seg', 'Delete selected Point / Segment, align selected handle with other point', inclTools = {TOOL_TYPE_FLEXI_EDIT}))
    editHotkeys.append(FTHotKeyData(hkToggleHdl, 'H', 'Hide / Unhide Handles', 'Toggle handle visibility', inclTools = {TOOL_TYPE_FLEXI_EDIT}))
    editHotkeys.append(FTHotKeyData(hkSplitAtSel, 'L', 'Split At Selected Points', 'Split curve at selected Bezier points', inclTools = {TOOL_TYPE_FLEXI_EDIT}))
    editHotkeys.append(FTHotKeyData(hkMnHdlType, 'S', 'Set Handle Type', 'Set type of selected handles', inclTools = {TOOL_TYPE_FLEXI_EDIT}))
    editHotkeys.append(FTHotKeyData(hkMnSelect, 'A', 'Select', 'Select elements from existing spline selection', inclTools = {TOOL_TYPE_FLEXI_EDIT}))
    editHotkeys.append(FTHotKeyData(hkMnDeselect, 'Alt+A', 'Deselect', 'Deselect elements from existing spline selection', inclTools = {TOOL_TYPE_FLEXI_EDIT}))
    hkToggleKeyMap = 'hkToggleKeyMap' # Common
    hkSwitchOut = 'hkSwitchOut'
    hkTweakPos = 'hkTweakPos'
    hkToggleDrwEd = 'hkToggleDrwEd'
    hkReorient = 'hkReorient'
    commonHotkeys = []
    commonHotkeys.append(FTHotKeyData(hkToggleKeyMap, 'Ctrl+Shift+H', 'Hide / Unhide Keymap', 'Hide / Unhide Keymap Displayed When Flexi Tool Is Active'))
    commonHotkeys.append(FTHotKeyData(hkSwitchOut, 'F1', 'Exit Flexi Tool', 'Switch out of the Flexi Tool mode'))
    commonHotkeys.append(FTHotKeyData(hkTweakPos, 'P', 'Tweak Position', 'Tweak position or enter polar coordinates of the draw / edit point', isExclusive = True))
    commonHotkeys.append(FTHotKeyData(hkToggleDrwEd, 'E', 'Toggle Draw / Edit', 'Toggle between Draw & Edit Flexi Tools', exclTools = {TOOL_TYPE_FLEXI_GREASE}))
    commonHotkeys.append(FTHotKeyData(hkReorient, 'U', 'Origin to Active Face', 'Move origin / orientation to face under mouse cursor \" + "if the origin / orientation is object face'))
    hkSnapVert = 'hkSnapVert' # Snapping
    hkSnapGrid = 'hkSnapGrid'
    hkSnapAngle = 'hkSnapAngle'
    hkSnapVertMeta = 'hkSnapVertMeta' # Snapping (Suffix important)
    hkSnapGridMeta = 'hkSnapGridMeta'
    hkSnapAngleMeta = 'hkSnapAngleMeta'
    snapHotkeys = [] # Order important
    snapHotkeys.append(FTHotKeyData(hkSnapVert, 'F5', 'Snap to Vert / Face', 'Key pressed with mouse click for snapping to Vertex or Face'))
    snapHotkeys.append(FTHotKeyData(hkSnapGrid, 'F6', 'Snap to Grid', 'Key pressed with mouse click for snapping to Grid'))
    snapHotkeys.append(FTHotKeyData(hkSnapAngle, 'F7', 'Snap to Angle', 'Key pressed with mouse click for snapping to Angle Increment'))
    snapHotkeysMeta = [] # Order should be same as snapHotkeys
    snapHotkeysMeta.append(FTHotKeyData(hkSnapVertMeta, 'ALT', 'Snap to Vert / Face', 'Key pressed with mouse click for snapping to Vertex or Face')) # These keys are not event.type (they can have an entry 'KEY')
    snapHotkeysMeta.append(FTHotKeyData(hkSnapGridMeta, 'CTRL', 'Snap to Grid', 'Key pressed with mouse click for snapping to Grid'))
    snapHotkeysMeta.append(FTHotKeyData(hkSnapAngleMeta, 'SHIFT', 'Snap to Angle', 'Key pressed with mouse click for snapping to Angle Increment'))
    idDataMap = {h.id: h for h in drawHotkeys + editHotkeys + commonHotkeys + snapHotkeys} # Metakeys not part of the map
    # keyDataMap = {h.key: h for h in drawHotkeys + editHotkeys + commonHotkeys + snapHotkeys} # There can be multiple keydata with same keys (e.g. same key for draw and edit).  Not creating a separate map for now. May require later.
    exclKeys = {'RET', 'SPACE', 'ESC', 'X', 'Y', 'Z', 'ACCENT_GRAVE', 'COMMA', 'PERIOD', 'F2', 'F3'}
    ...
    def getSnapHotKeys(kId):
        ...
        if(len(keydatas) > 0):
            keydata = keydatas[0]
            if(keydata.key != 'KEY'): return ['LEFT_' + keydata.key, 'RIGHT_' + keydata.key]
    ...
    def getHotKeyData(toolType, key, metas):
        kds = [kd for kd in FTHotKeys.idDataMap.values() if(kd.key == FTHotKeys.getKey(key, metas) and toolType in kd.inclTools)] # Map will be more efficient, but not so many keys... So ok for now
        return kds[0] if len(kds) > 0 else None

    def isHotKey(id, key, metas):
        currKeyData = FTHotKeys.idDataMap[id]
        if(currKeyData.isExclusive): return currKeyData.key == key # Only compare part without meta for exclusive keys
        else: return currKeyData.key == FTHotKeys.getKey(key, metas)

    def haveCommonTool(keyData1, keyData2):
        return len(keyData1.inclTools.intersection(keyData2.inclTools)) > 0
    
    def isAssignedWithoutMeta(kId, key): # The regular part of the snap keys is validated against assigned key without meta, So that if e.g. Ctrl+B is already assigned, B is not available as reg part
        if(key.endswith(INVAL)): return True
        return any([kd.key.split('+')[-1] == key for kd in FTHotKeys.idDataMap.values() if kd.id != kId and FTHotKeys.haveCommonTool(kd, FTHotKeys.idDataMap[kId])])

    def isAssigned(kId, key):
        if(key.endswith(INVAL)): return True
        currKeyData = FTHotKeys.idDataMap.get(kId)
        exclKeys = [kd for kd in FTHotKeys.idDataMap.values() if(FTHotKeys.haveCommonTool(kd, currKeyData) and ((key == kd.key) or (kd.isExclusive and (key.split('+')[-1] == kd.key)) or (currKeyData.isExclusive and (key == kd.key.split('+')[-1]))))]
        return len(exclKeys) > 0
    updating = False

    def updateHotkeys(dummy, context): # Validation for single key text field (format: meta key + regular key) # UI Format: Key and 3 toggle buttons for meta # TODO: Separate update for each id? # TODO: Reset on any exception?
        try: FTHotKeys.updateHKPropPrefs(context) # FTHotKeys.keyDataMap = {h.key: h for h in [k for k in (FTHotKeys.drawHotkeys + FTHotKeys.editHotkeys + FTHotKeys.commonHotkeys + FTHotKeys.snapHotkeys)]}
        except Exception as e:
            FTHotKeys.updateHKPropPrefs(context, reset = True)
            print("BezierUtils: Error fetching keymap preferences", e)

    def updateHKPropPrefs(context, reset = False): # TODO: This method combines two rather different functionality based on reset flag # TODO: Metakey setting in updateSnapMetaKeys and reset here also
        if(FTHotKeys.updating): return
        FTHotKeys.updating = True
        prefs = context.preferences.addons[__name__].preferences
        hmap = FTHotKeys.idDataMap
        for kId in hmap: # Checking entire map even if one key changed (TODO)
            if(reset): hmap[kId].key = hmap[kId].default
            prefKey = getattr(prefs, kId) # User pressed key
            combKey = ''
            for meta in FTHotKeys.metas:
                if(hasattr(prefs, kId + meta) and getattr(prefs, kId + meta) == True): combKey += meta + '+'
            prefKey = combKey + prefKey # key in map has format - meta1 + met2 ... + (event.type)
            if(hmap[kId].key == prefKey): continue
            valid = (prefKey != INVAL and not reset)
            if(valid):
                if(kId in [k.id for k in FTHotKeys.snapHotkeys]): valid = not FTHotKeys.isAssignedWithoutMeta(kId, prefKey)
                else: valid = not FTHotKeys.isAssigned(kId, prefKey)
            if(not valid): # Revert to key from map if invalid
                comps = hmap[kId].key.split('+')
                setattr(prefs, kId, comps[-1])
                for meta in FTHotKeys.metas: setattr(prefs, kId + meta, meta in comps[:-1])
            else: hmap[kId].key = prefKey
        if(reset):
    ...
    def getAvailKey(prefs):
        ...
        newKeys = set([getattr(prefs, kd.id) for kd in FTHotKeys.snapHotkeysMeta])
    ...
    # Validation for snap keys (format: EITHER meta key OR regular key) (regular part validated by updateHotkeys)
    # UI Format: Drop-down with entries Ctrl, Alt, Shift, Keyboard and ... a separate single key field activated with selection of 'Keyboard' in the dropdown
    def updateSnapMetaKeys(dummy, context):
        ...
        if(changedKeyIdx != -1):
            ...
            if(prefKeyMeta == 'KEY'):
                ...
                regKeyData = FTHotKeys.idDataMap[regKeyId]
                prefKeyReg = getattr(prefs, regKeyId)                
                if(prefKeyReg == INVAL or FTHotKeys.isAssignedWithoutMeta(regKeyId, prefKeyReg)): setattr(prefs, regKeyId, regKeyData.key) # Invalid regular key->assign available meta key to the dropdown entry
                else: regKeyData.key = prefKeyReg
            else:
                availKey = FTHotKeys.getAvailKey(prefs) # Change the other drop-down with the same key
    ...
    def keyCodeMap():
        kcMap = {}
        digits = ['ZERO', 'ONE', 'TWO', 'THREE', 'FOUR', 'FIVE', 'SIX', 'SEVEN', 'EIGHT', 'NINE']#, 'PERIOD']
        numpadDigits = ['NUMPAD_' + d for d in digits]
        # 199: 'NUMPAD_PERIOD'
        for i in range(26): kcMap[i + 97] = chr(ord('A') + i)
        # for i in range(10): kcMap[i + 48] = digits[i] # Digits not available, used for keyboard input
        for i in range(10): kcMap[i + 150] = numpadDigits[i]
        for i in range(12): kcMap[i + 300] = 'F' + str(i + 1)
    ...
    def getKeyMapTupleStr(): # Drop down item tuples with 400 entries # INVAL in all 3 fields for unavailable keycodes # TODO: Very big drop-down for every hotkey (because of default)
        kcMap = FTHotKeys.keyCodeMap()
        tuples = []
        exclKeys = FTHotKeys.exclKeys
        for i in range(0, 400):
            if(kcMap.get(i) != None and kcMap[i] not in exclKeys): char = kcMap[i]
            else: char = INVAL
            tuples.append((char, char, char))
        return ''.join(["('" + t[0] + "','" + t[1] + "','" + t[2] +"')," for t in tuples])
    ...
    def getHKFieldStr(keydata, addMeta):
        ...
        keys = default.split('+')
        key = keys[-1] # "items = (('A', 'A','A'),('B', 'B', 'B')), " + \
        retVal = propName + ": EnumProperty(name = '" + text + "', " + "items = (" + FTHotKeys.getKeyMapTupleStr() + "), " + (("default = '"+ key + "', ") if default != INVAL else '') + "update = " + updateFn + ", " + "description = '"+ description +"') \n"
        if(addMeta):
            for meta in FTHotKeys.metas: retVal += propName + meta + ": BoolProperty(name='"+ meta +"', " + "description='"+ meta +" Key', " + "update = " + updateFn + ", " + "default = "+ ('True' if meta in keys[:-1] else 'False') +") \n"
        return retVal
    ...
    def getMetaHKFieldStr(keydataMeta):
        ...
        itemsStr = "('CTRL', 'Ctrl', 'Ctrl'), ('ALT', 'Alt', 'Alt'), ('SHIFT', 'Shift', 'Shift'), ('KEY', 'Keyboard', 'Other keyboard key')"
        retVal = propName + ": EnumProperty(name = '" + text + "', " + "items = (" + itemsStr + "), " + "default = '"+ default + "', " + "update = " + updateFn + ", " + "description = '"+ description +"') \n"
        return retVal
    ...
    def getHKDispLines(toolType):
        ...
        for i, hk in enumerate(FTHotKeys.snapHotkeysMeta):
            if(hk.key == 'KEY' and toolType not in hk.exclTools): hkData.append(FTHotKeys.snapHotkeys[i])
            else: hkData.append(hk)
        if(toolType in TOOL_TYPES_FLEXI_DRAW_COMMON): hkData += [k for k in FTHotKeys.drawHotkeys if toolType not in k.exclTools]
        if(toolType == TOOL_TYPE_FLEXI_EDIT): hkData += [k for k in FTHotKeys.editHotkeys if toolType not in k.exclTools]
        labels = []
        config = []
        keys = []
        stdKeylabels = []
...
class FTProps: # Flexi Tool Constants
    propUpdating = False
    ...
    def updatePropsPrefs(context, resetPrefs = False):
        ...
        try:
            ...
            if(resetPrefs):
                FTProps.initDefault()
                for prop in props: exec('prefs.' + prop +' = FTProps.' + prop) # setattr(prefs, prop, getattr(FTProps, prop))
            else:
                for prop in props: exec('FTProps.' + prop +' = prefs.' + prop) # setattr(FTProps, prop, getattr(prefs, prop))
        except Exception as e:
...
class FTMenu: # TODO: Move to end to avoid eval/getattr due to forward referencing
    ...
    def getMenuData(caller, hotkeyId):
        ...
        if(keyIds != None and hotkeyId in keyIds): return FTMenu.idDataMap.get(hotkeyId)
        return None
    ...
    def procMenu(parent, context, event, outside):
        ...
        if(FTMenu.currMenuId != None):
            ...
            if(opt != None or not evtType.startswith('TIMER')): # What's TIMER_REPORT?
                ...
                if(evtType == 'TIMER'):
                    ...
                    fn(opt)
                else: FTMenu.abandoned = True
            return True
        if(outside): return False
        parentClassName = parent.__class__.__name__
        hkData = FTHotKeys.getHotKeyData(FTMenu.toolTypeMap.get(parentClassName), evtType, metakeys)
        ...
        if(event.value == 'RELEASE'):
            ...
            parent.menuTimer = context.window_manager.event_timer_add(time_step = 0.0001, window = context.window)
    ...
    def resetMenuOptions():
        params = bpy.context.window_manager.bezierToolkitParams
        for propName in FTMenu.getAllOptPropNames(): setattr(params, propName, False)

    def getAllOptPropNames():
        propNames = []
        maxMenuOpts = max(len(m.options) for m in FTMenu.idDataMap.values())
        for i in range(maxMenuOpts): propNames.append(FTMenu.propSuffix + str(i))
        return propNames
    ...
    def getMNClassDefStr(menuData):
        ...
        for i, opt in enumerate(menuData.options):
            retStr += '\t\top = pie.operator("object.ft_menu_options", text = "'+ opt[1] + '"' + ((', icon = "'+ opt[2] +'"') if(opt[2] != '') else '') + ')\n'
            retStr += '\t\top.optIdx = ' + str(i) + '\n'
        return retStr

    def getMNPropDefStr(menuData):
        retStr = ''
        for propName in FTMenu.getAllOptPropNames(): retStr += propName +': BoolProperty(default = False)\n'
        return retStr
...
class SnapDigits:
    digitMap = {'ZERO':'0', 'ONE':'1', 'TWO':'2', 'THREE':'3', 'FOUR':'4', 'FIVE':'5', 'SIX':'6', 'SEVEN':'7', 'EIGHT':'8', 'NINE':'9', 'PERIOD':'.'}
    numpadDigitMap = {'NUMPAD_0':'0', 'NUMPAD_1':'1', 'NUMPAD_2':'2', 'NUMPAD_3':'3', 'NUMPAD_4':'4', 'NUMPAD_5':'5', 'NUMPAD_6':'6', 'NUMPAD_7':'7', 'NUMPAD_8':'8', 'NUMPAD_9':'9', 'NUMPAD_PERIOD': '.'}
    ...
    def getPolarCos(self): # Theta in degrees
    ...
    def getDeltaFromPolar(self, polCos): # Theta in degrees
        r, theta = polCos
    ...
    def digitsToVec(self):
        ...
        else: self.deltaVec[self.axisIdx] += delta

    def vecToDigits(self):
        if(self.polar):
            vals = self.getPolarCos()
            v = round(vals[self.pDataIdx], 4)
        else: v = round(self.deltaVec[self.axisIdx], 4)
        self.digitChars = list(str(abs(v))) if v != 0 else ''
        while(len(self.digitChars) > 0 and self.digitChars[-1] == '0'): self.digitChars.pop()
        if(len(self.digitChars) > 0 and self.digitChars[-1] == '.'): self.digitChars.pop()
    ...
    def procEvent(self, context, event, metakeys):
            ...
            if(len(editCos) == 2):
                if(event.value == 'RELEASE'):
                    if(self.axes == None):
                        ...
                        self.polar = False
                    elif(len(self.getFreeAxes()) == 2): # Only possible within a plane right now
        ...
        if(FTProps.numpadEntry): dmap.update(self.numpadDigitMap)
        ...
        if(dval != None):
            if(event.value == 'RELEASE'):
                ...
                if(self.axes == None):
                    ...
                    self.axisIdx = self.axes[0]
            return True
        if(not self.hasVal()): return False # No further processing if nothing to process
        retVal = True
        if(event.type == 'MINUS'):
            if(event.value == 'RELEASE'): self.signChar = '' if(self.signChar == '-') else '-'
        elif(event.type == 'ESC'):
            if(event.value == 'RELEASE'): self.initialize()
                ...
                if(len(self.digitChars) > 0): self.digitChars.pop()
                    ...
                    if(self.polar):
                        ...
                        if(not floatCmpWithMargin(polarCos[self.pDataIdx], 0)):
                            self.vecToDigits()
                            polarCos[self.pDataIdx] = 0.00001 # TODO: Retain theta without this workaround
            ...
            if(event.value == 'RELEASE'):
                self.digitsToVec()
                if(self.polar): self.pDataIdx = (self.pDataIdx + 1) % 2
                else: self.axisIdx = self.axes[(self.axes.index(self.axisIdx) + 1) % len(self.axes)]
                self.digitChars = []
                self.signChar = ''
        else:
            retVal = False
    ...
    def getDeltaStrPolar(self):
        ...
        d = polCos[idx0]
        if(d != 0): strs[idx0] += str(round(d, 4)) + ('+' if (self.signChar == '') else '')
        strs[idx0] += self.signChar + ''.join(self.digitChars) + '] = ' + str(round((delta + d), 4)) + ('' if valid else ' <Invalid>')
    ...
    def getCurrDeltaStr(self):
        ...
        if(d != 0): retStr += str(round(d, 4)) + ('+' if (self.signChar == '') else '')
        retStr += self.signChar + ''.join(self.digitChars) + '] = ' + str(round((delta + d), 4)) + ('' if valid else ' <Invalid>')
        return retStr

class CustomAxis:
    def __init__(self): #TODO: What's better?
        if(bpy.data.scenes[0].get('btk_co1') == None): bpy.data.scenes[0]['btk_co1'] = LARGE_VECT
        if(bpy.data.scenes[0].get('btk_co2') == None): bpy.data.scenes[0]['btk_co2'] = LARGE_VECT
        self.axisPts = [Vector(bpy.data.scenes[0]['btk_co1']), Vector(bpy.data.scenes[0]['btk_co2'])]
        if(bpy.data.scenes[0].get('btk_snapPtCnt') == None): bpy.data.scenes[0]['btk_snapPtCnt'] = 3
        self.snapCnt = bpy.data.scenes[0]['btk_snapPtCnt']
        self.inDrawAxis = False # User drawing the custom axis

    def length(self):
        pts = self.axisPts
        if(all(pt < (LARGE_NO - 1000) for pt in pts[0] + pts[1])): return (pts[1] - pts[0]).length # Strange floating points!
        else: return 0
    ...
    def getSnapPts(self): # ptCnt excluding end points
        ...
        diffV = pts[1] - pts[0]
        for i in range(1, interval): snapPts.append(pts[0] + (diffV * (incr * i)  / self.length()) )
    ...
    def procDrawEvent(self, context, event, snapper, rmInfo):
        if(event.type == 'RIGHTMOUSE'):
            ...
            if(event.value == 'RELEASE' and snapOrigin == 'AXIS'):
                loc = snapper.get3dLocSnap(rmInfo, SnapParams(snapper, snapToAxisLine = False))
                if(not self.inDrawAxis): self.set(0, loc)
                else: self.set(1, loc)
                self.inDrawAxis = not self.inDrawAxis
            return True
        if(self.inDrawAxis):
            if(event.type in {'WHEELDOWNMOUSE', 'WHEELUPMOUSE', 'NUMPAD_PLUS', 'NUMPAD_MINUS','PLUS', 'MINUS'}):
                if(event.type in {'NUMPAD_PLUS', 'NUMPAD_MINUS', 'PLUS', 'MINUS'} and event.value == 'PRESS'): return True
...
class SnapParams: # TODO: Make independent of snapper
    def __init__(self, snapper, xyDelta = [0, 0], vec = None, refreshStatus = True, snapToAxisLine = True, lastCo1Axis = False,
                    enableSnap = True, vertSnap = None, gridSnap = None, angleSnap = None, refLine = None, refLineOrig = None, selCo = None, inEdit = None, hasSel = None,
                        transType = None, origType = None, axisScale = None, freeAxesN = None, dispAxes = True, snapToPlane = None):
        ...
        self.refLineOrig = snapper.getRefLineOrig() if(refLineOrig == None) else refLineOrig
        ...
        self.freeAxesN = snapper.getFreeAxesNormalized() if(freeAxesN == None) else freeAxesN
        self.dispAxes = dispAxes
        if(snapToPlane != None): self.snapToPlane = snapToPlane
        else:
            if(showSnapToPlane(params)): self.snapToPlane = params.snapToPlane # Only if Snap to Plane option is visible
...
class Snapper:
    ...
    def resetMetakeys(self): # metakeys for all functions
        self.shift = False
        self.ctrl = False
        self.alt = False
    ...
    def updateSnapKeyMap(self):
        ...
        varMap = {FTHotKeys.hkSnapVert: 'vertSnap', FTHotKeys.hkSnapAngle: 'angleSnap', FTHotKeys.hkSnapGrid: 'gridSnap'}
        for kId in kIds:
            keys = FTHotKeys.getSnapHotKeys(kId)
            for key in keys: self.snapKeyMap[key] = varMap[kId]
    ...
    def resetSnap(self): # Called even during isEditing
        ...
        self.digitsConfirmed = False # This variable lets caller know that return was pressed after digits were entered Caller can reset snapper as per convenience
        self.lastSelCo = None
        # self.snapStack = [] # TODO

    def getFreeAxesGlobal(self): # Return user selection in header menu, [] for None # (Tightly coupled with strings in BezierToolkitParams)
    ...
    def getFreeAxesCombined(self): # Return actual free axes (menu + hotkey combined), [] for None
    ...
    def getFreeAxesNormalized(self): # Return actual free axes (menu + hotkey combined), [0, 1, 2] for None
        if(len(self.getFreeAxesCombined()) == 0): return [0, 1, 2]
        else: return self.getFreeAxesCombined()

    def getSnapParamsFreeAxes(self):
        if(self.snapParams != None): return self.snapParams.freeAxesN
        else: return self.getFreeAxesNormalized()
    ...
    def getCurrOrig(self, rmInfo, obj, origType, refLineOrig, selCo):
        if(origType == 'AXIS'):
            ...
            selObj, location, normal, faceIdx = getSelFaceLoc(rmInfo.region, rmInfo.rv3d, rmInfo.xy, self.MAX_SNAP_FACE_CNT)
            if(faceIdx != None): return selObj.matrix_world @ selObj.data.polygons[faceIdx].center
    ...
    def getTransMatsForOrient(self, rmInfo, obj, transType, axisScale):
        ...
        if(custAxis != None and axisScale == 'AXIS'):
            ...
            if(unitD > DEF_ERR_MARGIN): tmScale = Matrix.Scale(1 / unitD, 4)
        tm = None
        if(transType == 'AXIS' and custAxis != None): tm, invTm = getLineTransMatrices(custAxis.axisPts[0], custAxis.axisPts[1])
        elif(transType == 'REFERENCE' and refLine != None): tm, invTm = getLineTransMatrices(refLine[0], refLine[1])
        elif(transType == 'CURR_POS' and currLine != None): tm, invTm = getLineTransMatrices(currLine[0], currLine[1])
        elif(transType == 'VIEW'): tm = rmInfo.rv3d.view_matrix
        elif(obj != None and transType == 'OBJECT'): tm = (obj.matrix_world).inverted_safe()
        elif(transType == 'FACE'):
            selObj, location, normal, faceIdx = getSelFaceLoc(rmInfo.region, rmInfo.rv3d, rmInfo.xy, self.MAX_SNAP_FACE_CNT)
            if(faceIdx != None):
    ...
    def isLocked(self):
        return len(self.freeAxes) > 0 or (self.snapDigits.hasVal() and not self.digitsConfirmed)

    def getMetakeys(self):
        return [self.alt, self.ctrl, self.shift]

    def procEvent(self, context, event): # To be called in modal method of parent
        retValMeta = updateMetaBtns(self, event) # update ctrl etc.
        ...
        refLineOrig = self.snapParams.refLineOrig if self.snapParams != None else self.getRefLineOrig()
        if(refLineOrig != None):
        ...
        if(FTHotKeys.isHotKey(FTHotKeys.hkReorient, event.type, metakeys)):
            if(event.value == 'RELEASE'): self.freezeOrient = not self.freezeOrient
            return EVT_CONS
        if(not self.ctrl and event.type in {'X', 'Y', 'Z'}):
            ...
            if(event.value == 'RELEASE'):
                ...
                if(self.shift): self.freeAxes = sorted(list({0, 1, 2} - set(self.freeAxes)))
                if(self.getFreeAxesCombined() == self.getFreeAxesGlobal()): self.freeAxes = [] # if already part of global axes, don't store (no escape needed)
            return EVT_CONS
        retVal = EVT_NOT_CONS
        if(self.isLocked()): # Consume escape or return / space only if there's something to process
            retVal = EVT_CONS
            ...
            else: retVal = EVT_NOT_CONS
        return retVal

    def getStatusStr(self, unit, invTm, refPt, newPt):
        manualEntry = self.snapDigits.hasVal()
        if(manualEntry and self.snapDigits.polar): return self.snapDigits.getDeltaStrPolar()
        ...
        for i, d in enumerate(diffV):
            ...
            v1 = chr(ord('x') + i)
            if(manualEntry and i == self.snapDigits.axisIdx): v2 = self.snapDigits.getCurrDeltaStr()
            else: v2 = str(round(d, 4))
    ...
    def getAllSnapLocs(self, snapToAxisLine):
        ...
        if(snapToAxisLine): snapLocs += self.customAxis.getSnapPts()
        vertCnt = 0
    ...
    def getTMInfoAndOrig(self, rmInfo, transType, origType, freezeOrient, axisScale, refLineOrig, selCo):
        obj = bpy.context.object
        if(self.tm != None and self.freezeOrient and transType == 'FACE'): tm, invTm = self.tm, self.tm.inverted_safe()
        else: tm, invTm = self.getTransMatsForOrient(rmInfo, obj, transType, axisScale)
        if(self.orig != None and freezeOrient and origType == 'FACE'): orig = self.orig
        else: orig = self.getCurrOrig(rmInfo, obj, origType, refLineOrig, selCo)
    ...
    def get3dLocSnap(self, rmInfo, snapParams = None):
        ...
        rv3d = rmInfo.rv3d
        xy = [rmInfo.xy[0] - snapParams.xyDelta[0], rmInfo.xy[1] - snapParams.xyDelta[1]]
        loc = None
        tm, invTm, orig = self.getTMInfoAndOrig(rmInfo, transType, origType, self.freezeOrient, axisScale, refLineOrig, selCo)
        if(hasSel): self.freezeOrient = True # Must be done after the call to getTMInfoAndOrig
        ...
        if(FTProps.dispSnapInd or vertSnap):
            snapLocs = self.getAllSnapLocs((snapToAxisLine and 'AXIS' in {transType, origType, axisScale})) + [orig] #TODO: Called very frequently (store the tree [without duplicating data])
            kd = kdtree.KDTree(len(snapLocs))
            for i, l in enumerate(snapLocs): kd.insert(getCoordFromLoc(region, rv3d, l).to_3d(), i)
            kd.balance()
        ...
        if(vertSnap):
            if(self.snapCo != None): loc = self.snapCo
            else: selObj, loc, normal, faceIdx = getSelFaceLoc(region, rv3d, xy, self.MAX_SNAP_FACE_CNT, checkEdge = True)
        if(loc != None):
            loc = tm @ loc
            ...
            loc = tm @ loc            
            if((transType != 'GLOBAL' and inEdit) or snapToPlane or gridSnap or self.snapDigits.hasVal() or (inEdit and (len(freeAxesN) < 3 or angleSnap))): # TODO: Get gridSnap and angleSnap out of this if
                # if(snapToPlane or refLineOrig == None): refCo = orig # snapToPlane means global constrain axes selection is a plane
                ...
                if(self.snapDigits.hasVal()):
                    ...
                    # if(len(freeAxesN) == 1 and refLineOrig != None): refCo = tm @ refLineOrig # Special condition for lock to single axis
                    if(len(freeAxesN) == 2):
                        constrAxes = freeAxesN
                        loc = refCo.copy()
                        ppt1 = loc.copy() # Any other two points on the plane
                        ppt1[constrAxes[0]] = loc[constrAxes[0]] + 10
                        ppt2 = loc.copy()
                        ppt2[constrAxes[1]] = loc[constrAxes[1]] + 10
                        ppt3 = ppt2.copy()
                        ppt2[constrAxes[0]] = loc[constrAxes[0]] + 10
                        pt = getPtProjOnPlane(region, rv3d, xy, invTm @ loc, invTm @ ppt1, invTm @ ppt2, invTm @ ppt3) # Raycast from 2d point onto the plane
                        for axis in constrAxes: # TODO: Better handling of boundary condition
                            if(pt == None or pt[axis] > 1000): loc[axis] = refCo[axis]
                    ...
                    if(len(freeAxesN) == 1):
                        ...
                        axis = freeAxesN[0]
                        ptOnAxis = refCo.copy() # Any one point on axis
                        lastCo2d = getCoordFromLoc(region, rv3d, invTm @ refCo) # Convert everything to 2d
                        ptOnAxis[axis] += .01 # Very small distance so that the point is on viewport # TODO: This is not foolproof :(
                        ptOnAxis2d = getCoordFromLoc(region, rv3d, invTm @ ptOnAxis)
                        pt2d = geometry.intersect_point_line(xy, lastCo2d, ptOnAxis2d)[0] # Find 2d projection (needed)
                        ppt1 = refCo.copy() # Any other 2 points on the plane, on which the axis lies
                        ppt1[axis] += 10
                        newAxis = [i for i in range(0, 3) if i != axis][0]
                        ppt2 = refCo.copy()
                        ppt2[newAxis] += 10
                        pt = getPtProjOnPlane(region, rv3d, pt2d, invTm @ refCo, invTm @ ppt1, invTm @ ppt2) # Raycast from 2d point onto the plane
                        loc = refCo.copy()
                        if(pt == None or pt[axis] > 1000): loc[axis] = refCo[axis]
                        else: loc[axis] = (tm @ pt)[axis]
                ...
                if(not self.snapDigits.hasVal() and gridSnap):
                    if(axisScale in {'AXIS' or 'REFERENCE'}):
                        diffV = (loc - refCo) # Independent of view distance
                        loc = refCo + round(diffV.length) * (diffV / diffV.length)
                    else:
                        rounding = getViewDistRounding(rmInfo.space3d, rv3d)
                        loc = tm @ roundedVect(rmInfo.space3d, invTm @ loc, rounding, freeAxesN)
                ...
                if(not self.snapDigits.hasVal() and angleSnap and len(refLine) > 0):
                    ...
                    snapStart = tm @ orig
                    actualLoc = loc.copy()
                    diff = [abs(v) for v in (actualLoc - snapStart)] #First decide the main movement axis
                    maxDiff = max(diff)
                    axis = diff.index(maxDiff)
                    loc = snapStart.copy()
                    loc[axis] = actualLoc[axis]
                    snapIncr = 45 / self.angleSnapSteps
                    snapAngles = [radians(snapIncr * a) for a in range(0, self.angleSnapSteps + 1)]
                    ...
                    for i in range(0, 3):
                        if(i != axis and (i in freeAxesN)):
                            ...
                            for j in range(0, len(snapAngles) + 1):
                                if(j == len(snapAngles)):
                                    loc[i] = snapStart[i] + dirn * l1 * tan(snapAngles[-1])
                                    break
                                cmpAngle = snapAngles[j]
                                if(abs(angle - cmpAngle) > prevDiff):
                                    loc[i] = snapStart[i] + dirn * l1 * tan(snapAngles[j-1])
                                    break
    ...
    def getEditCoPair(self):
        ...
        if(self.lastSelCo == None or refLineOrig == None): return []
        ...
        orig = self.getCurrOrig(self.rmInfo, bpy.context.object, origType, refLineOrig, selCo)
        return (self.tm @ orig, self.tm @ self.lastSelCo)

    def setStatus(self, area, text): #TODO Global
        area.header_text_set(text)

    def updateGuideBatches(self, bglDrawMgr): # Tightly coupled with get3dLocSnap
        drawAxes = [0, 1, 2] # Default values for resetting the bgl lines and points
        axisLineCos = [[], [], []]
        axisLineCols = [[], [], []]
        snapLineCos = []
        snapLineCols = []
        ...
        if(rmInfo != None): # self.snapParams is also not None
            ...
            tm, invTm, orig = self.getTMInfoAndOrig(rmInfo, transType, origType, self.freezeOrient, axisScale, refLineOrig, selCo)
            if(dispAxes and FTProps.dispAxes and ((refLineOrig != None or transType == 'VIEW' or len(freeAxesN) == 1) or (len(freeAxesN) > 0 and origType != 'REFERENCE'))):
            ...
            if(refLineOrig != None and self.lastSelCo != None and (self.angleSnap or ('keyboard' in self.lastSnapTypes and self.snapDigits.polar))):
                snapLineCos = [orig, self.lastSelCo]
                snapLineCols = [(.4, .4, .4, 1)]
                ptCol = (1, 1, 1, 1)
            if(self.customAxis.length() != 0 and (self.customAxis.inDrawAxis == True or 'AXIS' in {transType, origType, axisScale})):
        ...
        for i, axis in enumerate(drawAxes):
            ...
            bglDrawMgr.addLineInfo('snapAxis'+str(axis), FTProps.axisLineWidth, axisLineCols[i], axisLineCos[i], axisGradStart, axisGradEnd)
        bglDrawMgr.addLineInfo('SnapLine', FTProps.axisLineWidth, snapLineCols, snapLineCos)
        bglDrawMgr.addLineInfo('CustAxisLine', FTProps.axisLineWidth, custAxisLineCols, custAxisLineCos, custAxisGradStart, custAxisGradEnd, mid = False)
        bglDrawMgr.addPtInfo('CustAxisPt', FTProps.snapPtSize, custAxisPtCols, custAxisPtCos)
...
class ModalBaseFlexiOp(Operator):
    ...
    def drawKeyMap():
        regions = [r for area in bpy.context.screen.areas if  area.type == 'VIEW_3D' for r in area.regions if r.type == 'WINDOW']
        maxArea = max(r.width * r.height for r in regions)
        currRegion = [r for r in bpy.context.area.regions if r.type == 'WINDOW'][0]
        if(currRegion.width * currRegion.height < maxArea): return # Only display in window with max area
    ...
    def addDrawHandlers(context):
        ...
        ModalBaseFlexiOp.drawHdlrRef = bpy.types.SpaceView3D.draw_handler_add(hdlr, (), "WINDOW", "POST_VIEW")
        ModalBaseFlexiOp.drawTxtHdlrRef = bpy.types.SpaceView3D.draw_handler_add(ModalBaseFlexiOp.drawKeyMap, (), "WINDOW", "POST_PIXEL")

    def removeDrawHandlers():
        if(ModalBaseFlexiOp.drawHdlrRef != None):
            bpy.types.SpaceView3D.draw_handler_remove(ModalBaseFlexiOp.drawHdlrRef, "WINDOW")
            ModalBaseFlexiOp.drawHdlrRef = None
        if(ModalBaseFlexiOp.drawTxtHdlrRef != None):
            bpy.types.SpaceView3D.draw_handler_remove(ModalBaseFlexiOp.drawTxtHdlrRef, "WINDOW")
            ModalBaseFlexiOp.drawTxtHdlrRef = None

    def drawHandlerBase():
        if(ModalBaseFlexiOp.shader != None): ModalBaseFlexiOp.bglDrawMgr.redraw()

    def tagRedraw():
        areas = [a for a in bpy.context.screen.areas if a.type == 'VIEW_3D']
        for a in areas: a.tag_redraw()

    def propsChanged(): # Called back after add-on preferences are changed
        if(ModalBaseFlexiOp.opObj != None and ModalBaseFlexiOp.opObj.snapper != None):
            ModalBaseFlexiOp.opObj.snapper.updateSnapKeyMap()
            ModalBaseFlexiOp.hdlColMap ={'FREE': FTProps.colHdlFree, 'VECTOR': FTProps.colHdlVector, 'ALIGNED': FTProps.colHdlAligned, 'AUTO': FTProps.colHdlAuto}
    ...
    def refreshDisplayBase(segDispInfos, bptDispInfos, snapper):
        ...
        updateBezierBatches(ModalDrawBezierOp.bglDrawMgr, segDispInfos, bptDispInfos, areaRegionInfo)
        if(snapper != None): snapper.updateGuideBatches(ModalBaseFlexiOp.bglDrawMgr)
        ModalBaseFlexiOp.tagRedraw()

    @persistent
    def loadPostHandler(dummy):
        if(ModalBaseFlexiOp.shader != None): ModalBaseFlexiOp.resetDisplayBase()
        ModalBaseFlexiOp.running = False

    @persistent
    def loadPreHandler(dummy):
        ModalBaseFlexiOp.removeDrawHandlers()
        if(ModalBaseFlexiOp.drawFunc != None): bpy.types.VIEW3D_HT_tool_header.draw = ModalBaseFlexiOp.drawFunc
    ...
    def invoke(self, context, event):
        ...
        self.snapper = Snapper(context, self.getSnapLocs, self.getRefLine, self.getRefLineOrig, self.getSelCo, self.getCurrLine, self.hasSelection, self.isEditing)
        self.rmInfo = None
    ...
    def modal(self, context, event):
        ...
        if(event.type == 'LEFTMOUSE'):
            if(event.value == 'PRESS'): self.pressT = time.time()
        ...
        if(FTHotKeys.isHotKey(FTHotKeys.hkToggleKeyMap, event.type, metakeys)):
            ...
            return {'RUNNING_MODAL'}
        # Special condition for case where user has configured a different snap key. In such case, pass through mouse clicks, if there's a meta key held down... to allow e.g. alt + LMB to rotate view
        if(snapProc != EVT_CONS and any(metakeys) and not any([snapper.angleSnap, snapper.gridSnap, snapper.vertSnap]) and ((event.type == 'LEFTMOUSE' and event.value in {'PRESS', 'RELEASE'}) or event.type == 'MOUSEMOVE')): return {'PASS_THROUGH'}
        ...
        if(ret):
            if(event.value == 'RELEASE'): # Menu displayed on release, so retain metakeys till release
                snapper.resetMetakeys()
                snapper.resetSnapKeys()
            return {'RUNNING_MODAL'}
        if(self.isEditing() and self.rmInfo != rmInfo): return {'RUNNING_MODAL'}
        if(rmInfo == None): return {'PASS_THROUGH'}
        self.rmInfo = rmInfo
        ...
        if(evtCons and event.value == 'PRESS' and event.type != 'MOUSEMOVE' and not event.type.startswith('WHEEL') and (snapProc != EVT_META_OR_SNAP)): return {'RUNNING_MODAL'}
...
class Primitive2DDraw(BaseDraw):
    ...
    def set2DAxes(self):
        ...
        if(len(self.freeAxesN) > 2 and params.snapOrient in {'GLOBAL', 'REFERENCE', 'CURR_POS'}): self.freeAxesN = getClosestPlaneToView(self.parent.rmInfo.rv3d)

    def getNumSegsLimits(self):
        return 2, 100

    def getShapePts(self, mode, numSegs, bbStart, bbEnd, center2d, startAngle, theta, axisIdxs, z):
        raise NotImplementedError('Call to abstract method.')

    dynamicParams = { # index: ((incr_key, decr_key), metakey, description)
    ...
    def getParamHotKeyDescriptions():
        return [Primitive2DDraw.dynamicParams[p][1] for p in range(len(Primitive2DDraw.dynamicParams))]

    # Format (can be overriden in subclass): # default definition of all params
    #       def updateParam0(self, event, rmInfo, isIncr): pass # 0-5
    ...
    def getCurvePts(self, numSegs, axisIdxs, z = None):
        ...
        else: bbStart = tm @ self.bbStart
        cX = (bbEnd[idx0] - bbStart[idx0]) / 2
        cY = (bbEnd[idx1] - bbStart[idx1]) / 2
        if(cX == 0 and cY == 0): return None
        if(z == None): z = bbStart[idx2]
        center2d = complex(cX, cY)
        snapOrigin = bpy.context.window_manager.bezierToolkitParams.snapOrigin
        orig = complex(bbStart[idx0], bbStart[idx1])
        self.curveObjOrigin = tm.inverted_safe() @ get3DVector(orig + center2d, axisIdxs, z)
        curvePts = self.getShapePts(mode, numSegs, bbStart, bbEnd, center2d, startAngle, sweep, axisIdxs, z)
        if(curvePts == None): return None
        curvePts = [[tm.inverted_safe() @ p if type(p) != str else p for p in pts] for pts in curvePts]
        return curvePts
    ...
    def updateCurvePts(self):
        ...
        if(curvePts != None): self.setCurvePts(curvePts)
        else: self.setCurvePts([[self.bbStart, self.bbStart, self.bbStart], [self.bbEnd, self.bbEnd, self.bbEnd]])

    def getPtLoc(self):
        parent = self.parent
        rmInfo = parent.rmInfo
        if(self.freeAxesN == None): self.set2DAxes()
        return self.parent.snapper.get3dLocSnap(rmInfo, SnapParams(parent.snapper, lastCo1Axis = True, freeAxesN = self.freeAxesN, snapToPlane = (len(self.freeAxesN) == 2)))
    ...
    def procDrawEvent(self, context, event, snapProc):
        ...
        if(len(self.curvePts) > 0 and not snapProc):
            if(event.type in {'WHEELDOWNMOUSE', 'WHEELUPMOUSE', 'NUMPAD_PLUS', 'NUMPAD_MINUS','PLUS', 'MINUS'}):
                if(event.type in {'NUMPAD_PLUS', 'NUMPAD_MINUS', 'PLUS', 'MINUS'} and event.value == 'PRESS'): return {'RUNNING_MODAL'}
                self.updateSegCount(event, rmInfo, (event.type =='WHEELUPMOUSE' or event.type.endswith('PLUS')))
                return {'RUNNING_MODAL'}
            for i in range(len(self.dynamicParams)):
                hotkeys = self.dynamicParams[i][0]
                if(event.type in hotkeys):
                    if(event.value == 'RELEASE'): exec('self.updateParam' + str(i) + '(event, rmInfo, (event.type == hotkeys[0]))')
                    return {'RUNNING_MODAL'}
        ...
        if(not snapProc and event.type == 'LEFTMOUSE' and event.value == 'RELEASE'):
            if(self.bbStart == None):
                ...
                self.bbStart = loc
                self.bbEnd = loc
                self.setCurvePts([[loc, loc, loc, 'FREE', 'FREE'], [loc, loc, loc, 'FREE', 'FREE']])
            else:
                loc = self.getPtLoc()
                self.bbEnd = loc
                self.updateCurvePts()
    ...
    def getRefLine(self): #Reference point for restrict angle or lock axis
        if(self.bbStart != None):
            if(self.bbEnd != None): return[self.bbStart, self.bbEnd]
            else: return [self.bbStart]
        return []
...
class MathFnDraw(Primitive2DDraw):
    startPrefix = 'mathFnStart_' # Prefixes for param names generated dynamically for constants
    incrPrefix = 'mathFnIncr_'
    mathFnFileExt = 'mfn'
    defFnType = 'XY' # Default values
    defFNRes = 10
    ...
    defConstStart = 1
    defConstIncr = 0.1
    xDocTag = "drawMathFn" # XML tags / attributes
    xFnName = 'name'
    xFnDescr = 'descr'
    ...
    def isInverted(expr): # var = 'x' or 'y' # Returns None in case of invalid function
    ...
    def getEvaluatedExpr(expr):
        params = bpy.context.window_manager.bezierToolkitParams
        for j in range(Primitive2DDraw.getParamCnt()): expr = expr.replace(str(chr(ord('A') + j)), str(round(eval('params.'+ MathFnDraw.startPrefix + str(j)), 4)))
        return expr
    ...
    def getShapePts(self, mode, numSegs, bbStart, bbEnd, center2d, startAngle, \
        ...
        if(params.mathFnType == 'PARAMETRIC'):
            ...
            if(mode == 'CENTER'): # Let the plot be always centered
            ...
            if(params.drawMathTMapTo in {'X', 'HORIZONTAL'}): span = scaleFact * xSpan
            elif(params.drawMathTMapTo in {'Y', 'VERTICAL'}): span = scaleFact * ySpan
            else: span = scaleFact * sqrt(xSpan * xSpan + ySpan * ySpan)
            ...
            if(inverted == None): return curvePts
            span = (bbEnd[idx1] - bbStart[idx1]) if(inverted) else (bbEnd[idx0] - bbStart[idx0])
            intervals = int(self.shapeSegCnt * abs(span))
            if(intervals == 0): intervals = self.shapeSegCnt
            incr = span / intervals
            ...
            for step in range(intervals):
                try:
                    ...
                    dep = eval(expr)
                    if(abs(dep) > clip): dep = clip * (dep / abs(dep))
                    if(inverted): x = dep + bbStart[idx0] + (center2d.real if(mode == 'CENTER') else 0)
                    else: y = dep + bbStart[idx1] + (center2d.imag if(mode == 'CENTER') else 0)
                    pt2d = complex(x, y)
                    pt = get3DVector(pt2d, axisIdxs, z)
                    curvePts.append([pt, pt, pt, 'VECTOR', 'VECTOR'])
                except Exception as e:
    ...
    def getMathFnTxts():
        ...
        fnTxts = None
        params = bpy.context.window_manager.bezierToolkitParams
        tool = bpy.context.workspace.tools.from_space_view3d_mode('OBJECT', create = False) # TODO: Should be somewhere else
        if(tool.idname == FlexiDrawBezierTool.bl_idname and params.drawObjType == 'MATH'):
            if(params.mathFnType == 'PARAMETRIC'):
                fn1 = MathFnDraw.getEvaluatedExpr(params.drawMathFnParametric1)
                fn2 = MathFnDraw.getEvaluatedExpr(params.drawMathFnParametric2)
                fnTxts = ['y = ' + (fn2 if MathFnDraw.testFn(fn2, 't') else INVALID), 'x = '+ (fn1 if MathFnDraw.testFn(fn1, 't') else INVALID)]
            else:
                expr = MathFnDraw.getEvaluatedExpr(params.drawMathFn)
                inverted = MathFnDraw.isInverted(expr)
                if(inverted == None): fnTxts = [INVALID]
                elif(inverted): fnTxts = ['x = ' + expr]
                else: fnTxts = ['y = ' + expr]
        return fnTxts
    ...
    def getMathFnFolder(create = True):
        ...
        if(create and not os.path.isdir(mathFnFolder)): os.makedirs(mathFnFolder)
        return mathFnFolder
    ...
    def getMathFnList(dummy1 = None, dummy2 = None):
        if(MathFnDraw.mathFnItems == None or MathFnDraw.mathFnDirty):
            ...
            fNames = sorted([fName for fName in os.listdir(mathFnFolder) if fName.endswith('.' + MathFnDraw.mathFnFileExt)], key=lambda s: s.lower())
    ...
    def refreshDefaultParams():
        ...
        for i in range(Primitive2DDraw.getParamCnt()):
            char = chr(ord('A') + i)
            exec('params.' + MathFnDraw.startPrefix + str(i) + ' = ' + str(MathFnDraw.defConstStart))
            exec('params.' + MathFnDraw.incrPrefix + str(i) + ' = ' + str(MathFnDraw.defConstIncr))
    ...
    def refreshParamsFromFile(dummy1 = None, dummy2 = None):
        ...
        with open(filepath) as f: doc = minidom.parse(f)
...
class ResetMathFn(Operator):
    ...
    def execute(self, context):
        try: MathFnDraw.refreshParamsFromFile()
        except: MathFnDraw.refreshDefaultParams()
        return {'FINISHED'}

class SaveMathFn(Operator): # TODO: Better validation and error handling
    ...
    def execute(self, context):
        ...
        if(fnName.strip() == ''): return {'FINISHED'}  # Error Condition #1
        ...
        if(fnType == 'PARAMETRIC'):
            ...
            if(paramFn1.strip() == '' or paramFn2.strip() == ''): return {'FINISHED'} # Error Condition #2
            ...
            if(xyFn.strip() == ''): return {'FINISHED'}  # Error Condition #3
        ...
        try:
            with open(fnFile,"w") as f: doc.writexml(f)
            MathFnDraw.mathFnDirty = True
            params.mathFnList = fnName
        except: self.report({'ERROR'}, 'Error saving math function file') # Error Condition #4
        return {'FINISHED'}

class LoadMathFn(Operator): # TODO: Better validation and error handling
    ...
    def execute(self, context):
        params = bpy.context.window_manager.bezierToolkitParams
        try:
            with open(self.filepath) as f: doc = minidom.parse(f)
            fnName = doc.documentElement.getAttribute(MathFnDraw.xFnName)
            mathFnFolder = MathFnDraw.getMathFnFolder()
            ...
            MathFnDraw.mathFnDirty = True
            params.mathFnList = fnName
        except: self.report({'ERROR'}, 'Error importing math function file')
        return {'FINISHED'}
...
class DeleteMathFn(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        if(fnName != MathFnDraw.mathFnNoSelItem[0]):
            ...
            except: self.report({'ERROR'}, 'Error deleting math function file')
        return {'FINISHED'}

    def invoke(self, context, event):
        params = bpy.context.window_manager.bezierToolkitParams
        fnName = params.mathFnList
        if(fnName != MathFnDraw.mathFnNoSelItem[0]): return context.window_manager.invoke_props_dialog(self)
    ...
    def getShapePts(self, mode, numSegs, bbStart, bbEnd, center2d, startAngle, theta, axisIdxs, z):
        ...
        for segCnt in range(numSegs + 1):
            if(self.star):
                ...
                shift = complex(offset * radius * cos(radians(angle)), offset * radius * sin(radians(angle)))
...
class EllipseDraw(ClosedShapeDraw):
    ...
    def getPtAtAngle(a, b, theta): # https://math.stackexchange.com/questions/22064/calculating-a-point-that-lies-on-an-ellipse-given-an-angle
    ...
    def getShapePts(self, mode, numSegs, bbStart, bbEnd, center2d, startAngle, \
        ...
        endPts = [pt1, pt2]
        segs1 = getSegsForArc(endPts[startIdx], radius, 1, endPts[endIdx], 10, axisIdxs, z)
        endPts = [pt2, pt3]
        segs2 = getSegsForArc(endPts[startIdx], radius, 1, endPts[endIdx], 10, axisIdxs, z)
        ...
        vertCos = getInterpolatedVertsCo(pts, numSegs)
        newSegs = [] # TODO: A more efficient approach for dividing ellipse uniformly
        for i in range(1, len(vertCos)):
            ...
            segs1 = getSegsForArc(endPts[startIdx], radius, sweep, endPts[endIdx], 1, axisIdxs, z)
            newSegs += segs1
...
class BezierDraw(BaseDraw):
    ...
    def resetHandle(self, handle):
        if(len(self.curvePts) > 0): self.moveBptElem(handle, self.curvePts[-1][1])
    ...
    def procDrawEvent(self, context, event, snapProc):
        ...
        metakeys = snapper.getMetakeys()
        if(self.capture and FTHotKeys.isHotKey(FTHotKeys.hkGrabRepos, event.type, metakeys)):
            if(event.value == 'RELEASE'):
                self.dissociateHdl = False
                self.grabRepos = not self.grabRepos
            return {"RUNNING_MODAL"}
        if(self.capture and FTHotKeys.isHotKey(FTHotKeys.hkDissociateHdl, event.type, metakeys)):
            if(event.value == 'RELEASE'):
                self.grabRepos = False
                self.dissociateHdl = not self.dissociateHdl
            return {"RUNNING_MODAL"}
        if (snapProc and snapper.digitsConfirmed): # This can happen only when space was entered and something was there for Snapper to process
            snapper.resetSnap()
            snapper.digitsConfirmed = True # Because resetSnap sets this to False (TODO: Refactor resetSnap)
            if(not self.capture): # First space / enter is equivalent to mouse press without release
                self.capture = True
                snapper.setStatus(rmInfo.area, None)
                return {'RUNNING_MODAL'}
            else: snapProc = False # Second space / enter means it should be processed here, set snapProc to False so this modal will process it
        if(not snapProc and event.type == 'ESC'):
            if(event.value == 'RELEASE'):
                if(self.grabRepos): self.grabRepos = False
                elif(self.dissociateHdl): self.dissociateHdl = False
                elif(self.capture and self.isHandleSet()):
                    self.resetHandle('left')
                    self.resetHandle('right')
                    snapper.digitsConfirmed = True # Needed to indicate next space / entered to be processed here
                    snapper.setStatus(rmInfo.area, None)
                else: self.parent.initialize() # TODO: should not access parent.initialize
                self.parent.redrawBezier(rmInfo)
            return {'RUNNING_MODAL'}
        if(not snapProc and FTHotKeys.isHotKey(FTHotKeys.hkResetLastHdl, event.type, metakeys)):
            if(event.value == 'RELEASE'):
                if(len(self.curvePts) > 1):
                    ...
                    self.parent.redrawBezier(rmInfo)
            return {'RUNNING_MODAL'}
        if(not snapProc and FTHotKeys.isHotKey(FTHotKeys.hkUndoLastSeg, event.type, metakeys)):
            if(event.value == 'RELEASE'):
                snapper.resetSnap()
                if(not self.capture):
        ...
        if(not snapProc and event.type == 'LEFTMOUSE' and event.value == 'PRESS'):
            if(len(self.curvePts) == 0):
                loc = snapper.get3dLocSnap(rmInfo)
                self.newPoint(loc, 'ALIGNED', 'ALIGNED')
            if(len(snapper.freeAxes) == 1 and len(self.curvePts) > 1): snapper.resetSnap() # Special condition for hot-key single axis lock (useful)
        ...
        if (not snapProc and event.type == 'LEFTMOUSE' and event.value == 'RELEASE'):
            # if(snapper.isLocked()):
                # if(len(self.curvePts) == 1): self.moveBptElem('right', snapper.get3dLocSnap(rmInfo))# changes only rt handle
                # return {'RUNNING_MODAL'}
            # if(len(snapper.freeAxes) > 1): # See the special condition above in event.value == 'PRESS'
            snapper.resetSnap()
            self.reset()
            if(len(self.curvePts) == 0): self.parent.updateSnapLocs() # Rare condition: This happens e. g. when user clicks on header menu # like Object->Transform->Move. These ops consume press event but not release. So update the snap locations anyways if there was some transformation# Subclass (TODO: have a relook)
                ...
                else: loc = snapper.get3dLocSnap(rmInfo)
                # if(len(self.curvePts) == 1): self.moveBptElem('right', loc)# changes only rt handle
                self.newPoint(loc, 'ALIGNED', 'ALIGNED')
                self.parent.redrawBezier(rmInfo)
            return {'RUNNING_MODAL'}        
        if (snapProc or event.type == 'MOUSEMOVE'): # Refresh also in case of snapper events # except when digitsConfirmed (to give user opportunity to draw a straight line) # if ((snapProc and not snapper.digitsConfirmed) \
            ...
            if(len(self.curvePts) > 0):
                if(self.capture):
                    ...
                    if(self.grabRepos):
                        ...
                        loc = snapper.get3dLocSnap(rmInfo, SnapParams(snapper, xyDelta = [xy1[0] - xy2[0], xy1[1] - xy2[1]]))
                        delta = loc - lastPt[1]
    ...
    def getRefLine(self):
        if(len(self.curvePts) > 0):
            ...
            elif(len(self.curvePts) > 1): idx = -2 # There should always be min 2 pts if not capture, check anyway
            if((len(self.curvePts) + (idx - 1)) >= 0): return[self.curvePts[idx-1][1], self.curvePts[idx][1]]
            else: return[self.curvePts[idx][1]]
        return []
    def getRefLineOrig(self):
        refLine = self.getRefLine()
...
class ModalDrawBezierOp(ModalBaseFlexiOp):
    markerSize = 8 # Static members shared by flexi draw and flexi grease
    h = False
    drawObjMap = {}

    def drawHandler(): #static method
    ...
    def updateDrawSides(dummy, context):
        params = bpy.context.window_manager.bezierToolkitParams
        opObj = ModalDrawBezierOp.opObj
        if(opObj != None and opObj.drawType != 'BEZIER'): opObj.drawObj.shapeSegCnt = params.drawSides
    ...
    def preInvoke(self, context, event):
        ...
        ModalDrawBezierOp.drawObjMap = {'BEZIER': self.bezierDrawObj, 
    ...
    def initialize(self): #This will be called multiple times not just at the beginning
    ...
    def subInvoke(self, context, event):
        ...
        try: ModalDrawBezierOp.markerSize = context.preferences.addons[__name__].preferences.markerSize
        except Exception as e: ModalDrawBezierOp.markerSize = 8 # print("BezierUtils: Error fetching default sizes in Draw Bezier", e)
    ...
    def confirm(self, context, event, location = None):
        ...
        autoclose = (self.drawType == 'BEZIER' and shift and (event.type == 'SPACE' or event.type == 'RET'))
    ...
    def baseSubModal(self, context, event, snapProc): # Common subModal for Flexi Draw and Flexi Grease
    ...
    def refreshMarkerPos(self, rmInfo):
        ...
        self.resetDisplay()
        self.bglDrawMgr.addPtInfo('drawMarker', ModalDrawBezierOp.markerSize, [colMarker], [markerLoc])
        ModalBaseFlexiOp.refreshDisplayBase(segDispInfos = [], bptDispInfos = [], snapper = self.snapper)
    ...
    def redrawBezier(self, rmInfo, lastSegOnly = False, hdlPtIdxs = None, hltEndSeg = True):
        ...
        segColor = colSelSeg
        tipColors = [colTip, colEndTip, colTip] if (not ModalDrawBezierOp.h) else [None, colEndTip, None]
        segDispInfos = []
        bptDispInfos = []
        if(hdlPtIdxs == None): hdlPtIdxs = {ptCnt - 2} # Default last but one
        elif(len(hdlPtIdxs) == 0): hdlPtIdxs = range(ptCnt)
        for hdlPtIdx in hdlPtIdxs: bptDispInfos.append(BptDisplayInfo(curvePts[hdlPtIdx], tipColors, handleNos = [0, 1] if (not ModalDrawBezierOp.h) else []))
        startIdx = 0
        if(lastSegOnly and ptCnt > 1): startIdx = ptCnt - 2
...
class ModalFlexiDrawBezierOp(ModalDrawBezierOp):
    ...
    def updateAfterGeomChange(self, scene = None, dummy = None): # 3 params in 2.81 # For some curve-changing ops (like reset rotation); possible in draw
        self.updateSnapLocs()

    def isToolSelected(self, context):
        if(context.mode != 'OBJECT'): return False
        tool = context.workspace.tools.from_space_view3d_mode('OBJECT', create = False)
        if(tool == None or tool.idname != FlexiDrawBezierTool.bl_idname): return False # if(tool == None or tool.idname != 'flexi_bezier.draw_tool'):
        return True
    ...
    def preInvoke(self, context, event):
        super(ModalFlexiDrawBezierOp, self).preInvoke(context, event) # If the operator is invoked from context menu, enable the tool on toolbar
        if(not self.isToolSelected(context) and context.mode == 'OBJECT'): bpy.ops.wm.tool_set_by_id(name = FlexiDrawBezierTool.bl_idname) # bpy.ops.wm.tool_set_by_id(name = 'flexi_bezier.draw_tool')
        self.snapInfos = {} # Object name -> [spline index, [pts]] # Not used right now (maybe in case of large no of curves)
        self.updateSnapLocs()
    ...
    def subModal(self, context, event, snapProc):
        ...
        if(FTHotKeys.isHotKey(FTHotKeys.hkToggleDrwEd, event.type, metakeys)):
            if(event.value == 'RELEASE'): bpy.ops.wm.tool_set_by_id(name = FlexiEditBezierTool.bl_idname) # bpy.ops.wm.tool_set_by_id(name = 'flexi_bezier.edit_tool')
            return {"RUNNING_MODAL"}
    ...
    def getSnapLocsImpl(self):
        ...
        for info in infos: locs += info[1]
        if(len(self.drawObj.curvePts) > 0): locs += [pt[1] for pt in self.drawObj.curvePts[:-1]] # locs += [self.curvePts[-1][0], self.curvePts[-1][1], self.curvePts[-1][2]]
        return locs

    def updateSnapLocs(self, objNames = None):
        updateCurveEndPtMap(self.snapInfos, addObjNames = objNames)

    def createCurveObj(self, context, startObj = None, startSplineIdx = None, endObj = None, endSplineIdx = None, autoclose = False):
        collection = context.collection # First create the new curve
        if(collection == None): collection = context.scene.collection
        obj = createObjFromPts(self.drawObj.curvePts, '3D', collection, autoclose)
        if(endObj != None or startObj != None): # Undo stack in case the user does not want to join
            obj.select_set(True) # bpy.context.view_layer.objects.active = obj
            bpy.ops.ed.undo_push()
        else: return obj
        endObjs = []
        splineIdx = endSplineIdx # Connect the end curve (if exists) first
        if(endObj != None and startObj != endObj):
            endObjs, changeCnt = splitCurve([endObj], split = 'spline', newColl = False) # first separate splines
            obj = joinSegs([endObjs[splineIdx], obj], optimized = True, straight = False, srcCurve = endObjs[splineIdx]) # then join the selected spline from end curve with new obj
            endObjs[splineIdx] = obj
            objComps = endObjs #Use this if there is no start curve
        if(startObj != None):
            startObjs, changeCnt = splitCurve([startObj], split = 'spline', newColl = False) # Repeat the above process for start curve
            obj = joinSegs([startObjs[startSplineIdx], obj], optimized = True, straight = False, srcCurve = startObjs[startSplineIdx])
            if(startObj == endObj and startSplineIdx != endSplineIdx):
                obj = joinSegs([startObjs[endSplineIdx], obj], optimized = True, straight = False, srcCurve = startObjs[endSplineIdx]) # If startSplineIdx == endSplineIdx the join call above would take care # but if they are different they need to be joined with a separate call
                startObjs.pop(endSplineIdx) # can't replace the elem with new obj as in case of end curve # (see the seq below)
                if(endSplineIdx < startSplineIdx): startSplineIdx -= 1            
            objComps = startObjs[:startSplineIdx] + endObjs[:splineIdx] + [obj] + endObjs[(splineIdx + 1):] + startObjs[(startSplineIdx + 1):] # Won't break even if there were no endObjs
        obj = joinCurves(objComps)
        if(any(p.co.z != 0 for s in obj.data.splines for p in s.bezier_points)): obj.data.dimensions = '3D'
        return obj

    def getSnapObjs(self, context, locs): #TODO: At least store in map instead of linear search
        retVals = [[None, 0, 0]] * len(locs)
        foundVals = 0
        for obj in bpy.data.objects:
            if(cop.is_bezier(obj)):
                mw = obj.matrix_world
                for i, s in enumerate(obj.data.splines):
                    ...
                    for j, loc in enumerate(locs):
                        ...
                        if(vectCmpWithMargin(loc, mw @ p.co)):
                            retVals[j] = [obj, i, -1]
                            foundVals += 1
                        if(foundVals == len(locs)): return retVals
        return retVals

    def save(self, context, event, autoclose, location, align = True):
        curvePts = self.drawObj.curvePts
        if(len(curvePts) > 1):
            startObj, startSplineIdx, ptIdx2, endObj, endSplineIdx, ptIdx1 = [x for y in self.getSnapObjs(context, [curvePts[0][1], curvePts[-1][1]]) for x in y]
            ctrl = self.snapper.gridSnap # Overloaded key op            
            if(ctrl and endObj != None): obj = self.createCurveObj(context, autoclose = False) # ctrl pressed and there IS a snapped end obj, # so user does not want connection # (no option to only connect to starting curve when end object exists)
            else:
                startObjName = startObj.name if(startObj != None) else ''
                endObjName = endObj.name if(endObj != None) else ''
                obj = self.createCurveObj(context, startObj, startSplineIdx, endObj, endSplineIdx, autoclose)
...
class ModalFlexiDrawGreaseOp(ModalDrawBezierOp): ################### Flexi Draw Grease Bezier ###################
    ...
    def isToolSelected(self, context):
        if(context.mode != 'PAINT_GPENCIL'): return False
        tool = context.workspace.tools.from_space_view3d_mode('PAINT_GPENCIL', create = False)
        if(tool == None or tool.idname != FlexiGreaseBezierTool.bl_idname): return False # if(tool == None or tool.idname != 'flexi_bezier.grease_draw_tool'):
        return True
    ...
    def preInvoke(self, context, event):
        super(ModalFlexiDrawGreaseOp, self).preInvoke(context, event) # If the operator is invoked from context menu, enable the tool on toolbar
        if(not self.isToolSelected(context) and context.mode == 'PAINT_GPENCIL'): bpy.ops.wm.tool_set_by_id(name = FlexiGreaseBezierTool.bl_idname) # bpy.ops.wm.tool_set_by_id(name = 'flexi_bezier.grease_draw_tool')
        o = context.object
    ...
    def redrawBezier(self, rmInfo, hdlPtIdxs = None, hltEndSeg = True): # overridden
        curvePts = self.drawObj.curvePts
        ptCnt = len(curvePts)
        subdivCos = self.subdivCos if ptCnt > 1 else []
        self.bglDrawMgr.addLineInfo('gpSubdivLines', FTProps.lineWidth, [FTProps.colGreaseNonHltSeg], getLinesFromPts(subdivCos))
        if(ModalFlexiDrawGreaseOp.h): self.bglDrawMgr.resetPtInfo('gpSubdivPts')
        else: self.bglDrawMgr.addPtInfo('gpSubdivPts', FTProps.greaseSubdivPtSize, [FTProps.colGreaseSubdiv], subdivCos)
        if(self.drawType != 'BEZIER' and len(curvePts) > 0): ModalBaseFlexiOp.refreshDisplayBase(segDispInfos = [], bptDispInfos = [], snapper = self.snapper)
        else: super(ModalFlexiDrawGreaseOp, self).redrawBezier(rmInfo, lastSegOnly = True, hdlPtIdxs = {ptCnt-1}, hltEndSeg = hltEndSeg)
    ...
    def subModal(self, context, event, snapProc):
        ...
        if(not metakeys[2]):
            ...
            if(self.drawType in {'BEZIER', 'ELLIPSE'} and event.type in {'WHEELDOWNMOUSE', 'WHEELUPMOUSE', 'NUMPAD_PLUS', 'NUMPAD_MINUS','PLUS', 'MINUS'} and len(curvePts) > 1):
                if(event.type in {'NUMPAD_PLUS', 'NUMPAD_MINUS', 'PLUS', 'MINUS'}  and event.value == 'PRESS'): return {'RUNNING_MODAL'}
                elif(event.type =='WHEELUPMOUSE' or event.type.endswith('PLUS')): self.subdivAdd(cntIncr)
                elif(event.type =='WHEELDOWNMOUSE' or event.type.endswith('MINUS')): self.subdivAdd(-cntIncr)
        ...
        if(len(curvePts) > 0): # if(newPtCnt - ptCnt != 0):
    ...
    def getCurveSegLens(self):
        clen = []
        curvePts = self.drawObj.curvePts
        for i in range(1, len(curvePts) - 1): clen.append(getSegLen([curvePts[i-1][1], curvePts[i-1][2], curvePts[i][0], curvePts[i][1]]))
        return clen

    def updateSubdivCos(self, clen = None):
        if(self.drawType in {'POLYGON', 'STAR', 'RECTANGLE'}): self.subdivCos = [p[0] for p in self.drawObj.curvePts]
        elif(self.interpPts != []):
    ...
    def updateInterpPts(self, slens):
        curvePts = self.drawObj.curvePts[:] if(self.drawType != 'BEZIER') else self.drawObj.curvePts[:-1]
        ...
        for gpencil in gpencils:
            ...
            for layer in gpencil.data.layers:
                for f in layer.frames:
                    for s in f.strokes:
                        if(len(s.points) > 0): self.snapLocs += [mw @ s.points[0].co, mw @ s.points[-1].co] # Shouldn't be needed, but anyway...

    def save(self, context, event, autoclose, location):
        layer = self.gpencil.data.layers.active
        if(layer == None): layer = self.gpencil.data.layers.new('GP_Layer', set_active = True)
        if(len(layer.frames) == 0): layer.frames.new(0)
...
class EditSegDisplayInfo(SegDisplayInfo): ################### Flexi Edit Bezier Curve ###################
...
def getBptData(obj, withShapeKey = True, shapeKeyIdx = None, fromMix = True, updateDeps = False, local = False): # fromMix True: points after shape key value / eval_time applied
    worldSpaceData = [] # Less readable but more convenient than class # Format: [handle_left, co, handle_right, handle_left_type, handle_right_type]
    ...
    if(withShapeKey and shapeKey != None):
        if(shapeKeyIdx == None): shapeKeyIdx = obj.active_shape_key_index
        if(fromMix):
            if(not obj.data.shape_keys.use_relative): val = obj.data.shape_keys.eval_time
            else: val = obj.data.shape_keys.key_blocks[obj.active_shape_key_index].value
            if(floatCmpWithMargin(val, 0)): keyBlock = obj.data.shape_keys.key_blocks[0]
            else:
                tmpsk = obj.shape_key_add(name = 'tmp', from_mix = True)
                keyBlock = obj.data.shape_keys.key_blocks[tmpsk.name]
        else: keyBlock = obj.data.shape_keys.key_blocks[shapeKeyIdx]
...
def getAdjIdx(obj, splineIdx, startIdx, offset = 1, ptCnt = None): #TODO: splineIdx not needed if ptCnt given
    spline = obj.data.splines[splineIdx]
    if(ptCnt == None): ptCnt = len(spline.bezier_points)
    if(not spline.use_cyclic_u and ((startIdx + offset) >= ptCnt or (startIdx + offset) < 0)): return None
...
def getSegPtsInSpline(wsData, splineIdx, ptIdx, cyclic):
    ...
    if(ptIdx < (len(splinePts) - 1) ): ptRange = [ptIdx, ptIdx + 1]
        ...
        ptRange = [-1, 0]
        if(splinePts[-1][4] == 'VECTOR'): splinePts[-1][2] = (splinePts[-1][1] + 1/3 * (splinePts[-1][1] - splinePts[0][1]))
        if(splinePts[0][3] == 'VECTOR'): splinePts[0][0] = (splinePts[0][1] + 1/3 * (splinePts[0][1] - splinePts[-1][1]))
    else: return []
...
def getClosestPt2dWithinSeg(region, rv3d, coFind, selObj, selSplineIdx, selSegIdx, withHandles, withBezPts): # Wrapper for spatial search within segment
    infos = {selObj: {selSplineIdx:[[selSegIdx],[]]}}    
    return getClosestPt2d(region, rv3d, coFind, [selObj], infos, withHandles, withBezPts, withObjs = False, maxSelObjRes = MAX_SEL_CURVE_RES) # set selObj in objs for CurveBezPts

def getClosestPt2d(region, rv3d, coFind, objs, selObjInfos, withHandles = True, withBezPts = True, withObjs = True, maxSelObjRes = MAX_NONSEL_CURVE_RES, withShapeKey = True):
    objLocMap = {}
    ...
    objBezPtCounts = []
    objSplineEndPts = []
    for obj in objs: #TODO: Check of shape key bounding box
        if(obj.active_shape_key == None and not isPtIn2dBBox(obj, region, rv3d, coFind, FTProps.snapDist)): continue
        wsDataSK = None
        wsData = getBptData(obj, fromMix = True, updateDeps = True) # Curve data with shape key value applied (if shape key exists)
        if(withShapeKey and obj.active_shape_key != None): wsDataSK = getBptData(obj, fromMix = False) # active shape key data with value = 1
        for i, spline in enumerate(obj.data.splines):
            for j, pt in enumerate(spline.bezier_points):
                objLocList.append([obj, i, j])
                if(withObjs):
                    interpLocs = getInterpSegPts(wsData, i, j, spline.use_cyclic_u, res = SEARCH_CURVE_RES, maxRes = MAX_NONSEL_CURVE_RES)[1:-1]
                    if(wsDataSK != None): interpLocs += getInterpSegPts(wsDataSK, i, j, spline.use_cyclic_u, res = SEARCH_CURVE_RES, maxRes = MAX_NONSEL_CURVE_RES)[1:-1]
    ...
    for selObj in selObjInfos.keys():
        wsDataSK = None
        wsData = getBptData(selObj, fromMix = True, updateDeps = True) # Curve data with shape key value applied (if shape key exists)
        if(withShapeKey and selObj.active_shape_key != None): wsDataSK = getBptData(selObj, fromMix = False) # active shape key data with value = 1
        ...
        for splineIdx in info.keys():
            ...
            for segIdx in segIdxs:
                ...
                interpLocs = getInterpSegPts(wsData, splineIdx, segIdx, cyclic, res = SEARCH_CURVE_RES * 5, maxRes = maxSelObjRes)[1:-1]
                if(wsDataSK != None): interpLocs += getInterpSegPts(wsDataSK, splineIdx, segIdx, cyclic, res = SEARCH_CURVE_RES * 5, maxRes = maxSelObjRes)[1:-1]
    ...
    searchPtsList = [[], [], [], [], [], []]
    retStr = [[], [], [], [], [], []]
    searchPtsList[0], retStr[0] = hdls, 'SelHandles' #'SelHandles', 'SegLoc', 'CurveBezPt', 'CurveLoc'
    ...
    searchPtsList = [[getCoordFromLoc(region, rv3d, pt).to_3d() for pt in pts] for pts in searchPtsList] # TODO: Remove duplicates before sending for search?
    srs = NestedListSearch(searchPtsList).findInLists(coFind, searchRange = FTProps.snapDist)
    if(len(srs) == 0): return None
    sr = min(srs, key = lambda x: x[3])
    if(sr[0] > 1): sr = min(srs, key = lambda x: (x[0], x[3])) # If seg loc then first priority to the nearby handle, end pt (even if farther)
    idx = sr[1]
    retId = retStr[sr[0]]
...
class NestedListSearch:
    def findListIdx(counts, idx): # Find the list element containing the given idx from flattened list # return the index of the list element containing the idx
        cumulCnt = 0
        cntIdx= 0
        while(idx >= cumulCnt):
...
class SelectCurveInfo:
    def __init__(self, obj, splineIdx):
        ...
        self.updateWSData()
        self.ptSels = {} # User Selection (mouse click); format ptIdx: set(sel)... # where sel: -1->seg, 0->left hdl, 1->bezier pt, 2->right hdl
        self.hltInfo = {} # Highlighted point (mouse move) #'ptIdx': ptIdx, 'hltIdx':hltIdx {-1, 0, 1, 2} (just as in sel above)
        self.objName = obj.name # obj.name gives exception if obj is not in bpy.data.objects collection, so keep a copy
        self.interpPts = {}
        self.clickInfo = {} # Format 'ptIdx': segIdx, 'hdlIdx': hdlIdx, 'loc':loc, 't':t # hdlIdx - {-1, 0, 1, 2} similar to sel in ptSels
    ...
    def updateWSData(self):
        ...
        self.keyStartIdx = sum(len(self.obj.data.splines[i].bezier_points) for i in range(self.splineIdx)) # for shape keys
        self.wsData = getBptData(self.obj, fromMix = False)[self.splineIdx] # WS Data of the shape key (if exists)
    
    def getAdjIdx(self, ptIdx, offset = 1): # For convenience
    ...
    def getAllShapeKeysData(self, ptIdx):
        if(not self.hasShapeKey): return None
        pts = []
        for keyIdx in range(len(self.obj.data.shape_keys.key_blocks)): pts.append(self.getShapeKeyData(ptIdx, keyIdx))
        return pts
    ...
    def getAllPtsWithHdls(self): # All selected points which have handles displayed (for example for snapping)
    ...
    def setClickInfo(self, ptIdx, hdlIdx, clickLoc, lowerT = 0.001, higherT = .999):
        ...
        if(clickLoc != None):
            ...
            else: self.clickInfo = {'ptIdx': ptIdx, 'hdlIdx': hdlIdx, 'loc':clickLoc, 't':t}
        if(self.clickInfo == None): self.clickInfo = {'ptIdx': ptIdx, 'hdlIdx': hdlIdx}

    def addSel(self, ptIdx, sel, toggle = False):
        self.addSels(ptIdx, set([sel]), toggle)

    def addSels(self, ptIdx, sels, toggle = False): 
        if( -1 in sels and self.getAdjIdx(ptIdx) == None): sels.remove(-1) # TODO: Check this condition at other places
    ...
    def getSelCo(self):
        if(len(self.clickInfo) > 0):
            ...
            if(hdlIdx == -1): return self.clickInfo['loc']
            else:
        ...
        if(subdivCnt > 1):
            ...
            for ptIdx in sorted(self.ptSels.keys()):
                if(-1 in self.ptSels[ptIdx]):
                    vertCos = getInterpolatedVertsCo(self.interpPts[ptIdx], subdivCnt)[1:-1]
                    changedIdx = ptIdx + addCnt
                    insertBezierPts(self.obj, self.splineIdx, changedIdx, [invMw @ v for v in vertCos], 'FREE')
                    addCnt += len(vertCos)
        return addCnt > 0
    ...
    def initSubdivMode(self, rv3d):
        ...
        for ptIdx in self.ptSels.keys():
            if(-1 in self.ptSels[ptIdx]):
                self.interpPts[ptIdx] = getPtsAlongBezier3D(self.getSegPts(ptIdx), rv3d, curveRes = 1000, minRes = 1000)
                changed = True
        return changed
    ...
    def removeSegs(self): # Remove all selected segments # Returns map with spline index and seg index change after every seg removal
        changedSelMap = {}
        ...
        for segIdx in sorted(segSels):
            ...
            splineIdxIncr, segIdxIncr = removeBezierSeg(self.obj, changedSplineIdx, changedSegIdx)
    ...
    def alignHandle(self, ptIdx, hdlIdx, allShapekeys = False):
        ...
        bpt = self.getBezierPt(ptIdx)
        if(hdlIdx == 0 and bpt.handle_left_type != 'ALIGNED'): bpt.handle_left_type = 'FREE'
        if(hdlIdx == 2 and bpt.handle_right_type != 'ALIGNED'): bpt.handle_right_type = 'FREE'
        for pt in pts:
            if(hdlIdx == 0): pt.handle_left = pt.co - (pt.co - pt.handle_left).length * (pt.handle_right - pt.co).normalized()
            else: pt.handle_right = pt.co + (pt.co - pt.handle_right).length * (pt.co - pt.handle_left).normalized()
        return True
    ...
    def alignSelHandles(self):
        ...
        for ptIdx in self.ptSels:
            ...
            for hdlIdx in sels: changed = self.alignHandle(ptIdx, hdlIdx) or changed
        return changed

    def insertNode(self, handleType, select = True):
        if(self.hasShapeKey): return False
        invMw = self.obj.matrix_world.inverted_safe()
        insertBezierPts(self.obj, self.splineIdx, self.clickInfo['ptIdx'], [invMw @ self.clickInfo['loc']], handleType)
        return True
    ...
    def removeNode(self):
        ...
        for ptIdx in nodeSels: self.ptSels.pop(ptIdx)
        if(len(nodeSels) > 0):
    ...
    def getBevelPts(self, bevelCnt, pts, deltaPos):
        ...
        if(floatCmpWithMargin(deltaLen, DEF_ERR_MARGIN)): return pts, self.ptSels
        kFact = (bevelCnt/3) * (sqrt(2) - 1) # http://launchpadlibrarian.net/12692602/rcp.svg
        maxT = .5
        pts = [[p if isinstance(p, str) else p.copy() for p in pt] for pt in pts] # Deep copy
        newPts = []
        newSelPtIdxs = []
        bevelPtIdxs = set() # Add both points of the selected segments in selection
        for ptIdx in self.ptSels.keys():
        ...
        for ptIdx in bevelPtIdxs: # Extra loop because next points need to be determined beforehand
            if(ptSels.get(ptIdx) == None): ptSels[ptIdx] = {1}
            else: ptSels[ptIdx].add(1)
            nextIdx = self.getAdjIdx(ptIdx)
            prevIdx = self.getAdjIdx(ptIdx, -1)
            if(prevIdx != None and nextIdx != None and not hasAlignedHandles(pts[ptIdx])): newSelPtIdxs.append(ptIdx)
        ...
        for ptIdx, pt in enumerate(pts):
            if(ptIdx in newSelPtIdxs):
                ...
                diffV = (pt[1] - prevPt[1])
                segLen = diffV.length
                if(segLen < .001): newPts.append(pt)
                else:
                    t = deltaLen / segLen
                    if(t > maxT and (prevIdx in newSelPtIdxs)):
                        ...
                        t = 1
                        k = kFact * segLen
                    else: k = kFact * deltaLen
                    seg = [pts[prevIdx][1], pts[prevIdx][2], pt[0], pt[1]]
                    partialSeg = getPartialSeg(seg, t0 = 0, t1 = 1 - t)
                    newPt = partialSeg[3]
                    tangent0 = getTangentAtT(pts[prevIdx][1], pts[prevIdx][2], pt[0], pt[1], 1 - t)
                ...
                diffV = (nextPt[1] - pt[1])
                segLen = diffV.length
                if(floatCmpWithMargin(segLen, 0)): newPts.append(pt)
                else:
                    t = deltaLen / segLen
    ...
    def getDisplayInfos(self, hideHdls = False, subdivCnt = 0, bevelCnt = 0, newPos = None, deltaPos = None):
        cHltTip = FTProps.colHltTip # Making long short
        ...
        ptSels = self.ptSels
        if(newPos != None):
            nPtIdxs, nPts = self.getOffsetSegPts(newPos) # TODO: This method is in EditCurveInfo
            for i, ptIdx in enumerate(nPtIdxs): pts[ptIdx] = nPts[i] # Update list with new position (editing)
        elif(deltaPos != None): pts, ptSels = self.getBevelPts(bevelCnt, pts, deltaPos)
        for i, pt in enumerate(pts): # Default display of spline
            bptDispInfos.append(BptDisplayInfo(pt, [cAdjBezTip]))
            if(i > 0): segDispInfos.append(SegDisplayInfo([pts[i-1], pt], cNonHltSeg))
        lastIdx = self.getAdjIdx(len(self.wsData) - 1) # In case cyclic...
        if(lastIdx != None): segDispInfos.append(SegDisplayInfo([pts[-1], pts[0]], cNonHltSeg))
        hltInfo = self.getHltInfo()
        hltPtIdx = hltInfo.get('ptIdx')
        hltIdx = hltInfo.get('hltIdx')
        if(hltIdx == -1): # Process highlighted segments before selected ones because. selected segments take priority over highlighted
        ...
        for ptIdx in sorted(ptSels.keys()): # Process selections
            sels = ptSels[ptIdx]
            if(hideHdls):
            ...
            for hdlIdx in sorted(sels): # Start with seg selection i. e. -1
                if(hdlIdx == -1):
                    ...
                    if(nextIdx not in ptSels.keys()): # process next only if there are no selection pts with that idx
                        bptDispInfos[nextIdx].tipColors = tipColors[:]
                        bptDispInfos[nextIdx].handleNos = handleNos
                    vertCos = []
                    if(subdivCnt > 1): vertCos = getInterpolatedVertsCo(self.interpPts[ptIdx], subdivCnt)[1:-1]
                    selSegDispInfo = EditSegDisplayInfo(segPts, FTProps.colDrawSelSeg, vertCos)
                    segDispInfos[ptIdx] = selSegDispInfo
                elif(hdlIdx == 1 or not hideHdls): bptDispInfos[ptIdx].tipColors[hdlIdx] = FTProps.colSelTip
        if(hltIdx in {0, 1, 2}): bptDispInfos[hltPtIdx].tipColors[hltIdx] = cHltTip # Process highlighted points after selected ones because... highlighted points take priority over selected
...
class EditCurveInfo(SelectCurveInfo):
    def __init__(self, obj, splineIdx, ptSels = None):
        ...
        if(ptSels != None): self.ptSels = ptSels
    ...
    def setAlignedHdlsCo(self, pt, hdlIdx, ctrlPLoc):
        ...
        if(pt[typeIdx] == 'ALIGNED'):
            oppTypeIdx = 4 if hdlIdx == 0 else 3
            if(pt[oppTypeIdx] != 'VECTOR'): pt[hdlIdx] += (ctrlPLoc - pt[1])
            else: self.syncAlignedHdl(pt, ctrlPLoc, hdlIdx)

    def setFreeHdlsCo(self, pt, hdlIdx, newLoc):
        typeIdx = 3 if hdlIdx == 0 else 4
        if(pt[typeIdx] == 'FREE'): pt[hdlIdx] += (newLoc - pt[1])
    ...
    def setVectHdlsCo(self, pt, newLoc, hdlIdx, prevPt, nextPt):
        ...
        if(pt[typeIdx] == 'VECTOR'):
            ...
            if(diffV == None and pts[1] != None): diffV = newLoc - pts[1][1]
            if(diffV == None): pt[hdlIdx] = newLoc
            else: pt[hdlIdx] = newLoc + diffV * 1 / 3

    def syncCtrlPtHdls(self, ptIdx, newLoc): # Calculate both handle and adjacent pt handles in case of Vector type # TODO: AUTO has a separate logic set to ALIGNED for now
        ...
        for hdlIdx in [0, 2]: self.setVectHdlsCo(pt, newLoc, hdlIdx, prevPt, nextPt)
        for hdlIdx in [0, 2]: self.setFreeHdlsCo(pt, hdlIdx, newLoc)
        for hdlIdx in [0, 2]: self.setAlignedHdlsCo(pt, hdlIdx, newLoc)
        pt[1] = newLoc
    ...
    def syncHdls(self, pt, hdlIdx, newLoc): # Calculate the opposite handle values in case of ALIGNED and AUTO handles # Also set the type(s) of current (opposite) handle(s)
        typeIdx = 3 if hdlIdx == 0 else 4
        oppTypeIdx = 4 if hdlIdx == 0 else 3
    ...
    def getOffsetSegPts(self, newLoc): # Get seg points after change in position of handles or drag curve # The only function called on all 3 events: grab curve pt, grab handle, grab Bezier pt
        inf = self.clickInfo
        ptIdx = inf['ptIdx']
        hdlIdx = inf['hdlIdx']
        wsData = getBptData(self.obj, fromMix = False)
        ...
        if(hdlIdx == -1): # Grab point on curve
            ...
            if(delta == 0): return ptIdxs, pts
            t = inf['t']
            #****************************************************************              #*
            # Magic Bezier Drag Equations (Courtesy: Inkscape)             #*              #*
            #****************************************************************              #*
            if (t <= 1.0 / 6.0):    weight = 0                                             #*
            elif (t <= 0.5):        weight = (pow((6 * t - 1) / 2.0, 3)) / 2               #*
            elif (t <= 5.0 / 6.0):  weight = (1 - pow((6 * (1-t) - 1) / 2.0, 3)) / 2 + 0.5 #*
            else:                   weight = 1                                             #*
            offset0 = ((1 - weight) / (3 * t * (1 - t) * (1 - t))) * delta                 #*
            offset1 = (weight / (3 * t * t * (1 - t))) * delta                             #*
                                                                                           #*
            #****************************************************************
            pts[0][2] += offset0 # If the segment is edited, the 1st pt right handle...
            if(pts[0][4] == 'VECTOR'): pts[0][4] = 'FREE'
            if(pts[0][4] == 'AUTO'): pts[0][4] = 'ALIGNED'
            if(pts[0][3] == 'VECTOR' and pts[0][4] != 'FREE'): pts[0][3] = 'FREE' # opposite handle must be changed if this is not FREE
            if(pts[0][3] == 'AUTO' and pts[0][4] != 'FREE'): pts[0][3] = 'ALIGNED'
            self.syncAlignedHdl(pts[0], pts[0][1], hdlIdx = 0)
            pts[1][0] += offset1 # ...and 2nd pt left handle impacted
            if(pts[1][3] == 'VECTOR'): pts[1][3] = 'FREE'
            if(pts[1][3] == 'AUTO'): pts[1][3] = 'ALIGNED'
            if(pts[1][4] == 'VECTOR' and  pts[1][3] != 'FREE'): pts[1][4] = 'FREE' # opposite handle must be changed if this is not FREE
            ...
            return [ptIdx], [pt]
        else: return self.syncCtrlPtHdls(ptIdx, newLoc) # Grab the Bezier point
        ...
        if(self.hasShapeKey):
            for i, ptIdx in enumerate(ptIdxs):
                ...
                for idx in impIdxs:
                    ...
                    if(spline.bezier_points[idx].handle_left_type == 'AUTO'): spline.bezier_points[idx].handle_left_type = 'ALIGNED'
                    if(spline.bezier_points[idx].handle_right_type == 'AUTO'): spline.bezier_points[idx].handle_right_type = 'ALIGNED'
        else:
            for i, bpt in enumerate(bpts):
...
class ModalFlexiEditBezierOp(ModalBaseFlexiOp):
    ...
    def refreshDisplay(segDispInfos, bptDispInfos, locOnCurve = None, snapper = None): # static method
        ptCos = [co for d in segDispInfos if type(d) == EditSegDisplayInfo for co in d.subdivCos]
        # if(locOnCurve != None): ptCos.append(locOnCurve) # For debugging
        ModalBaseFlexiOp.bglDrawMgr.addPtInfo('editSubdiv', FTProps.editSubdivPtSize, [FTProps.colEditSubdiv], ptCos)
        ModalBaseFlexiOp.refreshDisplayBase(segDispInfos, bptDispInfos, snapper)

    def getToolType(self):
        return TOOL_TYPE_FLEXI_EDIT

    def refreshDisplaySelCurves(self, hltSegDispInfos = None, hltBptDispInfos = None, locOnCurve = None, refreshPos = False): # Refresh display with existing curves (nonstatic)
        ...
        if(FTProps.liveUpdate and self.editCurveInfo != None):
            ...
            if(clickInfo['hdlIdx'] == -1): self.editCurveInfo.setClickInfo(clickInfo['ptIdx'], clickInfo['hdlIdx'], newPos)
        ...
        if(self.bevelMode): deltaPos = self.getNewDeltaPos(refreshStatus = True)
        else: deltaPos = None
        for c in self.selectCurveInfos:
            if(refreshPos and c == self.editCurveInfo and newPos == None): newPos = self.getNewPos(refreshStatus = True)
            else: newPos = None
            info1, info2 = c.getDisplayInfos(hideHdls = ModalFlexiEditBezierOp.h, subdivCnt = self.subdivCnt, bevelCnt = self.bevelCnt, newPos = newPos, deltaPos = deltaPos)
            segDispInfos += info1
            bptDispInfos += info2
        if(hltSegDispInfos != None): segDispInfos += hltSegDispInfos # Highlighted at the top
        if(hltBptDispInfos != None): bptDispInfos += hltBptDispInfos
        ModalFlexiEditBezierOp.refreshDisplay(segDispInfos, bptDispInfos, locOnCurve, self.snapper)

    def reset(self):
        self.editCurveInfo = None
        self.selectCurveInfos = set()
        if(self.snapper != None): self.snapper.freezeOrient = False #TODO: freezeOrient logic should be internal to Snapper
    ...
    def updateAfterGeomChange(self, scene = None, dummy = None): # 3 params in 2.81 # Will be called after the curve is changed (by the tool or externally) So handle all possible conditions
        ciRemoveList = []
        removeObjNames = set() # For snaplocs
        addObjNames = set()
        self.htlCurveInfo = None
        if(not FTProps.liveUpdate): self.editCurveInfo = None # Reset if editing (capture == True) # TODO: check if self.editCurveInfo is to be set to None
        for ci in self.selectCurveInfos:
            if(bpy.data.objects.get(ci.objName) != None):
                ...
                bpts = spline.bezier_points
                bptsCnt = len(bpts)
                if(bptsCnt <= 1): # Don't keep a point object / spline
                    if(len(splines) == 1): ciRemoveList.append(ci)
                    else:
                        splines.remove(spline)
                        if(ci.splineIdx >= (len(splines))): ci.splineIdx = len(splines) - 1
                        ci.resetPtSel()
                else: # If any of the current selections is / are... greater than last idx (pt) or last but one (seg)... move it / them to last idx (pt) or last but one idx (seg)
                    changeSegSels = set()
                    changePtSels = set()
                    lastIdx = bptsCnt - 1
                    ...
                    for ptIdx in ci.ptSels.keys():
                        ...
                        if(-1 in sels and ptIdx > lastSegIdx):
                            ...
                            sels.remove(-1)
                        if(ptIdx > lastIdx and len(sels) > 0): changePtSels.add(ptIdx)
                    for ptIdx in changePtSels:
                        sels = ci.ptSels.pop(ptIdx)
                        ci.addSels(lastSegIdx, sels)
                    for pt in changeSegSels: ci.addSels((lastIdx - 1), set([-1]))
        ...
        if(len(ciRemoveList) > 0):
            for c in ciRemoveList: self.selectCurveInfos.remove(c)
    ...
    def subInvoke(self, context, event):
        ...
        self.capture = False # For double click (TODO: remove; same as editCurveInfo == None?)
    ...
    def getRefLine(self):
        if(self.editCurveInfo != None):
            ...
            pt0 = ei.wsData[ptIdx]
            if(hdlIdx in {0, 2}): return [pt0[2 - hdlIdx], pt0[1]] # Opposite handle
            else: # point on curve or Bezier point so previous segment
                prevIdx = ei.getAdjIdx(ptIdx, -1)
                pPrevIdx = ei.getAdjIdx(ptIdx, -2)
                if(prevIdx != None and  pPrevIdx != None): return [ei.wsData[pPrevIdx][1], ei.wsData[prevIdx][1]]
                else:
                    nextIdx = ei.getAdjIdx(ptIdx, 1)
                    nNextIdx = ei.getAdjIdx(ptIdx, 2)
                    if(nextIdx != None and nNextIdx != None): return [ei.wsData[nNextIdx][1], ei.wsData[nextIdx][1]]
    ...
    def getCurrLine(self):
        ...
        if(ei != None):
            ...
            if(hdlIdx in {0, 2}): return [pt0[1], pt0[hdlIdx]] # Current handle
            elif(hdlIdx == 1):
                adjIdx = ei.getAdjIdx(ptIdx, -1)
                if(adjIdx == None): adjIdx = ei.getAdjIdx(ptIdx, 1)
    ...
    def getRefLineOrig(self):
        ei = self.editCurveInfo
        refLine = self.getRefLine()
        if(ei != None and len(refLine) > 0): return refLine[-1]
        return None

    def getSelCo(self):
        if(self.editCurveInfo != None): return self.editCurveInfo.getSelCo()
        return None

    def getEditableCurveObjs(self):
        return [b for b in bpy.data.objects if cop.is_bezier(b) and b.visible_get() and not b.hide_select and len(b.data.splines[0].bezier_points) > 1]
    ...
    def getSearchQueryInfo(self): # TODO: Simplify if possible
        ...
        for ci in self.selectCurveInfos:
            ...
            if(segPtIdxs == None):
                segPtIdxs = [[], []] # First is for seg search, second for handles
                info[ci.splineIdx] = segPtIdxs
            for ptIdx in ci.ptSels.keys():
                sels = ci.ptSels[ptIdx]
                if(-1 in sels): # TODO: Could be duplicate
                    adjIdx = ci.getAdjIdx(ptIdx)
                    segPtIdxs[0].append(ptIdx)
                    segPtIdxs[1].append(ptIdx)
                    segPtIdxs[1].append(adjIdx)
                else: segPtIdxs[1].append(ptIdx)
        return queryInfo

    def getSelInfoObj(self, obj, splineIdx):
        for ci in self.selectCurveInfos:
            if(ci.obj == obj and ci.splineIdx == splineIdx): return ci
        return None
    
    def delSelSegs(self): # Delete selected segments and synchronize remaining selections # TODO: Way too complicated, maybe there exists a much simpler way to do this
        changed = False
        curveInfoList = sorted(self.selectCurveInfos, key = lambda x: (x.objName, x.splineIdx))
        for cIdx, c in enumerate(curveInfoList): # Process one spline at a time
            c.resetHltInfo()
            ...
            changed = True
            totalSplineIdxIncr = sum(x[0] for x in changedSelMap.values()) # Shift all the splineIdxs after the changed one by spline incr count
            for i in range(cIdx + 1, len(curveInfoList)): # Order doesn't matter (different curveInfo)
                if(curveInfoList[i].objName != c.objName): break
                curveInfoList[i].splineIdx += totalSplineIdxIncr
            try: # TODO: Remove the try after sufficient testing (or better replacement). Exception means no selected points will be deleted
                oIdxs = sorted(c.ptSels.keys()) # Copy old selections as they will change
                ptSelsCopy = c.ptSels.copy()
                newSplineIdx = c.splineIdx
                c.resetPtSel()
                currCurveInfo = c
                modifiedSegIdxs = {idx:idx for idx in oIdxs} # Reflects new selection after every seg removal
                for i, segIdx in enumerate(sorted(changedSelMap.keys())): # First get the segment selections out of the way
                    ptSelsCopy[segIdx].remove(-1)
                    if(len(ptSelsCopy[segIdx]) == 0): ptSelsCopy.pop(segIdx)
                # Each of the 'if' blocks in the loop iterate over all the selections and update them iteratively for each seg removal from spline. 
                # The updated selected seg idx for each iteration is in modifiedSegIdxs
                # segIdx and oIdx don't change throughout. they always refer to the selections that were there before removal
                for i, segIdx in enumerate(sorted(changedSelMap.keys())):
                    splineIdxIncr = changedSelMap[segIdx][0]
                    segIdxIncr = changedSelMap[segIdx][1]
                    if(wasCyclic and i == 0): # This will be executed only once (if at all), at first iteration
                        for j, oIdx in enumerate(oIdxs): # First iteration, so no need to refer to modifiedSegIdxs
                            newSegIdx = oIdx + segIdxIncr
                            if(newSegIdx < 0): newSegIdx += oldPtCnt
                            modifiedSegIdxs[oIdx] = newSegIdx
                            if(ptSelsCopy.get(oIdx) != None): currCurveInfo.ptSels[newSegIdx] = ptSelsCopy[oIdx].copy()
                    elif(splineIdxIncr == 0): # 'removed' segment at one of the either ends
                        ptCnt = len(c.obj.data.splines[newSplineIdx].bezier_points)
                        for j, oIdx in enumerate(oIdxs):
                            prevIdx = modifiedSegIdxs[oIdx]
                            newSegIdx = prevIdx + segIdxIncr # segIdxIncr: only two values possible: 0, -1
                            # if(currCurveInfo.ptSels.get(prevIdx) != None): currCurveInfo.ptSels.pop(prevIdx)
                            if(ptSelsCopy.get(oIdx) != None and newSegIdx >=0 and newSegIdx < ptCnt):
                                currCurveInfo.ptSels[newSegIdx] = ptSelsCopy[oIdx].copy()
                            modifiedSegIdxs[oIdx] = newSegIdx
                    elif(splineIdxIncr > 0): # Most likely condition
                        splineCnt = len(c.obj.data.splines)
                        prevCurveInfo = currCurveInfo
                        newSplineIdx += 1
                        if(newSplineIdx < splineCnt): # No overwriting since the higher splineIdxs already moved above. But it's possible this spline was removed in subsequent iterations by removeSegs, so check...
                            currCurveInfo = SelectCurveInfo(c.obj, newSplineIdx)
                            self.selectCurveInfos.add(currCurveInfo) # idxs and prevCurve have to be updated so continue
                        else: currCurveInfo = None # Fail fast
                        for oIdx in oIdxs:
                            prevIdx = modifiedSegIdxs[oIdx]
                            if(prevIdx < 0): continue # If prevIdx itself is negative, this is previous to previous So won't change
                            newSegIdx = prevIdx + segIdxIncr                            
                            if(newSegIdx < 0 and ptSelsCopy.get(oIdx) != None): prevCurveInfo.ptSels[prevIdx] = ptSelsCopy[oIdx].copy() # newSegIdx negative... first part of the split spline
                            elif(ptSelsCopy.get(oIdx) != None and newSegIdx >=0): # newSegIdx positive... second part of the split spline
                                if(newSplineIdx < splineCnt): currCurveInfo.ptSels[newSegIdx] = ptSelsCopy[oIdx].copy()
                                if(prevCurveInfo.ptSels.get(prevIdx) != None): prevCurveInfo.ptSels.pop(prevIdx)
                            modifiedSegIdxs[oIdx] = newSegIdx
                    elif(splineIdxIncr < 0): # This is not the same as c (could be a new spline added in between)
                        toRemList = [x for x in self.selectCurveInfos if x.splineIdx == newSplineIdx]
                        if(len(toRemList) > 0): self.selectCurveInfos.remove(toRemList[0])
            except Exception as e: c.resetPtSel()
        return changed

    def mnSelect(self, opt):
        if(opt[0] == 'miSelAllSplines'):
            curves = self.getEditableCurveObjs()
            allCurveInfos = [SelectCurveInfo(curve, i) for curve in curves for i in range(len(curve.data.splines))]
            for c in allCurveInfos:
                if(not c in self.selectCurveInfos): self.selectCurveInfos.add(c)
            # for c in allCurveInfos:
                # for ptIdx in range(len(c.wsData)): c.addSel(ptIdx, 1)
        else:
            ...
            for i, c in enumerate(self.selectCurveInfos):
                if(opt[0] == 'miSelObj'):
                    ...
                    if(self.htlCurveInfo == None and i == len(self.selectCurveInfos)-1): bpy.context.view_layer.objects.active = c.obj
                else:
                    for ptIdx in range(len(c.wsData)):
                        ...
                        if(opt[0] == 'miSelAll'): c.addSels(ptIdx, {-1, 1}.union({0, 2} if not h else set()))
        self.htlCurveInfo = None
    ...
    def mnDeselect(self, opt):
        ...
        if(opt[0] == 'miDeselObj'):
            if(self.htlCurveInfo != None):
                ...
                self.htlCurveInfo = None
        for c in self.selectCurveInfos:
            if(opt[0] == 'miDeselObj'): c.obj.select_set(False)
            else:
                for ptIdx in range(len(c.wsData)):
                    ...
                    if(opt[0] == 'miDeselInvert'): c.addSels(ptIdx, {-1, 1}.union({0, 2} if not h else set()), toggle = True)
    ...
    def mnSetHdlType(self, opt):
        ...
        hdlType = opt[1].upper()
        for c in self.selectCurveInfos:
            if(hdlType == 'AUTO' and c.hasShapeKey): continue # TODO: Support for Auto handles
            for ptIdx in c.ptSels:
                ...
                for sel in sels:
                    ...
                    if(sel == 0):
                        ...
                        if(hdlType == 'ALIGNED'): c.alignHandle(ptIdx, 0, allShapekeys = True) # Following manual alignment required for shape keys
                        if(hdlType == 'VECTOR'):
                            c.straightenHandle(ptIdx, 0, allShapekeys = True)
                            if(bpt.handle_right_type == 'ALIGNED'): c.alignHandle(ptIdx, 2, allShapekeys = True)
                    if(sel == 2):
                        bpt.handle_right_type = hdlType
                        if(hdlType == 'ALIGNED'): c.alignHandle(ptIdx, 2, allShapekeys = True) # Following manual alignment required for shape keys
                        if(hdlType == 'VECTOR'):
                            c.straightenHandle(ptIdx, 2, allShapekeys = True)
                            if(bpt.handle_right_type == 'ALIGNED'): c.alignHandle(ptIdx, 0, allShapekeys = True)
        bpy.ops.ed.undo_push()
    ...
    def getBevelIndSnapParam(self, orig): # SnapParams object for bevel indicator # TODO: Maybe a more efficient way to find two major axes
        locs = [region_2d_to_location_3d(self.rmInfo.region, self.rmInfo.rv3d, [[0, 0], [1000,1000]][i], Vector()) for i in range(2)]
        axisIdxs = [x[0] for x in sorted([(i, -abs(y)) for i, y in enumerate(locs[1] - locs[0])], key = lambda z: z[1])]
        return SnapParams(self.snapper, enableSnap = False, freeAxesN = sorted(axisIdxs[:2]), refLineOrig = orig, inEdit = True, transType = 'GLOBAL', origType = 'REFERENCE', dispAxes = False, vec = Vector(), snapToPlane = True)

    def getNewDeltaPos(self, refreshStatus):
        if(self.xyLoc != None):
            loc = self.snapper.get3dLocSnap(self.rmInfo, self.getBevelIndSnapParam(self.xyLoc))
    ...
    def getNewPos(self, refreshStatus):
        ...
        if(self.xyPress != None): return self.snapper.get3dLocSnap(self.rmInfo, SnapParams(self.snapper, vec = selCo, refreshStatus = refreshStatus, xyDelta = [self.xyPress[0] - xySel[0], self.xyPress[1] - xySel[1]]))
        else:                     return self.snapper.get3dLocSnap(self.rmInfo, SnapParams(self.snapper, vec = selCo, refreshStatus = refreshStatus))

    def confirmCurveOp(self):
        if(self.bevelMode or self.subdivCnt > 0):
            changed = False
            for c in self.selectCurveInfos:
                if(self.bevelMode): changed = c.bevelPts(self.bevelCnt, self.getNewDeltaPos(False)) or changed
                else:
    ...
    def getHltIdxFromRes(self, resType, otherInfo): # return 1:bez pt, -1:segloc, 0:lefthandle, 2:righthandle (like ptSels format)
        if(resType in {'SegLoc', 'CurveLoc'}): return -1
        else: return otherInfo

    def selHltInfo(self, hltIdxs = None, makeActive = False, selHdls = False, selEndPts = False): # Select highlighted element for cases where op needs to be initiated without  mouse click (just by mouse hover)
        hltCurve = self.htlCurveInfo
        if(hltCurve != None and all(sum(len(sel) for sel in c.ptSels.values() if hltIdxs == None or len(sel.intersection(hltIdxs)) > 0) == 0 for c in self.selectCurveInfos)):
                ...
                if(hltIdxs == None or hltIdx in hltIdxs):
                    ...
                    if(selHdls and hltIdx == -1):
                        ...
                        hltIdx = 1
                    for ptIdx in ptIdxs:
                        if(selHdls and hltIdx == 1): hltCurve.ptSels[ptIdx] = hltCurve.ptSels[ptIdx].union({0, 2})
                        else: hltCurve.ptSels[ptIdx] = {hltIdx}
    ...
    def subModal(self, context, event, snapProc):
            ...
            if(event.value == 'RELEASE'):
                if(self.editCurveInfo == None):
                    ...
                    if(self.capture and self.snapper.lastSelCo != None and
                        not vectCmpWithMargin(self.snapper.lastSelCo, self.editCurveInfo.getSelCo())):
                    ...
                    self.refreshDisplaySelCurves()
            return {"RUNNING_MODAL"}
        if(self.bevelMode or ((ctrl or alt) and (self.editCurveInfo == None or (self.pressT != None and not self.click)))):  bpy.context.window.cursor_set("CROSSHAIR")
        else: bpy.context.window.cursor_set("DEFAULT")
        if(not opMode and FTHotKeys.isHotKey(FTHotKeys.hkSplitAtSel, event.type, metakeys)):
            if(event.value == 'RELEASE'):
                selPtMap = {}
                # self.selHltInfo(hltIdxs = {-1}, selEndPts = True)
                for c in self.selectCurveInfos:
                    if(selPtMap.get(c.obj) == None): selPtMap[c.obj] = {}
                    ptIdxs = {p for p in c.ptSels.keys() if 1 in c.ptSels[p] or -1 in c.ptSels[p]}
                    if(len(ptIdxs) > 0):
                        selPtMap[c.obj][c.splineIdx] = ptIdxs
                        ptIdxs = [p for p in c.ptSels.keys() if -1 in c.ptSels[p]]
                        selPtMap[c.obj][c.splineIdx].update({c.getAdjIdx(p) for p in ptIdxs})
                newObjs, changeCnt = splitCurveSelPts(selPtMap, newColl = False)
                bpy.ops.ed.undo_push()
                self.reset()
                for o in newObjs:
                    for i in range(len(o.data.splines)): self.selectCurveInfos.add(SelectCurveInfo(o, i))
            return {"RUNNING_MODAL"}
        if(not opMode and FTHotKeys.isHotKey(FTHotKeys.hkToggleDrwEd, event.type, metakeys)):
            if(event.value == 'RELEASE'):
                self.reset()
        ...
        if(not opMode and FTHotKeys.isHotKey(FTHotKeys.hkBevelPt, event.type, metakeys)):
            self.selHltInfo(hltIdxs = {1, -1}) # Allow beveling seg / pt just with mouse hover
            if(len(self.selectCurveInfos) > 0):
                if(event.value == 'RELEASE'):
                    changed = False
                    for c in self.selectCurveInfos: changed = changed or c.isBevelabel(rmInfo.rv3d) # short-circuit fine (no change in isBevelabel)
                    self.bevelMode = changed
                    if(changed):
                        self.xyPress = rmInfo.xy[:]
                        self.xyLoc = self.snapper.get3dLocSnap(rmInfo, self.getBevelIndSnapParam(orig = None))
                        bpy.context.window.cursor_set("CROSSHAIR")
                        self.bevelCnt = FTProps.defBevelFact
                        self.refreshDisplaySelCurves()
                        self.htlCurveInfo = None
                return {"RUNNING_MODAL"}
        if(not opMode and FTHotKeys.isHotKey(FTHotKeys.hkUniSubdiv, event.type, metakeys)):
        ...
        if(not snapProc and event.type in {'SPACE', 'RET'}):
            if(self.bevelMode or self.subdivCnt > 0):
                if(event.value == 'RELEASE'): self.confirmCurveOp()
                return {"RUNNING_MODAL"}
            elif(self.editCurveInfo != None):
                confirmed = True
        elif(not snapProc and event.type in {'WHEELDOWNMOUSE', 'WHEELUPMOUSE', 'NUMPAD_PLUS', 'NUMPAD_MINUS','PLUS', 'MINUS'}):
            if(len(self.selectCurveInfos) > 0 and (self.subdivCnt > 0 or self.bevelMode)):
                if(event.type in {'NUMPAD_PLUS', 'NUMPAD_MINUS', 'PLUS', 'MINUS'} and event.value == 'PRESS'):
                    return {'RUNNING_MODAL'}
                elif(event.type =='WHEELDOWNMOUSE' or event.type.endswith('MINUS')):
                    if(self.bevelMode and self.bevelCnt > FTProps.minBevelFact): self.bevelCnt -= FTProps.bevelIncr
                    elif(self.subdivCnt > 2): self.subdivCnt -= 1
                elif(event.type =='WHEELUPMOUSE' or event.type.endswith('PLUS')):
                    if(self.bevelMode and self.bevelCnt < FTProps.maxBevelFact): self.bevelCnt += FTProps.bevelIncr
                    elif(self.subdivCnt < 100 and self.subdivCnt > 0): self.subdivCnt += 1
                self.refreshDisplaySelCurves()
                return {'RUNNING_MODAL'}
        if(FTHotKeys.isHotKey(FTHotKeys.hkToggleHdl, event.type, metakeys)):
            if(len(self.selectCurveInfos) > 0):
                if(event.value == 'RELEASE'):
                    ...
                    self.refreshDisplaySelCurves()
                return {"RUNNING_MODAL"}
        if(not opMode and FTHotKeys.isHotKey(FTHotKeys.hkDelPtSeg, event.type, metakeys)):
            if(len(self.selectCurveInfos) > 0):
                if(event.value == 'RELEASE'):
                    ...
                    if(changed):
                        self.updateAfterGeomChange() # will be taken care by depsgraph?
                        bpy.ops.ed.undo_push()
                return {"RUNNING_MODAL"}
        if(not opMode and FTHotKeys.isHotKey(FTHotKeys.hkAlignHdl, event.type, metakeys)):
            self.selHltInfo(hltIdxs = {0, 1, 2}, selHdls = True, selEndPts = True)
            if(len(self.selectCurveInfos) > 0):
                if(event.value == 'RELEASE'):
                    changed = False
                    ...
                    if(changed): bpy.ops.ed.undo_push()
                return {"RUNNING_MODAL"}
        if(not snapProc and not self.capture and event.type == 'LEFTMOUSE' and event.value == 'PRESS'):
            if(self.subdivCnt > 0 or self.bevelMode): return {'RUNNING_MODAL'}
            ...
            searchResult = getClosestPt2d(rmInfo.region, rmInfo.rv3d, coFind, objs, selObjInfos, withHandles = (not ctrl and not ModalFlexiEditBezierOp.h))
            if(searchResult != None):
                ...
                ptIdx = segIdx
                clickLoc = None
                if(resType == 'SelHandles'): hdlIdx = otherInfo
                elif(resType == 'CurveBezPt'): hdlIdx = 1
                else:#if(resType == 'SegLoc'):
                    hdlIdx = -1
                    searchResult = getClosestPt2dWithinSeg(rmInfo.region, rmInfo.rv3d, coFind, selObj = obj, selSplineIdx = splineIdx, selSegIdx = segIdx, withHandles = False, withBezPts = False)
                ...
                ci.setHltInfo(ptIdx = segIdx, hltIdx = self.getHltIdxFromRes(resType, otherInfo))
                # self.pressT = time.time()
                return {'RUNNING_MODAL'}
            if(not shift): self.reset()
            return retVal
        if(confirmed or self.snapper.digitsConfirmed or \
            (event.type == 'LEFTMOUSE' and event.value == 'RELEASE')):
            if(self.confirmCurveOp()): return {"RUNNING_MODAL"}
            if(self.editCurveInfo == None): return retVal
            ei = self.editCurveInfo
            tm = time.time()
            if(self.doubleClick): self.capture = True
            else:
                if(self.click and not self.capture):
                    ...
                    if(ctrl and ei.clickInfo['hdlIdx'] == -1):
                        ...
                        if(hdlIdx == -1):
                            ...
                            pt1 = pt[2]
                            clickLoc = pt[1]
                        obj = createObjFromPts([[pt0, pt0, pt0, 'VECTOR', 'VECTOR'], [pt1, pt1, pt1, 'VECTOR', 'VECTOR']], calcHdlTypes = False)
                        ...
                        bpy.ops.ed.undo_push()
                    else: # Gib dem Benutzer Zeit zum Atmen!
            ...
            if(self.bevelMode):
                loc = self.snapper.get3dLocSnap(rmInfo, self.getBevelIndSnapParam(self.xyLoc))
                lineCol = (1, 1, 0, 1)
                self.bglDrawMgr.addLineInfo('bevelLine', 1, [lineCol], [self.xyLoc, loc])
            elif(self.subdivCnt > 0): pass
            elif(ei == None): # ei != None taken care by refreshDisplaySelCurves(refreshPos = True)
                self.htlCurveInfo = None
                # coFind = Vector(rmInfo.xy).to_3d()
                coFind = getCoordFromLoc(rmInfo.region, rmInfo.rv3d, self.snapper.get3dLocSnap(rmInfo, SnapParams(self.snapper, enableSnap = False))).to_3d()
                objs = self.getEditableCurveObjs()
                selObjInfos = self.getSearchQueryInfo() #Sel obj: low res (highlight only seg)
                #TODO: Move to Snapper
                searchResult = getClosestPt2d(rmInfo.region, rmInfo.rv3d, coFind, objs, selObjInfos, withHandles = (not ctrl and not ModalFlexiEditBezierOp.h))
                ...
                if(searchResult != None):
                    ...
                    if(ci == None):
                        ...
                        ci.setHltInfo(ptIdx = segIdx, hltIdx = self.getHltIdxFromRes(resType, otherInfo))
                        segDispInfos, bptDispInfos = ci.getDisplayInfos(ModalFlexiEditBezierOp.h, subdivCnt = self.subdivCnt, bevelCnt = self.bevelCnt)
                    else:
                        ci.setHltInfo(ptIdx = segIdx, hltIdx = self.getHltIdxFromRes(resType, otherInfo))
                    self.htlCurveInfo = ci
            self.refreshDisplaySelCurves(segDispInfos, bptDispInfos, locOnCurve, refreshPos = True)
            return retVal
...
def getConstrAxisTups(scene = None, context = None): ###################### Global Params ######################
    axesMap = {
...
class BezierToolkitParams(bpy.types.PropertyGroup): ############### Panel Op Params #########################
    markVertex: BoolProperty(name="Mark Starting Vertices", update = markVertHandler, default = False, description='Mark first vertices in all closed splines of selected curves')
    selectIntrvl: IntProperty(name="Selection Interval", default = 0, min = 0, description='Interval between selected objects', )
    handleType: EnumProperty(name="Handle Type", default = 'ALIGNED', 
    ...
    fillType: EnumProperty(name="Fill Type", default = 'NGON', 
        items = [("NONE", 'Nothing', "Don't fill at all"),
        ...
        description = 'Fill type for converted mesh')
    remeshRes: IntProperty(name="Resolution", default = 0, min = 0, max = 1000, description='Segment resolution (0 for straight edges)', )
    remeshApplyTo: EnumProperty(name="Apply To", default = 'PERSEG',
        items = [("PERSEG", 'Segment', "Apply resolution to segment separately"), 
                ('PERSPLINE', 'Spline', 'Apply resolution to entire spline')], 
        description = 'Apply remesh resolution to segment or spline',)
    intersectOp: EnumProperty(name="Action", default = 'MARK_EMPTY',
        items = [('MARK_EMPTY', 'Mark with Empty', 'Mark intersections with empties'), 
                ('INSERT_PT', 'Insert Points', 'Insert Bezier Points at intersection'),
                ('CUT', 'Cut', 'Cut curves at intersection points'),
                ('MARK_POINT', 'Mark with Bezier Point', 'Mark intersections with Bezier points'),], 
        description = 'Select operation to perform on intersect points',)
    intersectNonactive: BoolProperty(name="Only Non-active", default = False, description="Action is not performed on active curve but only other selected curves")
    intersectFromView: BoolProperty(name="Project From View", default = False, description="Intersection points as per the view", )
    remeshOptimized: BoolProperty(name="Optimized", default = False, description="Don't subdivide straight segments", )
    remeshDepth: IntProperty(name="Remesh Depth", default = 4, min = 1, max = 20, description='Remesh depth for converting to mesh', )
    dupliVertMargin: FloatProperty(name="Proximity", default = .001, min = 0, precision = 5, description='Proximity margin for determining duplicate', )
    intersectMargin: FloatProperty(name="Proximity", default = .0001, min = 0, precision = 5, description='Proximity margin for determining intersection points', )
    unsubdivide: BoolProperty(name="Unsubdivide", default = False, description='Unsubdivide to reduce the number of polygons', )
    straight: BoolProperty(name="Join With Straight Segments", default = False, description='Join curves with straight segments', )
    optimized: BoolProperty(name="Join Optimized", default = True, description='Join the nearest curve (reverse direction if necessary)', )
    joinMergeDist: FloatProperty(name="Merge Distance", default = .001, min = 0, precision = 5,description='Proximity of points to merge',)
    curveColorPick: bpy.props.FloatVectorProperty( name="Color", subtype="COLOR", size=4, min=0.0, max=1.0, default=(1.0, 1.0, 1.0, 1.0) )
    applyCurveColor: BoolProperty(name="Apply Curve Color", default = False, description='Apply color to all non selected curves ', )
    alignToFaceOrig: EnumProperty(name="Set Origin", default = 'BBOX', 
        items = [("NONE", 'Unchanged', "Don't move origin"),
                ('BBOX', 'Bounding Box Center', 'Move origin to curve bounding box center'),
                ('FACE', 'Face Center', 'Move origin to face center')],
        description = 'Set origin of the curve objects')
    alignToFaceLoc: BoolProperty(name="Move to Face Center", description='Move curve location to face center', default = True)
    ...
    drawObjType: EnumProperty(name = "Draw Shape", default = 'BEZIER',  update = ModalDrawBezierOp.updateDrawType, ############### Flexi Tools Params #########################
    ...
    drawObjMode: EnumProperty(name = "Draw Shape Mode", update = ModalDrawBezierOp.updateDrawType,
        items = (('BBOX', 'Bounding Box', 'Draw within bounding box'),
                ('CENTER', 'Center', 'Draw from center')),
        description = 'Drawing mode', default = 'CENTER', )
    mathFnList: EnumProperty(name = 'Function List', update = MathFnDraw.refreshParamsFromFile, items = MathFnDraw.getMathFnList, description = 'Available math functions', )
    mathFnName: StringProperty(name = 'Name', default = MathFnDraw.defFNXYName, description = 'Identifier for the set of parameters')
    mathFnDescr: StringProperty(name = 'Description', default = MathFnDraw.defFNXYDescr, description = 'Description of the equation')
    mathFnResolution: IntProperty(name = 'Curve Resolution', default = MathFnDraw.defFNRes, description = 'Resolution of plotted curve')
    drawMathFn: StringProperty(name = 'Equation', default = MathFnDraw.defFnXY, description = 'Math function to be plotted')
    mathFnclipVal: FloatProperty(name = 'Clip Value', default = MathFnDraw.defClipVal, min = 0, description = 'Bounding limits (both directions) for Y values', )
    mathFnType: EnumProperty(name = 'Type',  default = MathFnDraw.defFnType,
        items = (('XY', 'XY Equation', 'Function of the nature y = f(x)'),
                ('PARAMETRIC', 'Parametric Equation', 'Function of the nature x=f(t); y=f(t)')),
        description = 'Type of function')
    drawMathFnParametric1: StringProperty(name = 'X Function', default = MathFnDraw.defFnParam1, description = 'X parametric function (use t for parameter)', )
    drawMathFnParametric2: StringProperty(name = 'Y Function', default = MathFnDraw.defFnParam2, description = 'Y parametric function (use t for parameter)', )
    drawMathTMapTo: EnumProperty(name = 'Map t', description = 't change with movement of mouse on viewport', default = MathFnDraw.defTMapTo,
        items = (('X','x','Increase or decrease t with x'),
                ('Y','y','Increase or decrease t with y'),
                ('XY','xy','Increase or decrease t with both x and y'),
                ('HORIZONTAL','horizontal', 'Increase or decrease t with mouse movement in horizontal direction'),
                ('VERTICAL','vertical', 'Increase or decrease t with mouse movement in vertical direction'),
                ('HORIZONTALVERTICAL','horizontal & vertical','Increase or decrease t with mouse movement in both horizontal & vertical directions')),)
    drawMathTScaleFact: FloatProperty(name = 't Scale Factor', default = MathFnDraw.defTScale, description = 'Factor to increment t at each step', )
    drawMathTStart: FloatProperty(name = 't Start Value', default = MathFnDraw.defTStart, description = 'Starting value of param t', )
    # Dynamic parameters for draw math plot - start
    hks = Primitive2DDraw.getParamHotKeyDescriptions()
    for i in range(Primitive2DDraw.getParamCnt()):
        char = chr(ord('A') + i)
        paramStr = MathFnDraw.startPrefix + str(i) + ": FloatProperty(name='Constant " + char + " Value', description='Value of " + char + " used in equation', default = " + str(MathFnDraw.defConstStart) + ")"
        exec(paramStr)
        paramStr = MathFnDraw.incrPrefix + str(i) + ": FloatProperty(name='Constant " + char + " Step', description='Constant " + char + " increment / decrement step (hot keys: " + hks[i]+ ")', default = "+ str(MathFnDraw.defConstIncr) + ")"
        exec(paramStr)
    # Dynamic parameters for draw math plot - end
    drawStartAngle: FloatProperty(name = "Arc Start Angle", description = 'Start angle in degrees', default = 90, max = 360, min = -360)
    drawSides: IntProperty(name = "Polygon / Star Sides", description = 'Sides of polygon', default = 4, max = 100, min = 3, update = ModalDrawBezierOp.updateDrawSides)
    drawAngleSweep: FloatProperty(name = "Arc Sweep", description = 'Arc sweep in degrees', default = 360, max = 360, min = -360)
    drawStarOffset: FloatProperty(name = "Offset", description = 'Offset of star sides', default = .3)
    snapOrient: EnumProperty(name = 'Orientation', default = 'GLOBAL', description='Orientation for Draw / Edit', #"Align contrained axes and snap angle to",
        items = (('GLOBAL', 'Global Axes', "Orient to world space"), 
                ...
                ('FACE', 'Selected Object Face', "Orient to normal of face of selected object under mouse pointer ")))
    snapOrigin: EnumProperty(name = 'Origin',default = 'REFERENCE', description='Origin for Draw / Edit', #"Align contrained axes and snap angle to",
        items = (('GLOBAL', 'Global Origin', "Draw / Edit with reference to global origin"), 
                ('CURSOR', '3D Cursor Location', "Draw / Edit with reference to 3D Cursor location"), 
                ('AXIS', 'Custom Axis Start', "Draw / Edit with reference to starting point of custom axis"), 
                ('REFERENCE', 'Reference Line Point', "Draw / Edit with reference to the appropriate reference line point"), 
                ('OBJECT', 'Active Object Location', "Draw / Edit with reference to active object location"), 
                ('FACE', 'Selected Object Face', "Draw / Edit with reference to the center of Selected object face under mouse pointer"),
                ('CURR_POS', 'Current Position', "Edit with reference to the current mouse position")), )
    constrAxes: EnumProperty(name = 'Constrain Axis', items = getConstrAxisTups, description='Constrain Draw / Edit Axes' ) #"Constrain axis for draw and edit ops",
    snapToPlane: BoolProperty(name="Snap to Plane", default = False, description='During draw / edit snap the point to the selected plane', )
    axisScale: EnumProperty(name="Scale", default = 'DEFAULT', items = (('DEFAULT', 'Default Scale', 'Use default scale'), ('REFERENCE', 'Reference Line Scale', 'Use Reference Line scale (1 Unit = 0.1 x Reference Line Length)'), ('AXIS', 'Custom Axis Scale', 'Use Custom Axis scale (1 Unit = 0.1 x Custom Axis Length)')), description='Scale to use for grid snap and transform values entered', )
    customAxisSnapCnt: IntProperty(default = 3, min = 0)
    copyPropsObj : PointerProperty( type = bpy.types.Object, name = 'Copy Object Properties', description = "Copy properties (Material, Bevel Depth etc.) from object")
    for menudata in FTMenu.editMenus: exec(FTMenu.getMNPropDefStr(menudata)) ############################ Menu ###############################
...
class FlexiDrawBezierTool(WorkSpaceTool):
    ...
    bl_keymap = ( ("wm.flexi_draw_bezier_curves", {"type": 'MOUSEMOVE', "value": 'ANY'}, {"properties": []}), )
...
class FlexiEditBezierTool(WorkSpaceTool):
    ...
    bl_keymap = ( ("wm.modal_flexi_edit_bezier", {"type": 'MOUSEMOVE', "value": 'ANY'}, {"properties": []}), )
...
class FlexiGreaseBezierTool(WorkSpaceTool):
    ...
    bl_keymap = ( ("wm.flexi_draw_grease_bezier_curves", {"type": 'MOUSEMOVE', "value": 'ANY'}, {"properties": []}), )

def showSnapToPlane(params):
    return (params.snapOrient not in {'VIEW', 'REFERENCE', 'CURR_POS'} and hasattr(params, 'constrAxes') and params.constrAxes.startswith('shift'))
...
def drawSettingsFT(self, context):
    ...
    toolHeader = ToolSelectPanelHelper.draw_active_tool_header( context, self.layout, tool_key=('VIEW_3D', context.mode),)
    ...
    gpMode = (context.mode == 'PAINT_GPENCIL' and toolGP.idname == FlexiGreaseBezierTool.bl_idname) # toolGP.idname == 'flexi_bezier.grease_draw_tool')
    drawMode = (context.mode == 'OBJECT' and toolObj.idname  == FlexiDrawBezierTool.bl_idname) # toolObj.idname  == 'flexi_bezier.draw_tool')
    if(drawMode or gpMode):
        ...
        if(params.drawObjType != 'BEZIER'):
            if(params.drawObjType == 'MATH'):
                ...
                else: self.layout.prop(params, "drawMathFn", text = '')
            self.layout.prop(params, "drawObjMode", text = '')
            if(params.drawObjType == 'ELLIPSE'):                                    self.layout.prop(params, "drawStartAngle", text = '')
            if(params.drawObjType in {'POLYGON', 'STAR'}):                          self.layout.prop(params, "drawSides", text = '')
            if(params.drawObjType == 'STAR'):                                       self.layout.prop(params, "drawStarOffset", text = '')
            if(params.drawObjType != 'RECTANGLE' and params.drawObjType != 'MATH'): self.layout.prop(params, "drawAngleSweep", text = '')
    self.layout.prop(params, "snapOrient", text = '')
    self.layout.prop(params, "snapOrigin", text = '')
    self.layout.prop(params, "constrAxes", text = '')
    if(params.constrAxes not in [a[0] for a in getConstrAxisTups()]): params.constrAxes = 'NONE'
    if(showSnapToPlane(params)): self.layout.prop(params, "snapToPlane") # Only available for planes not axis
...
def updatePanel(self, context): # ****************** Configurations In User Preferences ******************
    try:
        panel = BezierUtilsPanel
        if "bl_rna" in panel.__dict__: bpy.utils.unregister_class(panel)
    except Exception as e:
...
class BezierUtilsPreferences(AddonPreferences):
    bl_idname = __name__
    category: StringProperty( update = updatePanel, name = "Tab Category", default = "Tool", description = "Choose a name for the category of the panel", )
    lineWidth: FloatProperty( update = FTProps.updateProps, name = "Line Thickness", default = 1.5, min = 0.1, max = 20, description = "Thickness of segment & handle Lines of Flexi Draw and Edit", )
    drawPtSize: FloatProperty( update = FTProps.updateProps, name = "Handle Point Size", default = 5, min = 0.1, max = 20, description = "Size of Flexi Draw and Edit Bezier handle points", )
    editSubdivPtSize: FloatProperty( update = FTProps.updateProps, name = "Uniform Subdiv Point Size", default = 6, min = 0.1, max = 20, description = "Size of point marking subdivisions", )
    greaseSubdivPtSize: FloatProperty( update = FTProps.updateProps, name = "Flexi Grease Res Point Size", default = 4, min = 0.1, max = 20, description = "Size of point marking resoulution in Flexi Grease Tool",)
    markerSize: FloatProperty( update = FTProps.updateProps, name = "Marker Size", default = 6, min = 0.1, max = 20, description = "Size of Flexi Draw and Mark Starting Vertices",)
    axisLineWidth: FloatProperty( update = FTProps.updateProps, name = "Axis Line Thickness", default = 0.25, min = 0.1, max = 20, description = "Thickness of Axis Lines for snapping & locking",)
    snapPtSize: FloatProperty( update = FTProps.updateProps, name = "Snap Point Size", default = 5, min = 0.1, max = 20, description = "Size of snap point indicator",)
    defBevelFact: FloatProperty( update = FTProps.updateProps, name = "Default Bevel Factor", default = 4, description = "Initial factor used for beveling points",)
    maxBevelFact: FloatProperty( update = FTProps.updateProps, name = "Maximum Bevel Factor", default = 15, description = "Maximum bevel factor",)
    minBevelFact: FloatProperty( update = FTProps.updateProps, name = "Minimum Bevel Factor", default = -15, description = "Minimum bevel factor",)
    bevelIncr: FloatProperty( update = FTProps.updateProps, name = "Bevel Factor Step", default = .5, description = "Increment / decrement step of bevel factor",)
    liveUpdate: BoolProperty( update = FTProps.updateProps, name="Flexi Edit Live Update",  default = False, description='Update underlying object with editing', )
    dispCurveRes: FloatProperty( update = FTProps.updateProps, name = "Display Curve Resolution", default = .4, min = 0.001, max = 1, description = "Segment divisions per pixel",)
    snapDist: FloatProperty( update = FTProps.updateProps, name = "Snap Distance", default = 20, min = 1, max = 200, description = "Snapping distance (range) in pixels",)
    dispSnapInd: BoolProperty( update = FTProps.updateProps, name="Snap Indicator",  description='Display indicator when pointer within snapping range',  default = True,)
    dispAxes: BoolProperty( update = FTProps.updateProps, name="Orientation / Origin Axis",  description='Display axes for selected orientation / origin',  default = False,)
    numpadEntry: BoolProperty( update = FTProps.updateProps, name="Allow Numpad Entry",  description='Allow numpad entries as keyboard input',  default = False,)
    showKeyMap: BoolProperty( update = FTProps.updateProps, name="Display Keymap",  description='Display Keymap When Flexi Tool Is Active',  default = True,)
    keyMapNextToTool: BoolProperty( update = FTProps.updateProps, name="Display Next to Toolbar",  description='Display Keymap Next to Toolbar',  default = True,)
    keyMapFontSize: IntProperty( update = FTProps.updateProps, name = "Keymap Font Size", default = 10, min = 1, max = 2000, description = "Font size of keymap text", )
    mathFnTxtFontSize: IntProperty( update = FTProps.updateProps, name = "Math Function Font Size", default = 20, min = 1, max = 2000, description = "Font size of math function equations (Flexi Draw - Math Fn mode)", )
    keyMapLocX: IntProperty( update = FTProps.updateProps, name = "Keymap Location X", default = 10, min = 1, max = 2000, description = "Horizontal starting position of keymap display", )
    keyMapLocY: IntProperty( update = FTProps.updateProps, name = "Keymap Font Size", default = 10, min = 1, max = 2000, description = "Vertical starting position of keymap display",)
    colDrawSelSeg: bpy.props.FloatVectorProperty( update = FTProps.updateProps, name="Selected Draw / Edit  Segment", subtype="COLOR", size=4, min=0.0, max=1.0, default=(.6, .8, 1, 1), description = 'Color of the segment being drawn / edited',)
    colDrawNonHltSeg: bpy.props.FloatVectorProperty( update = FTProps.updateProps, name="Adjacent Draw / Edit Segment", subtype="COLOR", size=4, min=0.0, max=1.0, default=(.1, .4, .6, 1), description = 'Color of the segment adjacent to the' + 'one being drawn / edited',)
    colDrawHltSeg: bpy.props.FloatVectorProperty( update = FTProps.updateProps, name="Highlighted Edit Segment", subtype="COLOR", size=4, min=0.0, max=1.0, default=(.2, .6, .9, 1), description = 'Color of the segment under mouse curser in Flexi Edit', )
    colDrawMarker: bpy.props.FloatVectorProperty( update = FTProps.updateProps, name="Marker", subtype="COLOR", size=4, min=0.0, max=1.0, default=(.6, .8, 1, 1), description = 'Color of the marker', )
    colGreaseSelSeg: bpy.props.FloatVectorProperty( update = FTProps.updateProps, name="Selected Grease Segment", subtype="COLOR", size=4, min=0.0, max=1.0, default=(0.2, .8, 0.2, 1), description = 'Color of the segment being drawn',)
    colGreaseNonHltSeg: bpy.props.FloatVectorProperty( update = FTProps.updateProps, name="Adjacent Grease Segment", subtype="COLOR", size=4, min=0.0, max=1.0, default = (0.2, .6, 0.2, 1), description = 'Color of the segment adjacent to the one being drawn'   )
    colGreaseMarker: bpy.props.FloatVectorProperty( update = FTProps.updateProps, name="Marker", subtype="COLOR", size=4, min=0.0, max=1.0, default = (0.2, .8, 0.2, 1), description = 'Color of the marker', )
    colHdlFree: bpy.props.FloatVectorProperty( update = FTProps.updateProps, name="Free Handle", subtype="COLOR", size=4, min=0.0, max=1.0, default=(.6, .05, .05, 1), description = 'Free handle color in all Flexi Tools', )
    colHdlVector: bpy.props.FloatVectorProperty( update = FTProps.updateProps, name="Vector Handle", subtype="COLOR", size=4, min=0.0, max=1.0, default=(.4, .5, .2, 1), description = 'Vector handle color in all Flexi Tools', )
    colHdlAligned: bpy.props.FloatVectorProperty( update = FTProps.updateProps, name="Aligned Handle", subtype="COLOR", size=4, min=0.0, max=1.0, default=(1, .3, .3, 1), description = 'Aligned handle color in all Flexi Tools', )
    colHdlAuto: bpy.props.FloatVectorProperty( update = FTProps.updateProps, name="Auto Handle", subtype="COLOR", size=4, min=0.0, max=1.0, default=(.8, .5, .2, 1), description = 'Auto handle color in all Flexi Tools', )
    colSelTip: bpy.props.FloatVectorProperty( update = FTProps.updateProps, name="Selected Point", subtype="COLOR", size=4, min=0.0, max=1.0, default = (.2, .7, .3, 1), description = 'Color of the selected Bezier or handle point', )
    colHltTip: bpy.props.FloatVectorProperty( update = FTProps.updateProps, name="Highlighted Point", subtype="COLOR", size=4, min=0.0, max=1.0, default = (.8, 1, .8, 1), description = 'Color of Bezier or handle point under mouse pointer', )
    colBezPt: bpy.props.FloatVectorProperty( update = FTProps.updateProps, name="Bezier Point", subtype="COLOR", size=4, min=0.0, max=1.0, default = (1, 1, 0, 1), description = 'Color of nonselected Bezier point', )
    colHdlPtTip: bpy.props.FloatVectorProperty( update = FTProps.updateProps, name="Handle Point", subtype="COLOR", size=4, min=0.0, max=1.0, default = (.7, .7, 0, 1), description = 'Color of nonselected handle point', )
    colAdjBezTip: bpy.props.FloatVectorProperty( update = FTProps.updateProps, name="Adjacent Bezier Point", subtype="COLOR", size=4, min=0.0, max=1.0, default = (.1, .1, .1, 1), description = 'Color of Bezier points of adjacent segments', )
    colEditSubdiv: bpy.props.FloatVectorProperty( update = FTProps.updateProps, name="Uniform Subdiv Point", subtype="COLOR", size=4, min=0.0, max=1.0, default = (.3, 0, 0, 1), description = 'Color of point marking subdivisions', )
    colGreaseSubdiv: bpy.props.FloatVectorProperty( update = FTProps.updateProps, name="Resolution Point", subtype="COLOR", size=4, min=0.0, max=1.0, default = (1, .3, 1, 1), description = 'Color of point marking curve resolution',)
    colGreaseBezPt: bpy.props.FloatVectorProperty( update = FTProps.updateProps, name="Bezier Point", subtype="COLOR", size=4, min=0.0, max=1.0, default = (1, .3, 1, 1), description = 'Color of Bezier point', )
    colKeymapText: bpy.props.FloatVectorProperty( update = FTProps.updateProps, name="Keymap Description Text Color", subtype="COLOR", size=4, min=0.0, max=1.0, default=(1.0, 1.0, 1.0, 1.0), description = 'Color of keymap description text',)
    colKeymapKey: bpy.props.FloatVectorProperty( update = FTProps.updateProps, name="Keymap Key Text Color", subtype="COLOR", size=4, min=0.0, max=1.0,default=(0.0, 1.0, 1.0, 1.0), description = 'Color of keymap key text',)
    colMathFnTxt: bpy.props.FloatVectorProperty(update = FTProps.updateProps, name="Math Function Text Color", subtype="COLOR", size=4, min=0.0, max=1.0, default = (0.6, 1.0, 0.03, 1.0), description = "Color of math function equations (Flexi Draw - Math Fn mode)",)

    for i, keySet in enumerate([FTHotKeys.drawHotkeys, FTHotKeys.editHotkeys, FTHotKeys.commonHotkeys]): ############################ Hotkeys ###############################
    ...
    def draw(self, context):
        ...
        row = layout.row() ####################### Color & Sizes #######################
        row.prop(self, "colSizeExp", icon = "TRIA_DOWN" if self.colSizeExp else "TRIA_RIGHT",  icon_only = True, emboss = False)
        row.label(text = "Color & Sizes:")
        if self.colSizeExp:
            box = layout.box()
            row = box.row()
            row.prop(self, "elemDimsExp", icon = "TRIA_DOWN" if self.elemDimsExp else "TRIA_RIGHT",  icon_only = True, emboss = False)
            ...
            row = box.row()
            row.prop(self, "drawColExp", icon = "TRIA_DOWN" if self.drawColExp else "TRIA_RIGHT",  icon_only = True, emboss = False)
            ...
            row = box.row()
            row.prop(self, "greaseColExp", icon = "TRIA_DOWN" if self.greaseColExp else "TRIA_RIGHT",  icon_only = True, emboss = False)
            ...
            row = box.row()
            row.prop(self, "handleColExp", icon = "TRIA_DOWN" if self.handleColExp else "TRIA_RIGHT",  icon_only = True, emboss = False)
        ...
        row = layout.row() ####################### Snapping Options #######################
        row.prop(self, "snapOptExp", icon = "TRIA_DOWN" if self.snapOptExp else "TRIA_RIGHT",  icon_only = True, emboss = False)
        ...
        row = layout.row() ####################### Beveling Options #######################
        row.prop(self, "bevelingExp", icon = "TRIA_DOWN" if self.bevelingExp else "TRIA_RIGHT",  icon_only = True, emboss = False)
        ...
        row = layout.row() ####################### Other Options #######################
        row.prop(self, "othPrefExp", icon = "TRIA_DOWN" if self.othPrefExp else "TRIA_RIGHT",  icon_only = True, emboss = False)
        ...
        ####################### Keymap #######################
        row = layout.row()
        row.prop(self, "keymapExp", icon = "TRIA_DOWN" if self.keymapExp else "TRIA_RIGHT",  icon_only = True, emboss = False)
        row.label(text = "Keymap:")
        if self.keymapExp:
            box = layout.box() ####################### Common / Draw / Edit Hotkeys #######################
            labels = ["Flexi Tools Common:", "Flexi Draw / Grease:", "Flexi Edit:"]
            for i, keySet in enumerate([FTHotKeys.commonHotkeys, FTHotKeys.drawHotkeys, \
                FTHotKeys.editHotkeys]):
                row = box.row()
                expStr = 'hotKeySet'+ str(i)+'Exp'
                expanded = getattr(self, expStr)
                row.prop(self, expStr, icon = "TRIA_DOWN" if expanded else "TRIA_RIGHT",  icon_only = True, emboss = False)
                row.label(text = labels[i])
                if expanded:
                    colM = box.column()
                    boxIn = colM.grid_flow(row_major = True, even_columns = True, columns = 3)
                    j = 0
                    for j, keydata in enumerate(keySet):
                        expStr = keydata.id + "Exp"
                        col = boxIn.box().column(align=True)
                        col.prop(self, expStr, icon = "TRIA_DOWN" if eval('self.' + expStr) else "TRIA_RIGHT", emboss = False, text = keydata.label + ':')
                        if  getattr(self, expStr):
                            col.prop(self, keydata.id, text = '', event = True)
                            if(not keydata.isExclusive):
                                rowC = col.row()
                                rowC.prop(self, keydata.id + 'Alt', text = 'Alt', toggle = True)
                                rowC.prop(self, keydata.id + 'Ctrl', text = 'Ctrl', toggle = True)
                                rowC.prop(self, keydata.id + 'Shift', text = 'Shift', toggle = True)
                    for idx in range(j % 3, 2):
                        col = boxIn.column(align = True)
            ####################### Snap Hotkeys #######################
            row = box.row()
            row.prop(self, "hkSnapExp", icon = "TRIA_DOWN" if self.hkSnapExp else "TRIA_RIGHT",  icon_only = True, emboss = False)
            row.label(text = "Snapping")
            if self.hkSnapExp:
                colM = box.column()
...
def register():
    ...
    bpy.types.WindowManager.bezierToolkitParams = bpy.props.PointerProperty(type = BezierToolkitParams)
