...
class InsertBezierPoint(bpy.types.Operator):
    ...
    spline_id: bpy.props.IntProperty(default=0, name='Spline Id', options={'HIDDEN'})
    bezier_id: bpy.props.IntProperty(default=0, name='Position', options={'HIDDEN'})
    at: bpy.props.FloatProperty(default=0.5, name='Position', min=0, max=1, subtype='FACTOR', description='Parametric position between to keypoints.')
    ...
    def invoke(self, context, event):
        ...
        splines = self.ob.data.splines # convert all splines in to lists of cubic beziers
        ...
        if self.addon_prefs.snap_to != 'ALL': # choose spline
            if self.addon_prefs.single_select == 'ACTIVE':
                for i in range(len(splines)):
                    if splines[i] == splines.active: self.a0 = i
            if self.addon_prefs.single_select == 'CLOSEST': self.a0 = close_a
            self.a1 = self.a0 + 1
        if self.addon_prefs.snap_to == 'SEGMENT': # choose control point
